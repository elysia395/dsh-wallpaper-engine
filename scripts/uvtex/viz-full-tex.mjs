...
// 完整 ASCII 轮廓，每 32px 一行，全部 118 行，行首标注 y%
const stepX = 32, stepY = 32;
const rows = Math.ceil(h / stepY);
for (let gy = 0; gy < rows; gy++) {
  let row = '';
  for (let gx = 0; gx < Math.ceil(w / stepX); gx++) {
    let cnt = 0, total = 0;
    for (let y = gy * stepY; y < Math.min(h, (gy + 1) * stepY); y += 8) {
      for (let x = gx * stepX; x < Math.min(w, (gx + 1) * stepX); x += 8) {
        total++;
  ...
  // 行标：每 8 行标一次
  const pct = (gy * stepY / h * 100).toFixed(0);
  const label = gy % 8 === 0 ? pct.padStart(3) + '%' : '   ';
