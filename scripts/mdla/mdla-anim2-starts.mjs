...
// 动画 2 数据 @254324 (相对 mdla)
const DATA2 = mdla + 254324;
const TOTAL_ENTRIES = Math.floor((mdle - DATA2) / 36);
console.log('动画2 数据条目:', TOTAL_ENTRIES);

function decodeForBoneAt(b, entryIdx) {
  const rot = (2 * b) % 9;
...
// 逐骨骼找块起点: 从 cursor 起找解码后 anchor 匹配的 entry
let cursor = 0;
console.log('=== 逐骨骼 anchor 匹配找块起点 ===');
const blockStarts = [];
for (let b = 0; b < 53; b++) {
  const ax = bones[b].mtxT[0], ay = bones[b].mtxT[1];
  let start = -1;
  for (let k = cursor; k < Math.min(cursor + 100, TOTAL_ENTRIES); k++) {
    const un = decodeForBoneAt(b, k);
    if (isFinite(un[0]) && Math.abs(un[0] - ax) < 3 && Math.abs(un[1] - ay) < 3) { start = k; break; }
  }
  if (start < 0) { console.log(`  B${b}: 未找到起点 @cursor=${cursor}`); break; }
  // 连续匹配长度
  let len = 0;
  for (let k = start; k < TOTAL_ENTRIES; k++) {
    const un = decodeForBoneAt(b, k);
    if (isFinite(un[0]) && Math.abs(un[0] - ax) < 3 && Math.abs(un[1] - ay) < 3) len++;
    else break;
  }
  blockStarts.push({ b, start, len });
  console.log(`  B${String(b).padStart(2)}: start=${start} len=${len}`);
  cursor = start + len;
}
console.log('cursor 结束:', cursor, '/', TOTAL_ENTRIES);
console.log('\n块起点间隔:', blockStarts.slice(0, 12).map(x => `${x.start}(${x.len})`).join(' '));
