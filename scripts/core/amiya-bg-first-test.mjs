...
function analysis(r, label) {
  ...
  console.log(`${label}: 头覆盖 ${(diff/total*100).toFixed(1)}% 头区域与背景相同 ${(sameBg/total*100).toFixed(1)}% 头内容(${head.join(',')})`);
}
for (const mode of ['array', 'deep-first', 'deep-first-bg-first']) {
  try {
    const r = make(mode);
    analysis(r, mode);
