// 官方 puppet 定位变体测试:
// A) origin+raw (当前)
// B) origin + size/2 + raw (图像局部→场景, 头整体上移 size/2)
// C) origin - size/2 + raw (左上角锚)
// 关键: 组件关系 (头 vs 右眼) 应保持 dx=-21 dy=-2.5 (用户确认的正确值)
...
function make(anchor) {
  ...
  r.renderPuppet = function(o, model, tr, t) {
    ...
    else if (anchor === 'origin+half+raw') { baseX = tr.origin[0] + size[0]/2 + rawBounds.minX; baseY = tr.origin[1] + size[1]/2 + rawBounds.maxY; }
    else if (anchor === 'origin-half+raw') { baseX = tr.origin[0] - size[0]/2 + rawBounds.minX; baseY = tr.origin[1] - size[1]/2 + rawBounds.maxY; }
    else if (anchor === 'origin+rawYflip') { baseX = tr.origin[0] + rawBounds.minX; baseY = tr.origin[1] - rawBounds.minY; }
    // 应用 -eye 视图平移 (同 renderImage)
...
for (const anchor of ['origin+raw', 'origin+half+raw', 'origin-half+raw', 'origin+rawYflip']) {
  const r = make(anchor);
  ...
  if (head && body) console.log(`  重叠: ${(head[3]-body[1]).toFixed(0)}px`);
  if (head && eyeR) {
    const hc = [(head[0]+head[2])/2, (head[1]+head[3])/2];
    const ec = [(eyeR[0]+eyeR[2])/2, (eyeR[1]+eyeR[3])/2];
    console.log(`  头中心(${hc[0].toFixed(0)},${hc[1].toFixed(0)}) 右眼(${ec[0].toFixed(0)},${ec[1].toFixed(0)}) dx=${(hc[0]-ec[0]).toFixed(1)} dy=${(hc[1]-ec[1]).toFixed(1)} (期望 dx=-21 dy=-2.5)`);
  }
}
