/**
 * 生成程序化骨骼动画帧序列（24 帧循环）。
 * 从 anim-skin-proto.mjs 提取核心逻辑，渲染多帧。
 */
import fs from 'fs';
...
function skin(t) {
  ...
  for (const v of verts) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      let amp = 0;
      if (b >= 38) amp = 0.07;
      else if (b >= 4) amp = 0.018;
      else if (b >= 1) amp = 0.012;
...
// 渲染 24 帧
const frameDir = path.join(OUT, 'anim_frames');
if (!fs.existsSync(frameDir)) fs.mkdirSync(frameDir);
let dims = null;
for (let f = 0; f < FRAMES; f++) {
  const t = (f / FRAMES) * Math.PI * 2;
  const positions = skin(t);
  const d = render(positions, 'anim_frames/frame_' + String(f).padStart(2, '0') + '.png');
  if (!dims) dims = d;
}
console.log('完成', FRAMES, '帧, 尺寸', dims.w + 'x' + dims.h, '→', frameDir);
