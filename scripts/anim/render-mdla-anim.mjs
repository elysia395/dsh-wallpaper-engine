...
// 渲染动画2 采样帧
const framesToRender = [1, 30, 65, 95, 125, 200, 400, 600];
fs.mkdirSync(path.join(OUT, 'mdla_frames'), { recursive: true });
const results = [];
for (const f of framesToRender) {
  ...
  const fn = `anim2_frame_${String(f).padStart(3)}.png`;
  fs.writeFileSync(path.join(OUT, 'mdla_frames', fn), encodePng(img.w, img.h, img.rgba));
  let nonZero = 0;
  for (let i = 3; i < img.rgba.length; i += 4) if (img.rgba[i] > 10) nonZero++;
  console.log(fn, img.w + 'x' + img.h, '非空', (100 * nonZero / (img.w * img.h)).toFixed(1) + '%');
  results.push({ f, img });
}

// 帧间差异
for (let i = 1; i < results.length; i++) {
  const a = results[0].img.rgba, b = results[i].img.rgba;
  let diff = 0;
  for (let j = 0; j < a.length; j += 4) {
    if (Math.abs(a[j] - b[j]) > 30 || Math.abs(a[j+1] - b[j+1]) > 30 || Math.abs(a[j+2] - b[j+2]) > 30) diff++;
  }
  console.log(`帧 ${results[0].f} vs ${results[i].f}: 差异像素 ${diff}`);
}
