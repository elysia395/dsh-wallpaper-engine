...
function rasterize(mesh, positions, tex) {
...
// 确定有效动画起点: 找到所有骨骼都"就位"的帧
// f=0 特殊帧 (scale 0), f=1..4 B22 等 pos=0. 有效起点 = 所有动画骨骼 pos != 0
function findValidStart(anim) {
  for (let f = 0; f < anim.frames; f++) {
    let allValid = true;
    for (let i = 0; i < anim.perBone.length; i++) {
      const p = anim.perBone[i].frames[f];
      // 特殊帧 = scale 接近 0 或 pos 为 0
      if (Math.abs(p.scale[0]) < 0.5 || Math.abs(p.scale[1]) < 0.5) { allValid = false; break; }
    }
    if (allValid) return f;
  }
  return 0;
}
const start1 = findValidStart(animData.anim1);
const start2 = findValidStart(animData.anim2);
console.log('动画1 有效起点 f=', start1, '动画2 有效起点 f=', start2);

// 生成动画1 全部 133 帧 (呼吸摆动)
fs.mkdirSync(path.join(OUT, 'mdla_anim1'), { recursive: true });
for (let f = 0; f < 133; f++) {
  const pose = poseFromAnim(animData.anim1, f);
  const positions = skinPositions(mesh, pose);
  const img = rasterize(mesh, positions, tex);
  fs.writeFileSync(path.join(OUT, 'mdla_anim1', `frame_${String(f).padStart(3)}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('动画1: 133 帧已生成');

// 生成动画2 全部 601 帧 (长发摆动)
fs.mkdirSync(path.join(OUT, 'mdla_anim2'), { recursive: true });
for (let f = 0; f < 601; f++) {
  const pose = poseFromAnim(animData.anim2, f);
  const positions = skinPositions(mesh, pose);
  const img = rasterize(mesh, positions, tex);
  fs.writeFileSync(path.join(OUT, 'mdla_anim2', `frame_${String(f).padStart(3)}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('动画2: 601 帧已生成');
