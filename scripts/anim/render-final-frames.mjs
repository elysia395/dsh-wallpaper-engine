...
// 生成最终 demo 用帧:
// 1. 人物呼吸帧 (anim2 f=12..132 摆动段, 统一画布) — 已存在 mdla_anim2_uniform
// 2. 人物眨眼帧 (anim1 全 133 帧, 统一画布) — 需要生成
// 3. 后发动画帧 — 已存在 backhair_frames (父子传递)
...
// 生成 anim1 统一画布 133 帧 (眨眼/头部动画)
const dir1 = path.join(OUT, 'anim1_uniform');
fs.mkdirSync(dir1, { recursive: true });
for (let f = 0; f < 133; f++) {
  const img = renderPose(pose1(f));
  fs.writeFileSync(path.join(dir1, `frame_${String(f).padStart(3, '0')}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('anim1 统一画布 133 帧 →', dir1);

// 生成 anim2 摆动段压缩版 (呼吸循环): f=12..132 每 2 帧 = 61 帧
const dir2 = path.join(OUT, 'breath_frames');
fs.mkdirSync(dir2, { recursive: true });
for (let k = 0; k < 61; k++) {
  const f = 12 + k * 2;
  const img = renderPose(pose2(f));
  fs.writeFileSync(path.join(dir2, `frame_${String(k).padStart(2, '0')}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('anim2 呼吸 61 帧 →', dir2);
