...
// 后发 MDL 解析（复用人物逻辑但用后发结构）
function parseBackhair(buf) {
...
const mesh = parseBackhair(mdlRaw);
console.log('后发 mesh:', mesh.vertexCount + 'v', '骨骼:', mesh.bones.filter(b => !b.error).length);
...
function decodeBone(b, f) {
  ...
  const un = [];
  for (let i = 0; i < 9; i++) un.push(dv.getFloat32(o + ((i + rot) % 9) * 4, true));
  return un;
}

function poseAt(f) {
  const pose = new Array(BONE_COUNT);
  for (let b = 0; b < BONE_COUNT; b++) {
    const un = decodeBone(b, f);
    pose[b] = { pos: un.slice(0, 3), rot: un.slice(3, 6), scale: un.slice(6, 9) };
  }
  return pose;
}
...
for (let f = 0; f < 133; f++) {
  const pose = poseAt(f);
  const pos = skinPositions(mesh, pose);
  allPoses.push(pos);
  for (const p of pos) {
...
console.log('后发全局边界:', { gMinX, gMaxX, gMinY, gMaxY });
...
function rasterize(positions) {
  ...
  for (let t = 0; t < indices.length; t += 3) {
    ...
    const a = [positions[i0][0] - gMinX, flipY(positions[i0][1])];
    const b = [positions[i1][0] - gMinX, flipY(positions[i1][1])];
    const c = [positions[i2][0] - gMinX, flipY(positions[i2][1])];
...
// 生成后发动画 133 帧 (跳过 f=132 特殊帧, 用 f=131 代替)
const dir = path.join(OUT, 'backhair_frames');
fs.mkdirSync(dir, { recursive: true });
for (let f = 0; f < 133; f++) {
  const ff = f === 132 ? 131 : f; // 特殊帧替换
  const img = rasterize(allPoses[ff]);
  fs.writeFileSync(path.join(dir, `frame_${String(f).padStart(2, '0')}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('后发动画 133 帧已生成 →', dir, W + 'x' + H);
fs.writeFileSync(path.join(OUT, 'backhair_frames.offset.json'), JSON.stringify({ size: [W, H], drawOffset: [gMinX, -gMaxY], bounds: { gMinX, gMaxX, gMinY, gMaxY } }));
