// v17: 完整矩阵蒙皮渲染人物帧
// 1. breath_frames2: anim2 f12..132 每 2 帧 = 61 帧 (呼吸, 含旋转+层级)
// 2. anim1_full: anim1 f0..132 全 133 帧 (头部/眼动画, 含旋转+层级)
// 3. anim1_inc: anim1 相对 f12 的增量帧 (仅 B4 等运动骨骼, 用于叠加到呼吸帧) — 生成混合帧用
// 混合帧: 呼吸基底 + anim1 增量 → 单帧渲染, 无残影
import fs from 'fs';
...
// ── 完整矩阵蒙皮 (2x3 affine: 平移+旋转+缩放, 层级累积) ──
...
function pose2(f) {
  ...
  for (let i = 0; i < N; i++) pose[i] = animData.anim2.perBone[i].frames[f];
  return pose;
}
function pose1(f) {
  const pose = new Array(N);
  for (let i = 0; i < N; i++) pose[i] = animData.anim1.perBone[i].frames[f];
  return pose;
}
// 混合姿态: anim2 基底 + anim1 相对 f12 增量 (additive 语义)
function poseMix(f2, f1) {
  const pose = new Array(N);
  for (let i = 0; i < N; i++) {
    const p2 = animData.anim2.perBone[i].frames[f2];
    const p1 = animData.anim1.perBone[i].frames[f1];
    const b1 = animData.anim1.perBone[i].frames[12];
    if (!p2 || !p1 || !b1) { pose[i] = p2; continue; }
    const dx = (isFinite(p1.pos[0]) && isFinite(b1.pos[0]) && Math.abs(p1.pos[0]) < 5000 && Math.abs(b1.pos[0]) < 5000) ? p1.pos[0] - b1.pos[0] : 0;
    const dy = (isFinite(p1.pos[1]) && isFinite(b1.pos[1]) && Math.abs(p1.pos[1]) < 5000 && Math.abs(b1.pos[1]) < 5000) ? p1.pos[1] - b1.pos[1] : 0;
    // rot 增量 (anim1 rot - anim1 f12 rot)
    const r0 = p1.rot[2] - b1.rot[2];
    pose[i] = {
...
// ── 生成 1. breath_frames2 (anim2 呼吸, 61 帧, 完整蒙皮) ──
const dirB = path.join(OUT, 'breath_frames_v17');
fs.mkdirSync(dirB, { recursive: true });
for (let k = 0; k < 61; k++) {
  const f = 12 + k * 2;
  const skinned = buildSkinFull(pose2(f));
  const img = rasterize(skinned);
  fs.writeFileSync(path.join(dirB, `frame_${String(k).padStart(2, '0')}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('breath_frames_v17 (完整蒙皮+旋转) 61 帧 →', dirB);

// ── 生成 2. anim1 全帧 (133 帧, 完整蒙皮) ──
const dirA = path.join(OUT, 'anim1_full_v17');
fs.mkdirSync(dirA, { recursive: true });
for (let f = 0; f < 133; f++) {
  const skinned = buildSkinFull(pose1(f));
  const img = rasterize(skinned);
  fs.writeFileSync(path.join(dirA, `frame_${String(f).padStart(3, '0')}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('anim1_full_v17 133 帧 →', dirA);

// ── 生成 3. 眨眼混合帧: 呼吸基底 + anim1 增量 (f2=0..60, f1 眨眼段 12..67..12) ──
// 眨眼窗口: f1 = 12 → 67 → 12 (55 帧去程 + 55 帧回程 = 110 帧), 呼吸帧对应取模
const dirM = path.join(OUT, 'blink_mix_v17');
fs.mkdirSync(dirM, { recursive: true });
const BLINK_FRAMES = [];
for (let i = 0; i <= 55; i++) BLINK_FRAMES.push(12 + Math.round(i * (67 - 12) / 55));
for (let i = 1; i <= 54; i++) BLINK_FRAMES.push(67 - Math.round(i * (67 - 12) / 55));
console.log('眨眼帧序列:', BLINK_FRAMES.length, '帧, f1', BLINK_FRAMES[0], '→', BLINK_FRAMES[55], '→', BLINK_FRAMES[BLINK_FRAMES.length-1]);
for (let k = 0; k < BLINK_FRAMES.length; k++) {
  const f1 = BLINK_FRAMES[k];
  const f2 = 12 + (k % 61) * 2;
  const skinned = buildSkinFull(poseMix(f2, f1));
  const img = rasterize(skinned);
  fs.writeFileSync(path.join(dirM, `frame_${String(k).padStart(3, '0')}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('blink_mix_v17 (呼吸+眨眼增量混合) 110 帧 →', dirM);

// 验证: 呼吸 f0 vs 眨眼混合帧差异
const fB = fs.readFileSync(path.join(dirB, 'frame_00.png'));
const fM = fs.readFileSync(path.join(dirM, 'frame_55.png'));
console.log('呼吸 frame_00:', fB.length, 'B; 眨眼混合 frame_55:', fM.length, 'B');
