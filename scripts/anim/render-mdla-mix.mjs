...
// 混合姿态: anim2[f2] + anim1[f1] 的位移叠加
// 位移 = (pos - anchor)，叠加后回到 anchor + 总位移
function blendedPose(f2, f1, anim1Blend = 1) {
  const pose = new Array(animData.anim2.perBone.length);
  for (let i = 0; i < animData.anim2.perBone.length; i++) {
    const p2 = animData.anim2.perBone[i].frames[f2];
    const p1 = animData.anim1.perBone[i].frames[f1];
    const anc = animData.bones[i].anchor;
    if (!p2 || !p1) { pose[i] = p2; continue; }
    // 只有有效 pos (< 5000) 才应用
    const d2x = (isFinite(p2.pos[0]) && Math.abs(p2.pos[0]) < 5000) ? p2.pos[0] - anc[0] : 0;
    const d2y = (isFinite(p2.pos[1]) && Math.abs(p2.pos[1]) < 5000) ? p2.pos[1] - anc[1] : 0;
    const d1x = (isFinite(p1.pos[0]) && Math.abs(p1.pos[0]) < 5000) ? (p1.pos[0] - anc[0]) * anim1Blend : 0;
    const d1y = (isFinite(p1.pos[1]) && Math.abs(p1.pos[1]) < 5000) ? (p1.pos[1] - anc[1]) * anim1Blend : 0;
    pose[i] = {
      pos: [anc[0] + d2x + d1x, anc[1] + d2y + d1y, 0],
...
// 生成混合动画: 动画2摆动段 (f2=12..132) × 动画1呼吸 (f1 对应相位)
// 动画1 133帧, 呼吸周期: f1 = 12 + ((f2-12) * 133 / 121) % 121 近似同步
const dir = 'D:/dsh-wallpaper-engine/scene-layers-out/anim_frames_mdla_mix';
fs.mkdirSync(dir, { recursive: true });
const N = 61;
for (let k = 0; k < N; k++) {
  const f2 = 12 + k * 2;
  const f1 = 12 + ((f2 - 12) * 100 / 120) % 121;
  const pose = blendedPose(f2, Math.round(f1), 0.8);
  const positions = skinPositions(mesh, pose);
  const img = rasterize(positions);
  fs.writeFileSync(path.join(dir, `frame_${String(k).padStart(2)}.png`), encodePng(img.w, img.h, img.rgba));
}
console.log('混合动画 (长发+呼吸) 61 帧生成 →', dir);

// 对比: 纯长发 vs 混合
const fs2 = fs.readFileSync('D:/dsh-wallpaper-engine/scene-layers-out/anim_frames/frame_30.png');
const mix = fs.readFileSync(path.join(dir, 'frame_30.png'));
console.log('纯长发 frame_30:', fs2.length, 'B; 混合 frame_30:', mix.length, 'B');
