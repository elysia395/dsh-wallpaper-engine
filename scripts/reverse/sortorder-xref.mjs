// 找 sortorder/transparentsorting/customsortorder 字符串 RVA + xref
...
const names = ['sortorder', 'customsortorder', 'transparentsorting', 'REVERSEDEPTH'];
const targets = {};
...
for (let i = textSec.roff; i < textSec.roff + textSec.rsize - 7; i++) {
  ...
  for (const [n, t] of Object.entries(targets)) {
    if (target === t) results[n].push({ fileOff: i, rva: instrRva, va: 0x140000000 + instrRva });
  }
}
for (const [n, hits] of Object.entries(results)) {
  console.log(`${n}: ${hits.length} 处引用`);
  hits.slice(0, 5).forEach(h => console.log(`  va=0x${h.va.toString(16)}`));
}
