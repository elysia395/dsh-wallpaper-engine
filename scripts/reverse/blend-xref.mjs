// 找 blendin/blendout/animationlayer 的 xref — 动画层混合代码定位
...
// 字符串位置
const names = ['blendin', 'blendout', 'playSingleAnimation', 'getAnimationLayer', 'getAnimationLayerCount', 'createAnimationLayer', 'blend'];
const targets = {};
...
for (let i = textSec.roff; i < textSec.roff + textSec.rsize - 7; i++) {
  ...
  for (const [n, t] of Object.entries(targets)) {
    if (target === t) results[n].push({ fileOff: i, va: 0x140000000 + instrRva });
  }
}
for (const [n, hits] of Object.entries(results)) {
  console.log(`${n}: ${hits.length} 处`);
  hits.slice(0, 4).forEach(h => console.log(`  va=0x${h.va.toString(16)}`));
}
