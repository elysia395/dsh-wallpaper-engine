...
const { src, width, height, time, times, weAssetsDir, frameDelayMs, videoFrames, fmt, rawVideo, runScripts } = workerData;
...
try {
  ...
  if (!times || !times.length) {
    const renderer = new SceneRenderer(src, { width, height, time, weAssetsDir, videoFrames, runScripts, log: () => {}, onDegraded: collectDegraded });
    ...
    const renderer = new SceneRenderer(src, { width, height, time: times[0], times, weAssetsDir, videoFrames, runScripts, log: () => {}, onDegraded: collectDegraded });
    const frames = [];
