  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf11 = workshop scene.pkg (PKGV) + JPEG/TTF 解码 +
        // solidlayer + 对象旋转 + puppet 骨骼蒙皮 + 属性动画 + 依赖循环防护.
        const key = 'sf11_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
