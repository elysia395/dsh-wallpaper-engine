...
import { resolveSceneTexPath, pkgSceneAccess, dirSceneAccess, parseTex, decodeTex, setGlobalAssetsRoots, addGlobalAssetsRoot, readGlobalAsset, extractTexVideoMp4 } from './pkg-extract.js';
...
import { buildPuppetPayload, buildPuppetAnchors } from './we-renderer/puppet-export.js';
...
// P0-3①: 视频引用扫描 memoize (诊断导出 extractSceneVideoFrames 的底层缓存) —
...
// Windows 兼容: 定位真实 WE 安装后, 把 <install>/assets 补进 GL 全局素材根。
// resolveLooseDirs 默认仅 linux 有值 (~/Pictures/WallpaperEngine), 不设
// DSH_WE_ASSETS_DIR 时 Windows/macOS 上 setGlobalAssetsRoots 默认列表为空 →
// GL gate 的全局纹理 (粒子 halo/drop 等不随 pkg 分发) 全 miss。CPU 路径本就
// 吃 weAssetsDir=<install> 无此问题。locateWallpaperEngineP 有 60s TTL/并发
// 缓存, GL/CPU 两入口重复调用近零成本; 探测失败保持 null (下次请求重试)。
async function ensureWeInstallLocatedP() {
  if (_weInstallDirCache === null) {
    const dir = await locateWallpaperEngineP();
    if (dir) {
      _weInstallDirCache = dir;
      try { addGlobalAssetsRoot(join(dir, 'assets')); } catch { /* 登记失败无碍 */ }
    }
  }
  return _weInstallDirCache;
}
...
// 场景 ortho 宽高比缓存 (GL 场景 schema 修正用 — 视口比例 ≠ 场景
...
function sceneAspect(abs) {
  ...
  try {
    ...
    if (abs.toLowerCase().endsWith('.json')) {
      ...
      sc = pkgSceneAccess(new Uint8Array(readFileSync(abs))).readJson('scene.json');
    }
...
// 静态帧) / vid_* (视频纹理抽帧) / sv2_*
// (san_* 前缀已随 scene-anim 移除停增, 旧产物由启动清扫一次删净, 模式保留兜底)
...
function sanitizeSettings(raw) {
  ...
  return {
    ...
    // sceneGL: GL 实时渲染总开关 (默认开)。CPU 渲染路径已移除 — 关闭即纯静态帧。
    // 新增开关必须在此登记 — 缺失会被 sanitize 静默丢弃 (UI 切了没反应根因)。
    // 旧键 betaSceneAnim/enableSceneScripts/sceneGLDegrade 已随 CPU 渲染路径移除,
    // 旧配置残留键在此静默丢弃 (GL 默认开 = 全员升级到 GL on)。
    sceneGL: o.sceneGL !== false,
    sceneGLExperimental: o.sceneGLExperimental !== false,
    pauseOnHidden: o.pauseOnHidden !== false,
    pauseOnBlur: o.pauseOnBlur !== false, // 默认开（镜像 src/client.js）：遮挡满转触发 i915 GPU reset 的实测崩溃链
...
// scene-anim 产物清扫 (CPU 渲染路径移除后): san_* (动画产物, 可达几十 MB) 与
// *.prog (渲染进度文件) 全部是旧路径孤儿 — 启动时一次清空释放磁盘;
// frames 目录其余前缀 (sf/vid/sv2_) 仍由 gcFrameCacheSync 字节预算管理。
function sweepSceneProgArtifacts() {
  const dir = frameCacheDir();
  let names = [];
  try { names = readdirSync(dir); } catch { return; }
  for (const name of names) {
    if (!(/^san_/.test(name) || /\.prog$/.test(name))) continue;
    try { unlinkSync(join(dir, name)); } catch { /* ignore */ }
  }
}
...
// WE 全局素材根登记 (模块级一次): 官方 WE 从 <安装目录>/assets 解析不随壁纸
// 分发的全局纹理 (粒子 halo/drop/rosepetals 等)。本机无 WE 安装 → 默认取
// <每个壁纸根>/assets (用户在 ~/Pictures/WallpaperEngine/assets 自备的官方
// 素材), 可用 DSH_WE_ASSETS_DIR 覆盖 (逗号/分号分隔多根, ~ 展开同 loose)。
setGlobalAssetsRoots((() => {
  const raw = process.env.DSH_WE_ASSETS_DIR && process.env.DSH_WE_ASSETS_DIR.trim();
  if (raw) {
    return raw.split(/[,;]/).map((s) => s.trim()).filter(Boolean)
      .map((t) => (t === '~' ? homedir() : t.startsWith('~/') ? join(homedir(), t.slice(2)) : t));
  }
  return resolveLooseDirs().map((d) => join(d, 'assets'));
})());
...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // HEAD = 缓存探测 (GL 抓帧上传前的空槽判断): 只查磁盘, 绝不触发渲染。
      // 200 = 已有缓存帧, 204 = 无。键语义与 GET 完全一致 (sf39 + mtime)。
      if (method === 'HEAD') {
        const key = 'sf39_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
        const dir = ensureFrameCacheDir();
        const hit = existsSync(join(dir, key + '.png')) || existsSync(join(dir, key + '.jpg'));
        res.statusCode = hit ? 200 : 204;
        res.end();
        return;
      }
      // P2-11: 负缓存键 (失败收口的 .catch 在 async 作用域之外, 需在此层声明)。
      let negKey = null;
      (async () => {
        ...
        // sf39 = 提取器修复 (decodeTex 裁剪 2 的幂 mip 填充 + 拼合跳过木偶
        // atlas — 3784528825 部件坐标拼接异常)。旧键帧 (sf38 及以前) 作废。
        // sf38 = CPU 渲染全部屏蔽 (用户决策 2026-09-02 晚): 静态帧只走
        // 提取器 (0.8.1 修复版拼合/主纹理直出); GL 会话可用时缓存由 GPU
        // 抓帧回填 (PUT scene-frame-cache, 仅空槽)。
        // (sf37 = 曾短暂恢复的 CPU worker 渲染优先 — 已随决策撤除。)
        // P-12: mtime 失效键走 sceneCacheStamp — 全精度 mtimeMs + 松散场景
        // 纳入目录内引用文件的 mtime。
        const key = 'sf39_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
        const dir = ensureFrameCacheDir();
        ...
        negKey = key;
        // P2-11: 失败负缓存命中 → 直接回缓存错误码, 不再重复提取 (键含
        // sceneCacheStamp, mtime/内容变化自动换键失效; TTL 内失败结论视为
        // 仍成立)。
        ...
        if (!servePath) {
          ...
          if (!inflight) {
            inflight = (async () => {
              ...
              // 提取器实现 (多图层拼合/主纹理直出; JPEG 透传)。CPU 渲染链
              // 已屏蔽 — 这是唯一的静态帧实现。异步读盘: scene.pkg 可达几十
              // MB, 阻塞事件循环不可取。
              ...
              // GPU 抓帧先写 (scene-frame-cache 空槽回填): 已有帧直接用。
              if (existsSync(pngPath) || existsSync(jpgPath)) return existsSync(pngPath) ? pngPath : jpgPath;
              // 异步原子发布（.tmp+rename）：写入中途崩溃不留半截缓存文件。
              await atomicWriteFileP(target, frame.bytes);
              // P-10/N-05: 前缀字节预算 LRU (keepPath = 刚写入的本帧)
              gcFrameCacheSync(dir, target);
              return target;
            })();
            SCENE_FRAME_INFLIGHT.set(key, inflight);
            // 无论成败都摘除（失败允许后续请求重试）。
            inflight.then(
              // P2-11: 提取成功 → 负缓存条目主动失效
  ...
  // 3c1b. GPU 抓帧写入静态帧缓存: PUT /scene-frame-cache/<token>
  //      GL 会话渲染成功后从 canvas 抓一帧回传, 写入该壁纸的 sf39 缓存槽。
  //      仅空槽写入 (CPU 渲染结果或既有帧优先, 首写胜出 — 语义与 GET 的
  //      与 GET 唯一实现互不踩踏); 与 GET 共用 sf39 键 (abs+mtime)。
  //      每壁纸一份缓存, 谁先创建谁生效 (用户决策 2026-09-02)。
  disposers.push(webServer.register({
    kind: 'prefix',
    ...
    handler: (req, res) => {
      ...
      const pathname = new URL(req.url || '/', 'http://x').pathname;
      const token = decodeURIComponent(pathname.slice(`${BASE}/scene-frame-cache/`.length));
      const abs = mediaMap.get(token);
      if (!abs) {
        res.statusCode = 404;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
      ...
      req.on('data', (c) => {
        ...
        if (size > 32 * 1024 * 1024) {
          failed = true;
          try { req.destroy(); } catch { /* ignore */ }
          reply(413, { error: 'frame too large (>32MB)' });
          return;
        }
        chunks.push(c);
      });
      req.on('error', () => { if (!failed) { failed = true; reply(400, { error: 'body read error' }); } });
      req.on('end', () => {
        if (failed) return;
        const body = Buffer.concat(chunks);
        // 魔数校验: PNG (\x89PNG) / JPEG (\xff\xd8) — 其他一律拒绝, 防任意写。
        ...
        if (!isPng && !isJpg) { reply(415, { error: 'not a PNG/JPEG payload' }); return; }
        const key = 'sf39_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
        const dir = ensureFrameCacheDir();
        const pngPath = join(dir, key + '.png');
        const jpgPath = join(dir, key + '.jpg');
        // 空槽判断: 已有缓存帧 (提取器或早前抓帧) → 不覆盖。
        if (existsSync(pngPath) || existsSync(jpgPath)) {
          reply(200, { stored: false, reason: 'cache-exists' });
          return;
        }
        const target = isPng ? pngPath : jpgPath;
        atomicWriteFileP(target, body)
          .then(() => {
            // 负缓存无需显式清: 下次 GET 命中磁盘缓存时自行失效。
            gcFrameCacheSync(dir, target);
            reply(200, { stored: true, bytes: body.length });
          })
          .catch((e) => reply(500, { error: String(e && e.message ? e.message : e) }));
      });
    },
  }));
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // W5: 文字位图 (gate 期 renderText 栅格化, _glTextCache 按 access 缓存)
      if (subpath.startsWith('__text__/')) {
        const tIdx = Number(subpath.slice(9));
        const tAccess = Number.isInteger(tIdx) && tIdx >= 0 ? glSceneAccess(abs, null) : null;
        const tMap = tAccess ? _glTextCache.get(tAccess) : null;
        const hit = tMap ? tMap.get(tIdx) : null;
        if (!hit) {
          res.statusCode = 404;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({ error: 'text-not-rendered' }));
          return;
        }
        res.setHeader('Content-Type', 'image/png');
        res.setHeader('Cache-Control', 'no-store');
      ...
      try {
        ...
        if (!bytes) {
          ...
          const file = access.readFile(texPath) || readGlobalAsset(texPath);
  ...
  // scene-shader（官方 ES 1.00 shader 展开下发）。sceneGL 总开关 403 门控
  // (默认开) + token=base64url(abs)。GL 失败客户端落静态帧。

  // pkg/目录访问 LRU（key=abs|mtime, cap 2 — 附录 §6 定案）
  const _glPkgCache = new Map();
  // W4: 木偶 payload 缓存 — gate 期解析物化, /scene-puppet 下发。键 = access
  // 对象 (glSceneAccess LRU 同 abs 返回同对象 → 天然随 LRU 失效, 无 token 穿透)
  const _glPuppetCache = new WeakMap(); // access → Map<objIdx, payload>
  // W5: 文字位图 PNG 缓存 — 同款 WeakMap<access> (gate 期 renderText 栅格化 +
  // encodePng, /scene-resource __text__/<idx> 分支下发; idx 按 gate 调用内
  // 文字对象发射序递增, 与清单 mainTexture.path 严格同源)
  const _glTextCache = new WeakMap(); // access → Map<textIdx, {png,width,height}>
  ...
  // 壁纸是否含内嵌脚本 (W0 gate 标记用; 原 lib/scene-scripts.js 导出, CPU 执行
  // 路径移除后仅剩此判定): 递归找 {script: string, value} 节点, 首个命中即返回。
  function sceneHasScripts(scene) {
    let found = false;
    const walk = (obj) => {
      if (found || !obj || typeof obj !== 'object') return;
      if ('script' in obj && 'value' in obj && typeof obj.script === 'string') { found = true; return; }
  ...
  function glTexInfo(access, ref) {
    ...
    // GIF/精灵表帧元数据 (像素单位, TEXS): isAnimatedGif 多帧纹理若按整图
    // 采样, 每个粒子会画出整张帧网格 (sf40g 泡泡方阵)。>1 帧才下发。
    const frames = Array.isArray(info.frames) && info.frames.length > 1
      ? info.frames.map((f) => ({
        x: Number(f.x) || 0, y: Number(f.y) || 0,
        width: Number(f.width) || 0, height: Number(f.height) || 0,
      }))
      : undefined;
    // sf51: 帧时长 = 有效 frametime 均值 (逐字镜像 CPU loadTexImage BASE-24,
    // textures.js:185-194) — 图像路径精灵表轮帧节奏 (粒子路径按帧矩形 UV 采样,
    // 不消费 duration)。
    let frameDuration;
    if (frames) {
      let total = 0, known = 0;
      ...
      frameDuration = known ? total / known : 0.1;
    }
    return {
      path: texPath,
  ...
  const SCENE_GL_ENGINE = 'dsh-we-scene-gl/10'; // sf54: 清单下发 colorBlendMode 9 (BlendAdd 加性合成) + 带效果木偶走网格路径采样效果链输出 (W6 视频纹理 + W4 木偶网格仍在)
  ...
  function sceneGLCheck(scene, access, opts) {
    ...
    const degraded = [];
    // fail 携带 degraded — meta 硬拒路径要下发跳过清单 (W0 意图: 硬拒壁纸
    // 也能显示"内嵌脚本未运行"等标记; 此前 fail 丢弃 degraded 使该路径恒空)
    const fail = (reason) => ({ ok: false, reason, degraded });
    ...
    // W0: 内嵌脚本提醒 — GL 快照式架构从不加载/执行脚本 (CPU 执行路径已随
    // 其渲染器移除, enableSceneScripts 开关同废)。配置页据此显示"内嵌脚本未运行"。
    if (sceneHasScripts(scene)) mark(null, 'scene-script', '壁纸含内嵌脚本，未执行（GL 渲染不支持脚本）');
    ...
    // A6: 外部 project.json 用户属性 (pkg 模式下 project.json 是 pkg 旁独立文件,
    // pkg 内无该条目 — CPU _readUserProps BASE-34 同款; 外部值覆盖 pkg 内)。
    // {user:{condition,name},value} 条件绑定语义: visible = (属性值 === condition)
    // (3735447194 rover combo male/female 分别绑定 泡-中/泡-中-女)。
    const mergedUserProps = (() => {
      const base = userProps || {};
      const ext = opts && opts.extUserProps && typeof opts.extUserProps === 'object' ? opts.extUserProps : null;
      return ext ? Object.assign({}, base, ext) : base;
    })();
    const glVisibleSelf = (o) => {
      const v = o && o.visible;
      if (v == null) return true;
      if (typeof v === 'object' && v !== null && 'user' in v) {
        if (typeof v.user === 'object' && v.user !== null && v.user.name != null) {
          const up = mergedUserProps[v.user.name];
          if (up !== undefined) {
            const val = up && typeof up === 'object' && 'value' in up ? up.value : up;
            return String(val) === String(v.user.condition);
          }
          return !valHidden(v.value);
        }
        if (typeof v.user === 'string' && v.user) {
          const up = mergedUserProps[v.user];
          if (up !== undefined) {
            const val = up && typeof up === 'object' && 'value' in up ? up.value : up;
            return !valHidden(val);
          }
        }
        return !valHidden(v.value);
      }
    ...
    // W7 (0.8.12): 补 attachment 锚点 (MDAT0001) — 子对象 attachment 字段把 origin
    // 锚定到父 puppet 的**命名锚点**: 锚点偏移 = 该锚点骨骼绑定位姿 (tx,ty,angle)
    // + 按骨骼旋角旋转后的锚点矩阵平移, 与子自身 origin 同空间 (父局部, 先加锚点
    // 再加 origin, 同受祖先 scale/旋转影响)。缺失时 (无 attachment / 父非 puppet /
    // MDL 无 MDAT / 名字不匹配 / 骨骼索引越界) 退化为纯链折叠。
    // 0.8.13: 该语义实测坐实 —— 3463520581 "头部/躯干错位" 的**真因不在锚点本身**,
    // 而是 MDLS 骨骼名解析截断 (asuna body bottom 声明 7 根只解析出 1 根 →
    // `Attachment bottom` 锚点 boneIdx=2 越界归零, 躯干组少走 (+230.3,+274.2)
    // 父局部偏移)。骨骼修好后本路径零系数即对齐; 详见 puppet.js _nextBoneHeader。
    const _glAnchorCache = new Map(); // mdl 路径 → { anchors, boneRT } | null
    const glAnchorInfo = (parentObj) => {
      const model = parentObj && typeof parentObj.image === 'string' ? access.readJson(parentObj.image) : null;
      if (!model || typeof model.puppet !== 'string' || !model.puppet) return null;
      if (_glAnchorCache.has(model.puppet)) return _glAnchorCache.get(model.puppet);
      let info = null;
      try { info = buildPuppetAnchors(access, model); } catch { info = null; }
      _glAnchorCache.set(model.puppet, info);
      return info;
    };
    const glAnchorOffset = (child, parentObj) => {
      if (!child || typeof child.attachment !== 'string' || !child.attachment) return null;
      const info = glAnchorInfo(parentObj);
      if (!info) return null;
      const anch = info.anchors.find((a) => a.name === child.attachment);
      if (!anch) return null;
      // 锚点骨骼绑定位姿 (越界索引按根骨骼 0,0,0 — 与 CPU 侧 guard 同款)
      let bx = 0, by = 0, ba = 0;
      if (anch.boneIdx >= 0 && anch.boneIdx < info.boneRT.length) {
        const r = info.boneRT[anch.boneIdx];
        bx = r.tx; by = r.ty; ba = r.angle;
      }
    ...
    const foldChain = (chain) => {
      ...
      for (let i = chain.length - 2; i >= 0; i--) {
        ...
        const ao = glAnchorOffset(chain[i], chain[i + 1]);
        if (ao) {
          const ax = ao[0] * scale[0], ay = ao[1] * scale[1];
          origin = [origin[0] + ax * cos - ay * sin, origin[1] + ax * sin + ay * cos, 0];
        }
    ...
    // W5: 静态文字栅格化 — 与 CPU text.js renderTextObject 同语义 (compensate/
    // 字体回退/颜色 0..1); 即时(时钟类)与水印文字由对象循环上游拦截, 不到这里。
    // 字体缓存键 = 解析后路径; 字体缺失/空文本返回 null (调用处降级跳过)。
    const _glFontCache = new Map();
    const glRenderTextBitmap = (obj) => {
      const text = String(glVal(obj, 'text', ''));
      if (!text) return null;
      const color = glVec(glVal(obj, 'color', '1 1 1'), 3, [1, 1, 1]);
      const pointsize = Number(glVal(obj, 'pointsize', 32)) || 32;
      let fontPath = String(glVal(obj, 'font', '') || '');
      if (fontPath.startsWith('systemfont_') || !fontPath) fontPath = 'fonts/NotoSans-Regular.ttf';
      let font = _glFontCache.get(fontPath);
      if (font === undefined) {
        let raw = null;
        const f1 = access.readFile(fontPath);
        if (f1) raw = f1.bytes;
        if (!raw) { const f2 = access.readFile('fonts/' + basename(fontPath)); if (f2) raw = f2.bytes; }
        try { font = raw ? parseCffFont(raw) : null; } catch { font = null; }
        _glFontCache.set(fontPath, font);
      }
      if (!font) return null;
      const scale = glVec(glVal(obj, 'scale', '1 1 1'), 3, [1, 1, 1]);
    ...
    const out = [];
    let idx = -1;
    let textIdx = 0; // W5: 文字对象 PNG 下发序 (__text__/<textIdx>, _glTextCache 键)
    for (const obj of objects) {
      ...
      if (obj.parent != null) {
        ...
        // A7: 旧版在此给有效父链打 "完整渲染由 CPU 侧负责" 降级标记 — 该文案已
        // 陈旧 (W3 起 GL 三路径 image/particle/puppet 全部消费 effTr, 父链在 GL
        // 端完整渲染, 无任何降级)。标记只会虚增 degraded 计数 (3427824116 6×parent
        // 全为噪音), 移除; 仅链断时仍标记。
        effTr = foldChain(chain);
      }
      // W3: 粒子对象进入 GL 清单（此前整体跳过）。带 image 的复合对象按 CPU
      // _classify 优先级落图像路径（image 优先于 particle），不在此分支。
      if ((obj.particle || obj.particles) && !obj.image) {
        const pRef = obj.particle != null ? obj.particle : obj.particles;
        const pDef = typeof pRef === 'string' ? access.readJson(pRef)
          : (pRef && typeof pRef === 'object' && !Array.isArray(pRef) ? pRef : null);
        if (!pDef) { mark(name, 'particle', '粒子定义不可读，已跳过该对象'); continue; }
        const pMat = pDef.material ? access.readJson(String(pDef.material)) : null;
        const pPass = pMat && Array.isArray(pMat.passes) && pMat.passes[0] ? pMat.passes[0] : null;
        // 音频驱动恒跳过（GL 无音频输入）：emitter audio 字段 / 材质
        // AUDIOPROCESSING combo / 定义或实例覆盖内含 registerAudioBuffers 脚本绑定
        // （3735447194 Audio Reactive Dots 即后者 — 脚本永不执行，音频语义已死）。
        const pInst = obj.instanceoverride && typeof obj.instanceoverride === 'object' ? obj.instanceoverride : {};
        const hasAudioScript = (x) => { try { return /registerAudioBuffers/.test(JSON.stringify(x)); } catch { return false; } };
        const pCombos = pPass && pPass.combos ? pPass.combos : {};
        if ((Array.isArray(pDef.emitter) && pDef.emitter.some((e) => e && e.audio != null))
          || String(pCombos.AUDIOPROCESSING ?? '0') !== '0'
          || hasAudioScript(pInst) || hasAudioScript(pDef)) {
          mark(name, 'particle', '音频响应未支持，已跳过该对象');
          continue;
        }
        // 纹理（CPU _buildParticleSystemInner 同款来源：材质 pass textures[0]）：
        // _rt_ 渲染目标恒跳过；视频纹理 GL 无法当静态精灵 → 跳过；
        // pkg 解析不到（WE 全局 assets 纹理，本机无 WE 安装）→ 不跳过，texture=null
        // 下发，客户端按 CPU null-tex 语义画纯色圆点（记 degraded 提示用户）。
        const pTexRef = pPass && Array.isArray(pPass.textures) ? pPass.textures[0] : null;
        const pTexRefName = typeof pTexRef === 'string' ? pTexRef
          : (pTexRef && typeof pTexRef.name === 'string' ? pTexRef.name : null);
        if (pTexRefName && pTexRefName.startsWith('_rt_')) { mark(name, 'particle', '_rt_ 渲染目标纹理未支持，已跳过该对象'); continue; }
        let pTex = null;
        if (pTexRefName) {
          const ti = glTexInfo(access, pTexRef);
          if (ti === undefined) mark(name, 'particle', '粒子纹理不可用，按无纹理圆点渲染');
          else if (ti && ti.video) { mark(name, 'particle', '粒子纹理不可读，已跳过该对象'); continue; }
          else pTex = ti;
        }
        // {user,value} 绑定按 CPU getVal 语义预解析（userProps 命中读 project.json
        // 当前值，否则回退 value）——客户端只吃纯值，不背 project.json 上下文。
        // 注意 userProps 是 project.json general.properties 原始表 (值带
        // {type,value} 包装), CPU _readUserProps 建表时解包 v.value — 此处同款
        // 解包 (本地 pkg 无 project.json 不可达, workshop 壁纸才命中)。
        const resolveBound = (x) => {
          if (Array.isArray(x)) return x.map(resolveBound);
          if (x && typeof x === 'object') {
            if ('value' in x) {
              if (typeof x.user === 'string' && x.user && userProps
                && Object.prototype.hasOwnProperty.call(userProps, x.user)) {
                const up = userProps[x.user];
                return up && typeof up === 'object' && 'value' in up ? resolveBound(up.value) : up;
              }
              return x.value;
            }
            const o2 = {};
            for (const [k, v] of Object.entries(x)) o2[k] = resolveBound(v);
            return o2;
          }
          return x;
        };
        // usershadervalues color1/color2（CPU 同款：经 userProps 解析的 vec3 字符串，
        // 绘制时按 p.color[0] 在 color1→color2 间 mix）
        let color1 = [1, 1, 1], color2 = [1, 1, 1];
        const usv = pPass && pPass.usershadervalues;
        if (usv && userProps) {
          for (const [prop, uniform] of Object.entries(usv)) {
            const uRaw = userProps[prop];
            const v = uRaw && typeof uRaw === 'object' && 'value' in uRaw ? uRaw.value : uRaw;
            if (typeof v !== 'string') continue;
            if (uniform === 'color1') color1 = glVec(v, 3, [1, 1, 1]) || [1, 1, 1];
            else if (uniform === 'color2') color2 = glVec(v, 3, [1, 1, 1]) || [1, 1, 1];
          }
        }
        // 对象静态性：属性动画 → 静态首帧渲染 + 标记（与 image 对象同款）
        for (const key of SCENE_GL_ANIM_KEYS) {
          if (glHasAnim(obj[key])) { mark(name, 'anim:' + key, '属性动画(' + key + ')未支持，已按静态渲染'); break; }
        }
        // controlpoint[id=0] flags&1 = WE 鼠标绑定控制点 → 发射器跟随鼠标
        //（契约 W3 一等场景：3735447194 MrDogTastic's Mouse Trail；客户端喂
        // pointermove 坐标作 emitter 位置源，鼠标未移动时按对象 origin）。
        const pCps = Array.isArray(pDef.controlpoint) ? pDef.controlpoint : [];
        const cp0 = pCps.find((c) => c && c.id === 0) || pCps[0] || null;
        const mousefollow = !!(cp0 && (Number(cp0.flags) & 1));
        const pAlpha = Number(glVal(obj, 'alpha', 1));
        const pBrightness = Number(glVal(obj, 'brightness', 1));
        out.push({
          name,
          type: 'particle',
          origin: glVec(glVal(obj, 'origin', '0 0 0'), 3, [0, 0, 0]),
          size: [0, 0], // 粒子无对象矩形（发射范围在 emitter distancemin/max）
          scale: glVec(glVal(obj, 'scale', '1 1 1'), 3, [1, 1, 1]),
          angles: glVec(glVal(obj, 'angles', '0 0 0'), 3, [0, 0, 0]),
          // H-04: 父链折叠变换（与 image 对象同款 effTr 下发）
          ...(effTr ? { effTr: { origin: [effTr.origin[0], effTr.origin[1]], scale: [effTr.scale[0], effTr.scale[1]], angle: effTr.angle } } : {}),
          // 注：CPU renderParticleSystem 不读对象级 alpha/brightness（只吃
          // instanceoverride.alpha/rate），字段为对齐 image 清单而下发，客户端不应用。
          alpha: Number.isFinite(pAlpha) ? pAlpha : 1,
          brightness: Number.isFinite(pBrightness) ? pBrightness : 1,
          colorBlendMode: 0,
          alignment: '',
          // CPU: 仅 blending === 'additive' 走加色，其余一律 src-over
          blending: String(pPass && pPass.blending || 'translucent') === 'additive' ? 'additive' : 'normal',
          effects: [],
          particle: {
            maxcount: Number(pDef.maxcount) || 100,
            starttime: Number(pDef.starttime) || 0,
            emitter: resolveBound(Array.isArray(pDef.emitter) ? pDef.emitter : []),
            initializer: resolveBound(Array.isArray(pDef.initializer) ? pDef.initializer : []),
            operator: resolveBound(Array.isArray(pDef.operator) ? pDef.operator : []),
            renderer: Array.isArray(pDef.renderer) ? pDef.renderer : [],
            texture: pTex ? {
              path: pTex.path, width: pTex.width, height: pTex.height,
              headerWidth: pTex.headerWidth, headerHeight: pTex.headerHeight,
              // GIF/精灵表帧元数据（像素单位，TEXS）: >1 帧时客户端按帧子区域采样
              // （官方 SPRITESHEET 语义），否则整图当单帧会把整张帧网格画进每个粒子
              // （sf40g 泡泡方阵）。帧级宽高同步下发（纵横比按帧而非整图）。
              ...(pTex.frames ? { frames: pTex.frames } : {}),
            } : null,
            color1, color2,
            alphaMul: Number.isFinite(Number(glVal(pInst, 'alpha', 1))) ? Number(glVal(pInst, 'alpha', 1)) : 1,
            // sf41c: instanceoverride 乘子族 (官方 lwe 同款): count/rate=发射率,
            // lifetime=寿命, speed=重力/速度, size=尺寸。3735447194 泡泡带
            // {count:0.05, lifetime:2, speed:0.55, size:0.9} — 未接时密度 20 倍
            // 尺寸 2 倍 (官方 sizerandom 另有 ÷2) 且行程减半。
            rateMul: (() => {
              const c = Number(glVal(pInst, 'count', NaN));
              if (Number.isFinite(c) && c > 0) return c;
              const r = Number(glVal(pInst, 'rate', 1));
              return Number.isFinite(r) ? r : 1;
            })(),
            lifetimeMul: (() => { const v = Number(glVal(pInst, 'lifetime', 1)); return Number.isFinite(v) ? v : 1; })(),
            speedMul: (() => { const v = Number(glVal(pInst, 'speed', 1)); return Number.isFinite(v) ? v : 1; })(),
            sizeMul: (() => { const v = Number(glVal(pInst, 'size', 1)); return Number.isFinite(v) ? v : 1; })(),
            // sf43: colorn — 官方逐通道颜色乘子 (lwe: p.color = random×colorn;
            // 3593194513 樱花(0,1,1)/光束(0.51,0.51,0.97)/闪电/流星全带蓝调)
            colorMul: glVec(glVal(pInst, 'colorn', '1 1 1'), 3, [1, 1, 1]),
            mousefollow,
            // sf41a: 控制点表 — controlpointattract 消费 (flags&1 = 鼠标绑定 →
            // 客户端 pointerAttract 装 pointermove; 位置 = origin+offset)
            controlpoints: pCps.map((c) => ({
              id: Number(c.id) || 0,
              flags: Number(c.flags) || 0,
              offset: (() => { const v = glVec(glVal(c, 'offset', '0 0 0'), 3, [0, 0, 0]); return [v[0], v[1]]; })(),
            })),
          },
        });
        // sf47: eventfollow 子粒子系统（官方 children 语义 — 父粒子生成事件触发
        // 子系统瞬时爆发生成在父粒子位置; lwe 未实现 children, 本仓库按官方
        // 引擎行为实现）。子条目作为独立 particle 对象紧随父条目下发（painter
        // order: 子 glow 叠在父链上), followParent 标记客户端挂钩 spawn 事件。
        // 3427824116 Shooting_Star_01 → shootingstarglow (size 100-150, halo_4,
        // additive) = 流星可见主体; 父 ropetrail 是细核。
        for (const ch of (Array.isArray(pDef.children) ? pDef.children : [])) {
          // 实测 (3427824116): 子定义文件在 name 字段 (非 particle), type=eventfollow
          const chFile = ch && typeof ch.name === 'string' && ch.name.endsWith('.json') ? ch.name
            : (ch && typeof ch.particle === 'string' ? ch.particle : null);
          if (!ch || ch.type !== 'eventfollow' || !chFile) continue;
          const cDef = (() => { try { return access.readJson(chFile); } catch { return null; } })();
          if (!cDef || !cDef.material) continue;
          const cMat = access.readJson(String(cDef.material));
          const cPass = cMat && Array.isArray(cMat.passes) ? cMat.passes[0] : null;
          if (!cPass) continue;
          const cTexRef = Array.isArray(cPass.textures) ? cPass.textures[0] : null;
          const cTexRefName = typeof cTexRef === 'string' ? cTexRef : (cTexRef && typeof cTexRef.name === 'string' ? cTexRef.name : null);
          let cTex = null;
          if (cTexRefName && !cTexRefName.startsWith('_rt_')) {
            const ti = glTexInfo(access, cTexRefName);
            if (ti && !ti.video) cTex = ti;
          }
          out.push({
            name: name + ':child',
            type: 'particle',
            ...
            angles: glVec(glVal(obj, 'angles', '0 0 0'), 3, [0, 0, 0]),
            ...(effTr ? { effTr: { origin: [effTr.origin[0], effTr.origin[1]], scale: [effTr.scale[0], effTr.scale[1]], angle: effTr.angle } } : {}),
            alpha: 1, brightness: 1, colorBlendMode: 0, alignment: '',
            blending: String(cPass.blending || 'translucent') === 'additive' ? 'additive' : 'normal',
            effects: [],
            particle: {
              maxcount: Number(cDef.maxcount) || 4,
              starttime: 0,
              emitter: resolveBound(Array.isArray(cDef.emitter) ? cDef.emitter : []),
              initializer: resolveBound(Array.isArray(cDef.initializer) ? cDef.initializer : []),
              operator: resolveBound(Array.isArray(cDef.operator) ? cDef.operator : []),
              renderer: Array.isArray(cDef.renderer) ? cDef.renderer : [],
              texture: cTex ? {
                path: cTex.path, width: cTex.width, height: cTex.height,
                headerWidth: cTex.headerWidth, headerHeight: cTex.headerHeight,
                ...(cTex.frames ? { frames: cTex.frames } : {}),
              } : null,
              color1: [1, 1, 1], color2: [1, 1, 1],
              alphaMul: 1, rateMul: 1, lifetimeMul: 1, speedMul: 1, sizeMul: 1,
              colorMul: [1, 1, 1],
              mousefollow: false, controlpoints: [],
              // sf47: 父名挂钩 — 客户端把父粒子生成事件路由进本系统 pending
              followParent: name,
            },
          });
        }
      ...
      if (!obj.image) {
        if (obj.sound != null) mark(name, '声音对象', '声音对象未支持，已跳过该对象');
          ...
          // W5: 文字对象分流 — 即时(时钟/日期类, 脚本驱动)与水印文字与 CPU
          // text.js _isLiveText/_isWatermarkText 同款判定同款跳过; 静态文字
          // 栅格化为 PNG 按 image 对象走 GL。CPU text.js 不对文字应用效果链
          // (无 applyEffects 调用) → GL 同款: effects 不下发, 有效果时记标。
          const tt = obj.text;
          const isLive = tt && typeof tt === 'object' && typeof tt.script === 'string'
            && /new\s+Date\b|Date\.now|getHours\(|getMinutes\(|getSeconds\(|getFullYear\(|getMonth\(|getDate\(|getDay\(|engine\.frametime|performance\.now|Date\(\)/.test(tt.script);
          const vis = obj.visible;
          const isWatermark = tt && typeof tt === 'object' && typeof tt.script !== 'string'
            && vis && typeof vis === 'object' && typeof vis.user === 'string' && vis.user.length > 0;
          if (isLive) { mark(name, 'livetext', '即时文字（时钟/日期类）未支持，已跳过该对象'); continue; }
          if (isWatermark) { mark(name, 'watermarktext', '水印/声明文字未渲染，已跳过该对象'); continue; }
          const timg = glRenderTextBitmap(obj);
          if (!timg) { mark(name, 'text-render', '文字栅格化失败（字体缺失或空文本），已跳过该对象'); continue; }
          if (Array.isArray(obj.effects) && obj.effects.length) mark(name, 'text-effects', '文字对象效果链未支持，已按纯文本渲染');
          for (const key of SCENE_GL_ANIM_KEYS) {
            if (glHasAnim(obj[key])) { mark(name, 'anim:' + key, '属性动画(' + key + ')未支持，已按静态渲染'); break; }
          }
          let tMap = _glTextCache.get(access);
          if (!tMap) { tMap = new Map(); _glTextCache.set(access, tMap); }
          const myTextIdx = textIdx++;
          const png = Buffer.from(encodePng(timg.width, timg.height, timg.rgba));
          tMap.set(myTextIdx, { png, width: timg.width, height: timg.height });
          // alignment: CPU text.js — horizontalalign left/right/center ×
          // verticalalign top/bottom/center, 与 computeGeo includes() 语义一致
          const ha = String(glVal(obj, 'horizontalalign', 'center'));
          const va = String(glVal(obj, 'verticalalign', 'center'));
          const align = ((ha === 'left' || ha === 'right' ? ha : '') + ' ' + (va === 'top' || va === 'bottom' ? va : '')).trim();
          const tAlpha = Number(glVal(obj, 'alpha', 1));
          const tBright = Number(glVal(obj, 'brightness', 1));
          // 字段形态与 image 分支 out.push 同款 (客户端不区分 — 纯 image 路径)
          out.push({
            name,
            type: 'image',
            origin: glVec(glVal(obj, 'origin', '0 0 0'), 3, [0, 0, 0]),
            ...
            angles: glVec(glVal(obj, 'angles', '0 0 0'), 3, [0, 0, 0]),
            ...(effTr ? { effTr: { origin: [effTr.origin[0], effTr.origin[1]], scale: [effTr.scale[0], effTr.scale[1]], angle: effTr.angle } } : {}),
            alpha: Number.isFinite(tAlpha) ? tAlpha : 1,
            brightness: Number.isFinite(tBright) ? tBright : 1,
            colorBlendMode: 0,
            alignment: align,
            blending: 'normal',
            mainTexture: { path: '__text__/' + myTextIdx, width: timg.width, height: timg.height, headerWidth: timg.width, headerHeight: timg.height, video: false },
            effects: [],
          });
        }
      ...
      // W4: puppet 不再降级为静态贴图标记 — 网格渲染路径 (发射处建 payload)
      const isPuppet = !!modelJson.puppet;
      ...
      if (mainTex.video) {
        // W6/W7: 视频纹理对象 — 客户端 <video muted loop> 逐帧 texImage2D
        // (/scene-videotex 端点提取 mp4 + Range)。归 sceneGLExperimental 门控
        // (未充分验证特性默认不进 GL, 关闭时该对象跳过 → 场景可能 no-renderable
        // → scene-video mp4 回退, 行为 = W6 前)。本波不对视频对象跑效果链。
        if (!experimental) { mark(name, 'video-texture', '视频纹理属实验特性（可在设置中开启实验开关）'); continue; }
        const hasNonAudioFx = (Array.isArray(obj.effects) ? obj.effects : []).some((ef) => {
          if (valHidden(glVal(ef, 'visible', true))) return false;
          const d = ef && ef.file ? basename(dirname(String(ef.file))) : '';
          return !/audio|bars|oscilloscope|visualizer|equalizer|spectrum/i.test(d);
        });
        if (hasNonAudioFx) mark(name, 'video-effects', '视频对象的效果链未支持（已跳过效果，视频本体渲染）');
        for (const key of SCENE_GL_ANIM_KEYS) {
          if (glHasAnim(obj[key])) { mark(name, 'anim:' + key, '属性动画(' + key + ')未支持，已按静态渲染'); break; }
        }
        const vBlending = String(matPass.blending || 'normal');
        if (vBlending !== 'normal' && vBlending !== 'translucent') mark(name, 'blending:' + vBlending, '混合模式未支持，已按普通透明合成');
        out.push({
          name,
          type: 'video',
          origin: glVec(glVal(obj, 'origin', '0 0 0'), 3, [0, 0, 0]),
          ...
          angles: glVec(glVal(obj, 'angles', '0 0 0'), 3, [0, 0, 0]),
          ...(effTr ? { effTr: { origin: [effTr.origin[0], effTr.origin[1]], scale: [effTr.scale[0], effTr.scale[1]], angle: effTr.angle } } : {}),
          alpha: Number.isFinite(Number(glVal(obj, 'alpha', 1))) ? Number(glVal(obj, 'alpha', 1)) : 1,
          brightness: Number.isFinite(Number(glVal(obj, 'brightness', 1))) ? Number(glVal(obj, 'brightness', 1)) : 1,
          colorBlendMode: 0,
          alignment: String(glVal(obj, 'alignment', '') || ''),
          blending: vBlending === 'translucent' ? vBlending : 'normal',
          mainTexture: mainTex,
          effects: [],
        });
        continue;
      ...
      if (Number(glVal(obj, 'colorBlendMode', 0)) !== 0 && Number(glVal(obj, 'colorBlendMode', 0)) !== 9) mark(name, 'cbm', 'colorBlendMode 未支持，已按默认渲染');
      ...
      for (const ef of Array.isArray(obj.effects) ? obj.effects : []) {
        ...
        for (let pi = 0; pi < ePasses.length; pi++) {
          ...
          // 鼠标交互 shader 第三道: 引用鼠标位置/逆矩阵 uniform (lens_flare_sun 等
          // workshop 效果) — 客户端无指针 uniform 上传通道, 放行会退化渲染成固定
          // 位置色斑, 不如显式降级跳过 (配置页提醒)。
          const vertSrcChk = new TextDecoder('utf-8').decode(vertF.bytes);
          if (/g_PointerPosition|g_PointerLeft|g_PointerScreenPosition|g_ModelViewProjectionMatrixInverse/.test(fragSrc + vertSrcChk)) { skipFx('鼠标交互效果未支持'); fxBad = true; break; }
      ...
      // W4: puppet 对象建网格 payload (gate 期物化, /scene-puppet 按序下发);
      // 解析失败回退 image 路径 (静态贴图 — 与 W3 前行为一致) + degraded。
      // sf55: 传场景对象 — 未挂 animationlayers 的对象不物化动画帧 (无层 = 绑定
      // 姿态渲染; 详见 puppet-export.js hasVisibleAnimLayer)。
      let puppetRef = null;
      if (isPuppet) {
        const payload = buildPuppetPayload(access, modelJson, obj);
        if (payload) {
          let pMap = _glPuppetCache.get(access);
          if (!pMap) { pMap = new Map(); _glPuppetCache.set(access, pMap); }
          puppetRef = { objIdx: pMap.size, hasAnims: payload.animations.length > 0 };
          pMap.set(puppetRef.objIdx, payload);
        } else {
          mark(name, 'puppet', '木偶数据解析失败，按静态贴图渲染');
        }
      }
      out.push({
        name,
        type: puppetRef ? 'puppet' : 'image',
        origin: glVec(glVal(obj, 'origin', '0 0 0'), 3, [0, 0, 0]),
        size: glVec(glVal(obj, 'size', null), 2, null) || [mainTex.width, mainTex.height],
        scale: glVec(glVal(obj, 'scale', '1 1 1'), 3, [1, 1, 1]),
        angles: glVec(glVal(obj, 'angles', '0 0 0'), 3, [0, 0, 0]),
        ...
        // sf54: colorBlendMode 9 (BlendAdd) 下发 — GL present 按 SRC_ALPHA,ONE
        // 加性合成 (未夹取时与官方 mix(A,min(A+B,1),α) 逐位等价; 3784528825
        // ripple1440p 黑底水纹层黑盒根修)。其余非 0 模式仍按默认渲染 + 上方 mark。
        colorBlendMode: Number(glVal(obj, 'colorBlendMode', 0)) === 9 ? 9 : 0,
        alignment: String(glVal(obj, 'alignment', '') || ''),
        blending,
        mainTexture: mainTex,
        // sf51: spritesheet 材质 + 多帧纹理 → 客户端图像路径逐帧裁切轮播
        // (CPU image.js:140 同款 combos 大小写双查; frames 缺失/≤1 不加 — CPU
        // 此时也画整图, 行为对齐)。3735447194 Day/Night 按钮三帧表整图压进
        // 方形 quad 叠成一坨白色即此缺失。
        ...(Number((matPass.combos || {}).spritesheet ?? (matPass.combos || {}).SPRITESHEET ?? 0) === 1 && mainTex.frames
          ? { spritesheet: true } : {}),
        effects,
        // W4: 木偶网格元数据 (objIdx 对应 /scene-puppet 下发序; hasAnims 供
        // 客户端/实验层分流 — sf42 indexOf 修复后本地实测 20/20 带动画)
        // sf42: animationlayers 下发 (蒙皮层选择 — blend/rate/additive)
        ...(puppetRef ? {
          puppet: puppetRef,
          ...(Array.isArray(obj.animationlayers) && obj.animationlayers.length ? {
            animationlayers: obj.animationlayers.map((l) => ({
              animation: l.animation != null ? Number(l.animation) : 0,
              blend: Number.isFinite(Number(l.blend)) ? Number(l.blend) : 1,
              rate: Number.isFinite(Number(l.rate)) ? Number(l.rate) : 1,
              additive: l.additive === true,
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (!st || st.sceneGL === false) {
        res.statusCode = 403;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: 'scene-gl-disabled' }));
        return;
      }
      try {
        await ensureWeInstallLocatedP(); // Windows: <install>/assets 入全局素材根 (否则 GL 全局纹理全 miss)
        const access = glSceneAccess(abs, null);
        ...
        // A6: 外部 project.json 用户属性 (pkg 旁独立文件; pkg 内优先, 外部覆盖同键)
        const extUserProps = (() => {
          try {
            const extPath = join(dirname(abs), 'project.json');
            const ext = JSON.parse(readFileSync(extPath, 'utf8'));
            return ext && ext.general && ext.general.properties && typeof ext.general.properties === 'object'
              ? ext.general.properties : null;
          } catch { return null; }
        })();
        const check = sceneGLCheck(scene, access, { experimental: st.sceneGLExperimental === true, extUserProps });
  ...
  // 3g2. GET /scene-puppet/<token>/<objIdx> — W4 木偶网格 payload 下发
  //      (gate 期经 buildPuppetPayload 物化并缓存于 _glPuppetCache; 缓存 miss
  //      404 → 客户端回退静态贴图路径, 不在端点重解析 — gate 清单与下发序
  //      严格同源, 重解析会有遍历序漂移风险)
  disposers.push(webServer.register({
    kind: 'prefix',
    path: `${BASE}/scene-puppet`,
    handler: async (req, res) => {
      const method = (req.method || 'GET').toUpperCase();
      if (method !== 'GET') { res.statusCode = 405; res.end('method not allowed'); return; }
      const rest = decodeURIComponent(new URL(req.url || '/', 'http://x').pathname.slice(`${BASE}/scene-puppet/`.length));
      const token = rest.split('/')[0] ?? '';
      const abs = mediaMap.get(token);
      if (!abs) { res.statusCode = 404; res.setHeader('Content-Type', 'application/json; charset=utf-8'); res.end(JSON.stringify({ error: 'unknown-token' })); return; }
      const st = readSettings();
      ...
      const idxStr = rest.slice(token.length).replace(/^\/+/, '');
      const objIdx = Number(idxStr);
      if (!Number.isInteger(objIdx) || objIdx < 0) {
        res.statusCode = 404;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: 'bad-obj-idx' }));
        return;
      }
      try {
        const access = glSceneAccess(abs, null);
        const pMap = access ? _glPuppetCache.get(access) : null;
        const payload = pMap ? pMap.get(objIdx) : null;
        if (!payload) {
          res.statusCode = 404;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({ error: 'puppet-not-cached', objIdx }));
          return;
        }
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store');
  ...
  const _glslStubs = {
    ...
    // 重建版（按 CPU applyBlending 模式表逐支转写，官方头文件无公开副本）—
    // workshop shader 如 lens_flare_sun `#include "common_blending.h"` 的 ApplyBlending。
    'common_blending.h': readFileSync(join(_glslStubDir, 'common_blending.h'), 'utf8'),
  };
  disposers.push(webServer.register({
    kind: 'prefix',
  ...
  // 3e2. W6: GET /scene-videotex/<token>/<urlencoded-texPath> — 视频纹理 mp4
  //     提取 + 磁盘缓存 + Range 流式。客户端 <video muted loop> 逐帧 texImage2D
  //     (Range 是 <video> 循环播放的前提)。缓存/校验/清扫与 scene-video (sv2)
  //     同款语义, 键含 texPath (同壁纸多视频对象)。
  const SCENE_VIDEOTEX_INFLIGHT = new Map();
  disposers.push(webServer.register({
    kind: 'prefix',
    path: `${BASE}/scene-videotex`,
    handler: (req, res) => {
      const method = (req.method || 'GET').toUpperCase();
      if (method !== 'GET' && method !== 'HEAD') {
        res.statusCode = 405;
      ...
      const pathname = new URL(req.url || '/', 'http://x').pathname;
      const rest = decodeURIComponent(pathname.slice(`${BASE}/scene-videotex/`.length));
      const token = rest.split('/')[0] ?? '';
      const texPath = rest.slice(token.length).replace(/^\/+/, '');
      const abs = mediaMap.get(token);
      if (!abs || !texPath) {
        res.statusCode = 404;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: abs ? 'tex-path-missing' : 'unknown-token' }));
        return;
      }
      if (texPath.includes('..') || texPath.startsWith('/')) {
        res.statusCode = 404;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: 'tex-path-invalid' }));
        return;
      }
      (async () => {
        // vt1 = W6 管线首版 (extractTexVideoMp4 + 顶层 box 截尾 + ftyp/moov 校验)
        const key = 'vt1_' + createHash('sha256')
          .update(abs + '|' + texPath + '|' + sceneCacheStamp(abs))
          .digest('hex').slice(0, 40);
        const mp4Path = join(ensureFrameCacheDir(), key + '.mp4');
        if (!existsSync(mp4Path)) {
          let inflight = SCENE_VIDEOTEX_INFLIGHT.get(key);
          if (!inflight) {
            inflight = (async () => {
              const access = glSceneAccess(abs, null);
              const file = access && (access.readFile(texPath) || readGlobalAsset(texPath));
              if (!file || !file.bytes) return null;
              const raw = extractTexVideoMp4(file.bytes);
              if (!raw || raw.length === 0) return null;
              const mp4 = trimMp4ByTopBoxes(Buffer.from(raw));
              if (!mp4LooksValid(mp4)) return null;
              await atomicWriteFileP(mp4Path, mp4);
              gcFrameCacheSync(dirname(mp4Path), mp4Path);
              return mp4Path;
            })();
            SCENE_VIDEOTEX_INFLIGHT.set(key, inflight);
            inflight.then(
              () => SCENE_VIDEOTEX_INFLIGHT.delete(key),
              () => SCENE_VIDEOTEX_INFLIGHT.delete(key),
            );
          }
          const produced = await inflight;
          if (!produced) {
            res.statusCode = 422;
            res.setHeader('Content-Type', 'application/json; charset=utf-8');
            res.end(JSON.stringify({ error: 'tex-not-video', tex: texPath }));
            return;
          }
        }
        serveFile(mp4Path, req, res, method === 'HEAD');
