/**
 * dsh-wallpaper-engine 鈥?host half.
 *
 ...
 *        - type "video"  鈫?the project's `.mp4` (or other media) file
 *        - type "web"    鈫?the project's HTML entry
 *      Scene (native 3D) and Application wallpapers are listed too, but only
 *      their preview image is served (they cannot be rendered here 鈥?see README).
 ...
 *        GET /wallpaper-engine/inventory          鈫?{ installDir, wallpapers:[鈥, playlists:[鈥 }
 *        GET /wallpaper-engine/media/<token>      鈫?video / html (Range supported)
 *        GET /wallpaper-engine/preview/<token>    鈫?preview image
 *
...
// Async filesystem (thread pool) for the wallpaper-scan chain 鈥?keeps the
// event loop responsive on slow media (WSL DrvFS) instead of blocking it for
// seconds per chunk (see "Loading plugins鈥? stall report).
...
/** Async existence probes (fs.promises 鈥?thread pool, no event-loop blocking). */
 ...
 * (paths are DrvFS mounts 鈥?slow, which is exactly why only async probes run).
 */
 ...
 * even when read from inside WSL 鈥?without this the workshop library would
...
async function owningLibrariesP() {
  ...
  for (const probe of await steamProbeDirsP()) {
    ...
    // root 鈥?include it, or every workshop wallpaper silently disappears from
 ...
 * ship the reverse) 鈥?probe the declared file, then scene.pkg, then
...
async function enumerateWallpapersAsync(installDir, libraryDirs) {
  ...
  // bounded chunks 鈥?the heavy per-project I/O (readdir/stat/readFile) runs on
...
// 鈹€鈹€ Custom uploads (read-A storage: files live on disk in a plugin-managed
//    directory, served through the SAME token/media/preview routes as the
//    Wallpaper Engine media 鈥?no IndexedDB, no quota limits, survives
//    restarts by construction). 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
/**
 * Atomic whole-file write: temp file + fsync + rename. Crash/鏂數 mid-write
 ...
 * persisted in the SAME config.json as uploadDir 鈥?host-side, port-independent.
...
// 鈹€鈹€ Server-side settings validation (mirror of the client's readPersisted
//    whitelist in src/client.js; keep the two in sync) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
/** 瑙ｇ爜甯х巼涓婇檺 options (fps); 0 = 鏃犻檺鍒? Mirror of the client's list. */
...
// 鍦?worker 绾跨▼娓叉煋鍦烘櫙甯? 杩斿洖 { ok, png(Buffer), diff, checked } 鎴?{ ok:false, error }
  ...
  return new Promise((resolve) => {
    ...
    try {
      worker = new Worker(new URL('./scene-render-worker.mjs', import.meta.url), {
        workerData: { src, width, height, time, times, frameDelayMs, weAssetsDir },
        type: 'module',
      });
    } catch (e) {
    ...
    // 澶у瀷鍦烘櫙 CPU 鍏夋爡鍖栧彲鑳藉緢鎱? 缁欒冻瓒呮椂 (澶氬抚 脳 姣忓抚)
...
// dsh-better-sidebar 瀹夎妫€娴嬶細閬嶅巻 cordis loader 鐨勬潯鐩爲锛坈tx.loader 鏄牴
// EntryTree锛宔ntries() 瑕嗙洊鎵€鏈夊祵濂楀瓙鏍戯級锛屾壘 dsh-better-sidebar 涓旀湭绂佺敤鐨?
// 鏉＄洰銆傜敤瀹冨喅瀹氭祻瑙堝櫒绔殑銆屼晶鏍忕幓鐠冦€嶆帶鍒剁粍鏄惁鏄剧ず 鈥斺€?涓嶄緷璧栦晶鏍?DOM 鏄惁
// 宸叉寕杞斤紙渚ф爮鎳掑姞杞斤紝DOM 鎺㈡祴浼氭紡鍒わ級锛屼篃涓嶄緷璧栧叾鏈嶅姟 API锛堢増鏈棿涓嶇ǔ瀹氾級銆?
// 娉ㄦ剰 Entry 鏈韩娌℃湁 name getter锛氬寘鍚嶅湪 entry.options.name锛坧atch 琛岀殑 name
// 瀛楁锛屽嵆 import 璇存槑绗︼級锛涜仛鍚堝寘鎸傝浇鏃舵潯鐩?id 鍙兘鏄?web-ui-better-sidebar
// 涔嬬被锛屾晠 id 鍚?better-sidebar 涔熻涓哄懡涓€俵oader 鏈嶅姟闅?dsh-base 鎻愪緵锛?
// 璇讳笉鍒版椂鎸夈€屾湭瀹夎銆嶅鐞嗐€?
function isBetterSidebarLoaded(ctx) {
  try {
    ...
    for (const entry of loader.entries()) {
      ...
      if (!opts || opts.group) continue; // group 鑺傜偣璺宠繃
...
function sanitizeSettings(raw) {
  ...
  const rotationGroups = Array.isArray(o.rotationGroups)
    ? o.rotationGroups
        ...
        .map((g) => ({
          id: g.id,
          name: typeof g.name === 'string' && g.name.trim() ? g.name.trim() : '杞挱鍒楄〃',
...
// 鈹€鈹€ Media metadata probe (minimal MP4 box walker) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// Serves two purposes: the picker hint ("婧?4K 路 120fps 路 H.264") and the
// 甯х巼涓婇檺 decision 鈥?a source at/below the cap skips the transcode entirely.
...
function probeMp4(abs) {
  ...
  try {
    ...
    // must END at (or just before) EOF 鈥?filters out random 'moov' runs in mdat.
...
// 鈹€鈹€ Frame-skip transcode (鎶藉抚杞爜, ffmpeg) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// (4K120 鈫?4K60: ffmpeg drops every other frame, timeline stays 1.0x, reference
// chains are re-encoded intact) and the browser plays a normal capped-fps file.
// Output is AV1 via NVENC (decode throughput 鈮?2脳 H.264 on NVDEC, so decode
// util roughly halves again), falling back to H.264 when AV1 encode is missing.
// ffmpeg resolution: DSH_WE_FFMPEG env 鈫?a local ./ffmpeg/ffmpeg(.exe) next to
// the bundle 鈫?system PATH. Missing ffmpeg 鈬?the transcode route errors and the
...
// a hung encode can never leave /transcoded waiting forever 鈥?the child is
...
// 鈹€鈹€ Lazy ffmpeg provisioning (B + C + D) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
// Resolution chain (each level falls through to the next):
//   B. system PATH (bare name)                       鈫?last resort
...
//   env DSH_WE_FFMPEG  /  plugin-local ./ffmpeg/     鈫?explicit overrides
// Downloaded binaries are pinned by sha256 (FFMPEG_STATIC_SHA256, computed from
// the b6.0 release bytes) 鈥?a mismatch aborts before anything is executed; the
// magic-byte + size checks remain as a second layer.
const FFMPEG_STATIC_TAG = 'b6.0';
// process.platform 鈫?process.arch 鈫?release asset name (ffmpeg-static naming).
...
function sweepTranscodeArtifacts() {
  ...
  // child keeps writing to `.tmp<pid>` 鈥?removing it would corrupt the job).
...
//   phase 'download'  鈥?bytes/total (content-length when the mirror sends it)
//   phase 'transcode' 鈥?output-file growth (see runFfmpegTranscode)
//   phase 'done'      鈥?cached file is ready to serve
//   phase 'error'     鈥?the job failed (client falls back to the original)
...
// Download sources, raced in parallel (first success wins 鈥?the fastest mirror
// for THIS user wins automatically, no region pre-sorting):
//   npmmirror  鈥?fast for CN users (validated ~2 min for the 70MB binary)
//   GitHub     鈥?fast for everyone else
        ...
        downloadFfmpegToFile(url, tmpFiles[i], ctrls[i], job)
          .then((r) => {
            ...
            if (want && r.sha256 !== want) {
              errors.push(url + ' 鈫?sha256 mismatch');
          ...
          .catch((err) => {
            if (done) return;
            errors.push(url + ' 鈫?' + String(err && err.message ? err.message : err));
            remaining--; if (remaining === 0) { done = true; resolve(-1); }
          });
      });
    });
...
// We therefore: (1) never use pipes 鈥?stderr is redirected to a temp FILE so
...
async function runFfmpegTranscode(abs, out, fps) {
  ...
  // a fixed bitrate (size 鈭?time) and derive percent/ETA from the OUTPUT FILE
...
/**
 * Upload directory, resolved in order: env override 鈫?persisted user config 鈫?
...
/** Accepted upload MIME 鈫?file extension (matches mimeFor above). */
...
/** Scan the uploads dir 鈫?WE-shaped wallpaper entries (no project.json). */
 ...
 *  (EXDEV on Windows: C: 鈫?D: is the exact case users hit when relocating
...
export function apply(ctx) {
  ...
  // Token 鈫?absolute path map. Tokens are base64url of the abs path, so the
  ...
  // Build the inventory (async scan chain 鈥?fs.promises, event-loop friendly).
  ...
  async function buildInventory() {
    ...
    // the client-side "鏃犻瑙? placeholder.
  ...
  // 鈥?without this the source fd stays open until process exit, leaking one
  ...
  // Media metadata (source resolution / codec / fps) 鈥?the picker hint and the
  // 甯х巼涓婇檺 skip-decision. Registered BEFORE the /media loop ("/media-info"
    ...
    handler: (req, res) => {
      ...
      if (p) {
        ...
        if (phase === 'download') {
          ...
          // Rolling size samples 鈫?growth rate 鈫?ETA (wall seconds remaining).
          const now = Date.now();
  ...
  // Frame-skip transcode (鎶藉抚杞爜): serves a capped-fps re-encode (see
  ...
  // first paint is instant. Missing ffmpeg / failed encode 鈬?502, and the
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // the moov probe (media-info) stays MP4-only 鈥?other containers simply
  ...
  // Scenes are read in-place from the user's own library 鈥?nothing is copied,
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf12 = 5 缁勪欢淇 (楠ㄩ澶村彉浣?钂欑毊鍏煎/椤剁偣
        // sanity/systemfont/filmgrain _sat3) + MDLE 娈佃В鏋?
        const key = 'sf13_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
        ...
        if (!servePath) {
          ...
          // 瀹屾暣鍦烘櫙娓叉煋 (SceneRenderer) 浼樺厛: 鍦?worker 绾跨▼杈撳嚭 3840脳2160 鍏ㄥ満鏅抚
          // (鑳屾櫙+妯″瀷+绮掑瓙+shader鏁堟灉), 涓嶉樆濉炰富杩涚▼; 澶辫触鏃跺洖閫€鏃х殑涓荤汗鐞嗘彁鍙?
          let rendered = false;
          try {
            // 鏉炬暎 scene.json 椤圭洰浼犵洰褰? scene.pkg 浼犳枃浠?
            const src = abs.toLowerCase().endsWith('.json') ? dirname(abs) : abs;
            const result = await renderSceneFrameInWorker(src, 3840, 2160, 2.5);
            if (!result.ok) throw new Error(result.error);
            // 绌哄抚闂ㄧ: 娓叉煋鍣?鎴愬姛"杈撳嚭绌虹櫧/绾壊甯ф椂瑙嗕负澶辫触, 璧板洖閫€閾?
            if (result.diff < result.checked * 0.0005) throw new Error('blank frame (renderer)');
            writeFileSync(pngPath, result.png);
            servePath = pngPath;
            rendered = true;
          } catch (e) {
            // 娓叉煋澶辫触(闈?scene.pkg 缁撴瀯/缂鸿祫婧愮瓑) 鈫?鍥為€€涓荤汗鐞嗛潤鎬佸抚
  ...
  // 3b. Scene 鍔ㄧ敾甯?(APNG): /scene-anim/<token>?fps=..&sec=.. 鈥?澶氬抚娓叉煋鎴?
  //     鍔ㄧ敾 (绮掑瓙/鐩告満璺緞/鏁堟灉鍔ㄧ敾鍙瘎鍒?銆傞潤鎬佸抚缂撳瓨涓嶉€傜敤, 姣忔娓叉煋銆?
  disposers.push(webServer.register({
    kind: 'prefix',
    ...
    handler: (req, res) => {
      ...
      // 鍙傛暟: fps (榛樿 12), sec (榛樿 2 绉?, 鍒嗚鲸鐜囦笂闄?(CPU 娓叉煋鏄傝吹, 榛樿 960脳540)
      const fps = clampNum(Number(url.searchParams.get('fps')) || 0, 2, 30, 12);
      const sec = clampNum(Number(url.searchParams.get('sec')) || 0, 0.5, 6, 2);
      const w = clampNum(Number(url.searchParams.get('w')) || 0, 320, 1280, 960);
      const h = clampNum(Number(url.searchParams.get('h')) || 0, 180, 720, 540);
      const frameCount = Math.max(2, Math.round(fps * sec));
      // 鏃堕棿閲囨牱: 瑕嗙洊鐩告満璺緞鍛ㄦ湡 (鑻ュ瓨鍦? 鎴栫矑瀛?starttime 鍚?/ 鍧囧寑鍒嗗竷
      const src = abs.toLowerCase().endsWith('.json') ? dirname(abs) : abs;
      (async () => {
        ...
        try {
          ...
          // 鐩告満璺緞鎬诲懆鏈? 鍚?path duration 涔嬪拰
          ...
          // 绮掑瓙 starttime 鏈€澶у€?(绮掑瓙绯荤粺寤惰繜鍚姩)
        ...
        // 閲囨牱璧风偣: 绮掑瓙 starttime 鍚?(纭繚鍔ㄧ敾鍙), 瑕嗙洊鐩告満鍛ㄦ湡鎴?2 绉?
        const t0 = starttime > 0 ? starttime : 0;
        const loop = Math.max(period, 2);
        const times = [];
        for (let i = 0; i < frameCount; i++) times.push(t0 + (i / frameCount) * loop);
        const result = await renderSceneFrameInWorker(src, w, h, 0, { times, frameDelayMs: Math.round(1000 / fps) });
        if (!result.ok) throw new Error(result.error);
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      if (!ext) {
        ...
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({
          error: '涓嶆敮鎸佺殑鏍煎紡锛? + ctype + '锛堜粎鏀寔 JPG / PNG / MP4锛?,
        }));
        return;
      }
      const chunks = [];
      ...
      req.on('data', (c) => {
        ...
        if (size > UPLOAD_MAX_BYTES) {
          ...
          res.statusCode = 413;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({ error: '鏂囦欢杩囧ぇ锛堜笂闄?512MB锛? }));
          req.destroy();
          return;
        }
        chunks.push(c);
      ...
      req.on('end', () => {
        ...
        try {
          ...
          // duplicate entry 鈥?return the existing wallpaper instead. meta
          ...
          if (dupId) {
            ...
            // meta says the id exists but the file is gone 鈥?fall through and
  ...
  // 5. Custom upload removal 鈥?ONLY files inside the uploads dir with an
  //    `up-鈥 id (path traversal is impossible: the id is host-generated and
  ...
  //    UI 鈥?most users prefer their data off the system (C:) drive.
  disposers.push(webServer.register({
    kind: 'exact',
    ...
    handler: (req, res) => {
      ...
      req.on('end', () => {
        ...
        if (!dir) {
          ...
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({
            error: '璇疯緭鍏ユ湁鏁堢殑缁濆璺緞锛堝 D:\\MyWallpapers 鎴?/data/wallpapers锛?,
          }));
          return;
        }
        ...
        if (!isDir) {
          res.statusCode = 400;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({ error: '鏃犳硶鍦ㄨ璺緞鍒涘缓鐩綍锛堟潈闄愪笉瓒虫垨璺緞琚崰鐢級' }));
          return;
        }
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      if (method === 'GET') {
        // betterSidebar: 渚ф爮鐜荤拑鎺у埗缁勬槸鍚︽樉绀猴紙dsh-better-sidebar 宸插畨瑁呬笖
        // 鍚敤锛夈€傛寕鍦?settings 鍝嶅簲涓婏紝瀹㈡埛绔?loadPersisted 鏃朵竴娆″彇鍥炪€?
  ...
  return () => {
    ...
    // releases all file descriptors 鈥?zero residue.
