 ...
 *  官方安装: C:\Program Files (x86)\Steam\steamapps\common\wallpaper_engine\
 *    wallpaper64.exe = D3D11 渲染宿主 (单实例启动器, 支持 -control 命令:
 *    openWallpaper -file <scene.pkg> / workshopid -id <id> / pause|play|mute|unmute)
 *    bin/wallpaperservice64.exe = 渲染服务, bin/wallpaperui.exe = UI
 *  Workshop 壁纸: <steam>\steamapps\workshop\content\431960\<workshopid>\scene.pkg
 *  (431960 = WE 的 Steam appid)。weAssetsDir (官方 assets/shaders 等) 供 GLSL
 *  解释器的 common.h/common_blending.h 等官方头文件编译使用。 */
...
// 静态帧渲染时间: 有 single (入场/退场) 属性动画的场景渲染到动画结束后的静止态。
// 官方 mode:"single" = 播完保持终态。
// 旧实现硬编码 t=2.5 — 入场动画中段时整场景被放大/裁切 (3554161528 相机对象
// #3727 zoom 3→1 @18fps 关键帧 0/90/118, 静止态需 t≥5s; t=2.5 时 camZoom≈2.07
// → "静态帧像裁切放大了原版")。纯循环/无动画场景保持 2.5 (循环内任意时刻等价)。
function sceneStaticFrameTime(abs) {
  try {
    const src = abs.toLowerCase().endsWith('.json') ? dirname(abs) : abs;
    const sc = readSceneJsonFast(src);
    if (!sc || !Array.isArray(sc.objects)) return 2.5;
    const ANIM_KEYS = ['alpha', 'scale', 'origin', 'angles', 'visible', 'color', 'size', 'brightness', 'parallaxDepth', 'zoom'];
    let animEnd = 0;
    let hasSingleAnim = false;
    for (const o of sc.objects) {
      for (const key of ANIM_KEYS) {
        const v = o && o[key];
        if (!v || typeof v !== 'object' || !v.animation || !v.animation.options) continue;
        const a = v.animation;
        const fps = a.options.fps || 30;
        // 该动画最末关键帧时间 (单通道 c0 或多通道 c0/c1/c2 取最大帧号)
        let lastFrame = 0;
        for (const ch of ['c0', 'c1', 'c2']) {
          const fr = (a[ch] || []).filter((f) => f && typeof f.frame === 'number');
          for (const f of fr) if (f.frame > lastFrame) lastFrame = f.frame;
        }
        if (lastFrame > 0) animEnd = Math.max(animEnd, lastFrame / fps);
        const amode = String(a.options.mode || '');
        if (amode !== 'loop' && amode !== 'wraploop' && amode !== 'mirror') hasSingleAnim = true;
      }
    }
    // 相机路径 (motion-path camera): 官方按路径顺序循环 (camera.js _resolveCameraPose),
    // 总周期 = 各 path 时长之和; 路径结束后相机 clamp 停在末关键帧位姿。
    // 有相机路径的场景, 静态帧应取"路径播完后的稳定位姿" (否则 t=2.5 可能命中
    // 运镜中段) → 纳入 animEnd 并视为 single (2026-08-30 完善)。
    ...
    if (paths.length) {
      ...
      const collect = (list) => {
        for (const p of list) {
          if (!p || typeof p !== 'object' || !Array.isArray(p.transforms)) continue;
          const trs = p.transforms.filter((x) => x && typeof x.timestamp === 'number');
          const lastT = trs.length ? Math.max(...trs.map((x) => x.timestamp)) : 0;
          camTotal += Math.max(lastT, p.duration != null ? Number(p.duration) : lastT);
        }
      };
      for (const p of paths) {
        if (typeof p === 'string') {
          try {
            ...
            if (j && Array.isArray(j.paths)) collect(j.paths);
          } catch { /* 忽略单条路径解析失败 */ }
      ...
      if (camTotal > 0) {
        animEnd = Math.max(animEnd, camTotal);
        hasSingleAnim = true;
      }
    }
    if (!hasSingleAnim) return 2.5;
    // 静止态 = 最末关键帧之后 (加 1 帧余量防浮点边界), 上限 60s
    return Math.min(Math.max(animEnd + 1 / 30, 0.1), 60);
  } catch { return 2.5; }
}
...
// 渲染 workerData 指定的单帧, 结果经大文件回传协议 (worker 写临时 PNG,
// 宿主异步读回 + ack 握手) resolve。
function spawnRenderWorker(workerData, signal, gpuNode) {
  return new Promise((resolve) => {
    ...
    const finish = (v) => {
      ...
      gpuDiag('finish:', v.ok ? 'OK' : 'ERR', v.error || '', 'pngBytes=', v.png ? v.png.length : 0);
      resolve(v);
    };
    const onAbort = () => {
      try { if (worker) worker.terminate(); } catch { /* ignore */ }
    ...
    const onMessage = (msg) => {
      if (msg && msg.ok) {
        // 单帧 PNG: 不走 IPC — worker 写临时文件, 这里异步读回。
...
function sanitizeSettings(raw) {
  ...
  return {
    ...
    // GPU 渲染加速 (静态帧): 测试中功能; 输出与 CPU 有微小 float 精度差,
    // 缓存键含 gpu 标志隔离, 切换不得命中另一管线旧帧
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // 缓存键含渲染尺寸: 非 16:9 场景按 ortho 宽高比修正 fw/fh — 若旧缓存
        // 帧是修正前尺寸渲染的, 同键继续服务会垂直裁切/拉伸 (根因 E1)。
        let gpuFlag = '0';
        ...
        const key = 'sf34_' + gpuFlag + '_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + fw + 'x' + fh + '_' + Math.round(mtime);
          ...
          if (!inflight) {
            inflight = (async () => {
              ...
              try {
                ...
                // 静态帧时间: 有 single 入场动画的场景渲染到静止态 (旧硬编码 2.5
                // 落在入场中段 → 3554161528 相机 zoom≈2.07 整场景放大裁切)
                const frameTime = sceneStaticFrameTime(abs);
                const result = await renderSceneFrameInWorker(src, fw, fh, frameTime, { gpuAccel });
