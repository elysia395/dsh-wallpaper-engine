...
// sha256 pinning is supported per asset but left null until a release pins
// them; the magic-byte + size checks are the runtime guard meanwhile.
...
// Placeholder for pinned hashes (fill before a release, keep null to skip).
...
//   phase 'transcode' — ffmpeg's -progress file (out_time_ms vs source duration)
//   phase 'done'      — cached file is ready to serve
let transcodeProgress = null; // { key, phase, downloaded, total, source, outFile, expectedBytes, samples }
...
async function downloadFfmpegToFile(url, tmp, ctrl) {
  ...
  if (transcodeProgress && transcodeProgress.phase === 'download') {
    transcodeProgress.total = totalBytes;
    transcodeProgress.source = url;
  }
  ...
  try {
    for (;;) {
      ...
      if (value && value.length) {
        ...
        total += value.length;
        if (transcodeProgress && transcodeProgress.phase === 'download') {
          transcodeProgress.downloaded = total;
        }
      }
    }
    fsyncSync(fd);
...
async function ensureDownloadedFfmpeg() {
  ...
  ffmpegDownloadPromise = (async () => {
    ...
    const win = await new Promise((resolve) => {
      ...
      urls.forEach((url, i) => {
        downloadFfmpegToFile(url, tmpFiles[i], ctrls[i])
          .then((r) => {
            if (done) return;
...
async function resolveFfmpeg() {
  ...
  const dl = await ensureDownloadedFfmpeg();
  if (dl) return dl;
...
function spawnFfmpeg(ff, args) {
  return new Promise((resolve, reject) => {
    ...
    const runNext = () => {
      ...
      proc.on('error', (err) => {
        if (errFd) { try { closeSync(errFd); } catch { /* ignore */ } }
        errors.push(a.name + ' spawn error ' + (err && err.code ? err.code + ' ' + err.message : err));
        runNext();
      });
      proc.on('exit', (code) => {
        if (errFd) { try { closeSync(errFd); } catch { /* ignore */ } }
...
async function runFfmpegTranscode(abs, out, fps) {
  ...
  transcodeProgress = { key, phase: 'download', downloaded: 0, total: 0, source: '' };
  ...
  transcodeProgress = {
    key, phase: 'transcode', downloaded: 0, total: 0, source: ff,
    outFile: out,
    expectedBytes: mi && mi.duration ? Math.round((bitrate / 8) * mi.duration) : null,
    samples: [], // [{t, size}] rolling samples for growth-rate / ETA
  };
...
function transcodeToFps(abs, fps) {
  ...
  const p = (async () => {
    ...
    try {
      ...
      if (transcodeProgress && transcodeProgress.key === progKey) transcodeProgress.phase = 'done';
      return cachePath;
    } catch (err) {
      try { unlinkSync(tmp); } catch { /* ignore */ }
      if (transcodeProgress && transcodeProgress.key === progKey) transcodeProgress.phase = 'error';
...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      const p = transcodeProgress;
      if (abs && p && p.key === abs + '|' + fps) {
        phase = p.phase;
        source = p.source || '';
        if (phase === 'download') {
