...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf24 = 根 puppet 底部对齐 (wallpaper64 逆向):
        // 有子对象且子对象全为空锚点的根 puppet (如 Amiya 头697) origin 对齐
        // 网格底部 (shift=-minY+1), 子对象经父链跟随 → 头部链整体上移,
        // 身体链不动. 修复 Amiya 头被身体盖 (重叠 291px→0), 组件相对保持.
        const key = 'sf24_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
        ...
        if (!servePath) {
          ...
          // 完整场景渲染 (SceneRenderer) 优先: 在 worker 线程输出 3840×2160 全场景帧
          // (背景+模型+粒子+shader效果), 不阻塞主进程; 失败时回退旧的主纹理提取.
          let rendered = false;
          try {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 参数: fps (默认 12), sec (默认 2 秒), 分辨率上限 (CPU 渲染昂贵, 默认 960×540)
      ...
      // 时间采样: 覆盖相机路径周期 (若存在) 或粒子 starttime 后 / 均匀分布
      const src = abs.toLowerCase().endsWith('.json') ? dirname(abs) : abs;
      (async () => {
        ...
        // 采样起点: 粒子 starttime 后 (确保动画可见), 覆盖相机周期或 2 秒
