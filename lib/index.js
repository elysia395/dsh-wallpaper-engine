...
async function steamProbeDirsP() {
  ...
  steamProbeInflight = (async () => {
    ...
    const mac = STEAM_MAC_DIR && existsSync(STEAM_MAC_DIR) ? [STEAM_MAC_DIR] : [];
    return [...(reg ? [reg] : []), ...env, ...STEAM_PROBE_DIRS, ...wsl, ...mac];
  })();
  try {
...
async function locateWallpaperEngineP() {
  ...
  for (const raw of candidates) {
    ...
    if (await pathExistsP(marker)) return dir;
  }
  return null;
}
...
async function enumerateWallpapersAsync(installDir, libraryDirs, extraRoots = []) {
  const found = new Map();
  const roots = [];
  if (installDir) {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      if (await isDirectoryP(dir)) {
        ...
        if (await pathExistsP(nested)) roots.push(nested);
      } else if (LOOSE_MEDIA.test(entry)) {
        ...
        // pathExistsP path as project media.
...
function sanitizeSettings(raw) {
  ...
  return {
    ...
    hiddenIds: Array.isArray(o.hiddenIds)
      ...
      : [],
    playbackRate: clampNum(o.playbackRate, 0.25, 2, 1),
...
function spawnFfmpeg(ff, args, timeoutMs) {
...
async function runFfmpegTranscode(abs, out, fps) {
  ...
  for (const enc of ['av1_nvenc', 'h264_nvenc']) {
    ...
    try {
      ...
      await spawnFfmpeg(ff, [...base, '-c:v', enc, '-f', 'mp4', out], remaining);
      return;
    } catch (err) {
...
export function apply(ctx) {
  ...
  async function buildInventory() {
    ...
    // macOS content roots (WaifuX dirs / DSH_WALLPAPER_ENGINE_CONTENT) feed the
    // same async scan; bare loose media are their own wallpapers.
    const all = await enumerateWallpapersAsync(installDir, libraryDirs, contentRootsFromEnv());
    const byPath = new Map(all.map((w) => [pathKey(w.fileAbs), w.id]));
    const byId = new Map(all.map((w) => [w.id, w]));
    const wallpapers = await Promise.all(all.map(async (w) => {
      // 三次存在性探测并发（原本串行，慢盘上是 3× 延迟）。macOS loose media
      // (image/video) carry no sidecar preview, so image stays a media source.
      const [hasMedia, hasPreview, hasFrame] = await Promise.all([
        w.type === 'video' || w.type === 'web' || w.type === 'image' ? pathExistsP(w.fileAbs) : Promise.resolve(false),
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf3: multi-layer scene compositing (the
        // single-texture path showed only one panel of multi-panel scenes).
        // sf4: composite canvas fills the scene clear color (clearcolor lives
        // in scene.json; schemecolor is in project.json which pkgs lack) —
        // sf3 composites had transparent gaps that rendered black in the GUI.
        const key = 'sf4_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
  ...
  // 3b. Live scene WebGL player HTML. The <iframe> loads <token> as the last
  ...
  // 3c. Scene manifest JSON — the WebGL player fetches this to know the scene
