...
async function renderSceneFrameInWorker(src, width, height, time) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      worker = new Worker(new URL('./scene-render-worker.mjs', import.meta.url), {
        workerData: { src, width, height, time, weAssetsDir },
        type: 'module',
      });
    } catch (e) {
    ...
    worker.once('message', (msg) => {
      if (msg && msg.ok) finish({ ok: true, png: Buffer.from(msg.png), diff: msg.diff, checked: msg.checked });
      else finish({ ok: false, error: (msg && msg.error) || 'scene render failed' });
    });
