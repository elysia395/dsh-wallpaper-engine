...
// 帧缓存磁盘 LRU (P-12 顺手): sf* 静态帧缓存文件超过 32 个时清最旧 — 每次壁纸
// 更新 mtime 变化都会留一个新键, 旧文件永不清理只增不减。scene-anim 的 san_*
// 与视频抽帧 vid_* 键含参数/时间空间, 不纳入本 GC。
function gcFrameCacheSync(dir) {
  let names = [];
  try { names = readdirSync(dir).filter((n) => /^sf/.test(n)); } catch { return; }
  if (names.length <= 32) return;
  const statted = [];
  for (const n of names) {
    try { statted.push({ n, m: statSync(join(dir, n)).mtimeMs }); } catch { /* ignore */ }
  }
  statted.sort((a, b) => a.m - b.m); // 最旧在前
  for (let i = 0; i < statted.length - 32; i++) {
    try { unlinkSync(join(dir, statted[i].n)); } catch { /* 在用/权限, 下轮再清 */ }
  }
}
...
function sceneAnimCachePath(abs, fps, sec, w, h, ext) {
  ...
  const key = 'san_sf35_' + createHash('sha256')
    .update(abs + '|' + stamp + '|' + fps + '|' + sec + '|' + w + '|' + h + '|' + ext)
...
async function renderSceneFrameInWorker(src, width, height, time, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    timer = setTimeout(() => finish({ ok: false, error: 'scene render timeout' }), 600000 * framesN);
    ...
    worker.on('message', (msg) => {
      ...
      if (msg && msg.ok) {
        ...
        if (msg.apng) finish({ ok: true, apng: Buffer.from(msg.apng), degraded });
...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        const key = 'sf35_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
          ...
          if (!inflight) {
            inflight = (async () => {
              ...
              try {
                ...
                gcFrameCacheSync(dir); // P-12: sf* 帧缓存 LRU (>32 清最旧)
                return pngPath;
              } catch (e) {
  ...
  const SCENE_GL_EFFECT_WHITELIST = new Set(['waterripple', 'iris', 'waterwaves', 'foliagesway', 'shake']);
  ...
  function sceneGLCheck(scene, access) {
    ...
    // visible === false/'false'/0/'0' 均隐藏 (BASE-05 烘焙动画值可能是数值/
    // 字符串); {user,value} 绑定读 project.json general.properties 的当前值
    ...
    const valHidden = (x) => x === false || x === 'false' || x === 0 || x === '0';
    ...
    for (const obj of objects) {
      ...
      for (const ef of Array.isArray(obj.effects) ? obj.effects : []) {
        ...
        if (!SCENE_GL_EFFECT_WHITELIST.has(dir)) { skipFx('效果未支持'); continue; }
        if (passes.length !== 1) { skipFx('效果 pass 数≠1'); continue; }
        const pass = passes[0];
        const passCombos = pass.combos || {};
        const comboOf = (k) => String(passCombos[k] ?? '0');
        // 结构性 combo gate：只放行已验证的 shader 形态（纯 UV/顶点数学默认路径）
        if (comboOf('PERSPECTIVE') === '1') { skipFx('透视变体未支持'); continue; }
        // H-06: 双波两处开关都要挡 — combo DUALWAVES 与常量 direction2/scale2
        // 任一存在 (CPU waterwaves.js:15 dual = c.direction2 != null ||
        // c.scale2 != null; 旧 gate 只看 combo → 常量驱动的双波被 GL 静默渲单波)
        const wvConsts = pass.constantshadervalues || {};
        if (dir === 'waterwaves' && (comboOf('DUALWAVES') === '1'
          || 'direction2' in wvConsts || 'scale2' in wvConsts)) { skipFx('双波变体未支持'); continue; }
        if (dir === 'foliagesway' && comboOf('MODE') === '1') { skipFx('顶点模式需网格细分'); continue; }
        if (dir === 'shake' && comboOf('AUDIOPROCESSING') !== '0') { skipFx('音频响应未支持'); continue; }
        const texRefs = Array.isArray(pass.textures) ? pass.textures : [];
        const texSlots = [];
        let texBad = false;
        for (const ref of texRefs) {
          const t = glTexInfo(access, ref);
          if (t === undefined || (t && t.video)) { texBad = true; break; }
          texSlots.push(t || null);
        }
        if (texBad) { skipFx('效果纹理不可读'); continue; }
        effects.push({
          dir,
          textures: texSlots,
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        res.end(JSON.stringify({ effect: effectDir, engine: SCENE_GL_ENGINE, vert, frag, combos, uniforms, uniformsFrag: metaF.uniforms, textures }));
      } catch (err) {
        res.statusCode = 500;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: String(err && err.message ? err.message : err) }));
