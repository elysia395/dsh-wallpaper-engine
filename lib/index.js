...
async function extractSceneVideoFrames(src, time, signal) {
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    try {
      ...
      await spawnFfmpeg(ff, ['-ss', String(Number(time) || 0), '-i', tmpVideo, '-frames:v', '1', '-update', '1', '-f', 'image2', '-vcodec', 'png', outPng], { signal });
    } catch (e) {
...
function spawnFfmpeg(ff, args, opts = {}) {
  const signal = opts.signal || null;
...
async function runFfmpegTranscode(abs, out, fps, signal) {
  ...
  for (const enc of ['av1_nvenc', 'h264_nvenc']) {
    try {
      ...
      await spawnFfmpeg(ff, [...base, '-c:v', enc, '-f', 'mp4', out], { signal });
      return;
    } catch (err) {
        ...
        const pending = (async () => {
          ...
          if (isVideo) {
            ...
            try {
              ...
              await spawnFfmpeg(ff, vargs, { signal: ctrl.signal });
              if (ctrl.signal.aborted) throw new Error('cancelled');
