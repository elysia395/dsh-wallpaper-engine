...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 200 = 已有缓存帧, 204 = 无。键语义与 GET 完全一致 (sf38 + mtime)。
      if (method === 'HEAD') {
        const key = 'sf38_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
  ...
  //      GL 会话渲染成功后从 canvas 抓一帧回传, 写入该壁纸的 sf38 缓存槽。
  //      仅空槽写入 (CPU 渲染结果或既有帧优先, 首写胜出 — 语义与 GET 的
  //      与 GET 唯一实现互不踩踏); 与 GET 共用 sf38 键 (abs+mtime)。
  ...
  const SCENE_GL_ENGINE = 'dsh-we-scene-gl/9'; // sf51+sf52: 图像路径 spritesheet 逐帧轮播 + 鼠标坐标 object-fit 内容矩形映射 (W6 视频纹理 + W4 木偶网格仍在; sf53 粒子 v 向修复不改协议, 版本保持 /9)
