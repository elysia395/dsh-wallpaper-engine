...
/** Default macOS content folder (no env var needed for the common case). */
const MAC_DEFAULT_CONTENT_DIR =
  process.platform === 'darwin'
    ? join(process.env.HOME || '', 'Documents', 'dsh', 'we-content')
    : null;

/** Content folders expanded across known layouts (env var + macOS default). */
function contentRootsFromEnv() {
  const roots = [];
  const dirs = envDirs('DSH_WALLPAPER_ENGINE_CONTENT');
  if (MAC_DEFAULT_CONTENT_DIR) dirs.push(MAC_DEFAULT_CONTENT_DIR);
