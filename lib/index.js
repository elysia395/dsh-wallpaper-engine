...
import { buildPuppetPayload } from './we-renderer/puppet-export.js';
...
// 场景 CPU 渲染 worker 注册表 (P0-4②): 创建即注册, finish/卸载时收口 —
// 插件卸载后不允许孤儿线程继续光栅化吃 CPU。
const ACTIVE_SCENE_WORKERS = new Set();
// 在 worker 线程渲染单帧静态图 (SceneRenderer: 背景+模型+粒子+shader 效果),
// 返回 { ok, png(Buffer), diff, checked, degraded? } 或 { ok:false, error }。
// 仅单帧 — 动画烘焙链 (times/APNG/mp4 raw) 已按 0.8.0 决策移除, 不再恢复。
async function renderSceneFrameInWorker(src, width, height, time, opts = {}) {
  await ensureWeInstallLocatedP(); // 含 <install>/assets → GL 全局素材根登记
  ...
  return new Promise((resolve) => {
    ...
    try {
      worker = new Worker(new URL('./scene-render-worker.mjs', import.meta.url), {
        workerData: { src, width, height, time, weAssetsDir, videoFrames: videoFrames && videoFrames.size ? Object.fromEntries(videoFrames) : null },
        type: 'module',
      });
    } catch (e) {
    ...
    const onAbort = () => {
      // 客户端断开 (切换壁纸): 终止 worker 释放 CPU — 结果被丢弃。
      try { worker.terminate(); } catch { /* ignore */ }
    ...
    // 大型场景 CPU 光栅化可能很慢 (单帧 10 分钟上限; 0.7.1 实测 8K 工程
    // ~33s, 部件拼图 ~8s); 超时按失败收尾走提取器回退链。
    timer = setTimeout(() => finish({ ok: false, error: 'scene render timeout' }), 600000);
    worker.on('message', (msg) => {
      if (msg && msg.ok) {
        // 契约 C1: worker 上浮的 CPU degraded 清单 (可缺省) — 透传进返回值
        const degraded = Array.isArray(msg.degraded) ? msg.degraded : undefined;
        finish({ ok: true, png: Buffer.from(msg.png), diff: msg.diff, checked: msg.checked, degraded });
      } else {
...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 200 = 已有缓存帧, 204 = 无。键语义与 GET 完全一致 (sf37 + mtime)。
      if (method === 'HEAD') {
        const key = 'sf37_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
      ...
      (async () => {
        ...
        // sf37 = 静态帧恢复 CPU 渲染优先 (SceneRenderer worker 单帧, 含粒子/
        // shader 效果), 提取器拼合降为回退 (决策修订 2026-09-02: GL 失败/关闭
        // 时的静态底图质量回到 0.7.1 水平; 动画烘焙链保持删除)。
        // (sf36 = CPU 渲染路径移除, 提取器转正 — 已被 sf37 取代。)
        ...
        // P2-11: 渲染参数 (宽 3840, 高按场景 ortho, time=2.5) 进负缓存键。
        let fw = 3840, fh = 2160;
        ...
        if (!servePath) {
          ...
          if (!inflight) {
            // N-02: 渲染取消信号 — 客户端断开 (切壁纸/刷新) 后 worker 照跑到完
            // 是纯浪费; 全部等待者断开时 abort (最后一个关门才杀)。
            const ctrl = new AbortController();
            inflight = (async () => {
              const { extractSceneMainImage, extractSceneMainImageFromDir } = await import('./pkg-extract.js');
              // 完整场景渲染 (SceneRenderer) 优先: worker 线程输出 3840×(按
              // ortho) 全场景帧 (背景+模型+粒子+shader效果), 不阻塞主进程;
              // 失败时回退提取器拼合 (静态图层叠加, 无特效/粒子)。
              try {
                ...
                // GPU 抓帧先写 (scene-frame-cache 空槽写入): CPU 渲染结果丢弃
                // (槽已有帧即够用 — 首写胜出, 避免覆盖也避免浪费)。
                if (existsSync(pngPath) || existsSync(jpgPath)) return pngPath;
                ...
                // 渲染失败(非 scene.pkg 结构/缺资源等) → 回退提取器拼合
                // N-02: 取消 (等待者全部断开) 不是失败 — 不再走回退链白读
                // 整包, 直接向上抛由 422 收尾。
                if (ctrl.signal.aborted) throw new Error('cancelled');
                try { unlinkSync(pngPath); } catch { /* ignore */ }
              }
              // 提取器回退 (多图层拼合/主纹理直出; JPEG 透传)。异步读盘:
              // scene.pkg 可达几十 MB, 阻塞事件循环不可取。
  ...
  //      GL 会话渲染成功后从 canvas 抓一帧回传, 写入该壁纸的 sf37 缓存槽。
  //      仅空槽写入 (CPU 渲染结果或既有帧优先, 首写胜出 — 语义与 GET 的
  //      worker→提取器两段式互不踩踏); 与 GET 共用 sf37 键 (abs+mtime)。
  ...
  const SCENE_GL_ENGINE = 'dsh-we-scene-gl/9'; // sf51+sf52: 图像路径 spritesheet 逐帧轮播 + 鼠标坐标 object-fit 内容矩形映射 (W6 视频纹理 + W4 木偶网格仍在)
  ...
  function sceneGLCheck(scene, access, opts) {
    ...
    // 不做 attachment 锚点)。折叠结果作为 effTr 下发, GL 三路径 (image/particle/
    // puppet) 全部直接消费 (A7: 旧注释 "完整父链渲染由 CPU 侧负责" 已陈旧)。
    ...
    for (const obj of objects) {
      ...
      let puppetRef = null;
      if (isPuppet) {
        const payload = buildPuppetPayload(access, modelJson);
        if (payload) {
  ...
  return () => {
    ...
    ACTIVE_FFMPEG.clear();
    // 终止所有在途场景渲染 worker (P0-4② 卸载兜底): 不终止则卸载后孤儿
    // 线程继续光栅化吃 CPU。
