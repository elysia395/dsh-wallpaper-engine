...
import { resolveSceneTexPath, pkgSceneAccess, dirSceneAccess, parseTex, decodeTex, setGlobalAssetsRoots, readGlobalAsset } from './pkg-extract.js';
...
export function apply(ctx) {
  ...
  const SCENE_GL_ENGINE = 'dsh-we-scene-gl/6'; // W4: 木偶网格渲染 (绑定姿态, MDLA 动画属实验层)
  ...
  function sceneGLCheck(scene, access, opts) {
    ...
    for (const obj of objects) {
      ...
      // 跳过类对象（W3 起粒子已上移独立分支）：3D 模型 / 相机 / 无图像（文字、声音、纯 solid 等）
