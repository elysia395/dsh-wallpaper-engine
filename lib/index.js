...
import { expandIncludes, parseMetaGL } from './we-renderer/glsl/preprocess.js';
...
// sf34 = iris 效果 opacity mask 修复 (旧版无 combos 时全图位移 → 动画"整体上移")
function sceneAnimCachePath(abs, fps, sec, w, h, ext) {
  let mtime = 0;
  try { mtime = statSync(abs).mtimeMs; } catch { /* keep 0 */ }
  const key = 'san_sf34_' + createHash('sha256')
    ...
    const runNext = () => {
      ...
      try {
        proc = spawn(a.file || ff, a.args || args,
          { ...a.opts, cwd: process.env.SystemRoot || (process.platform === 'win32' ? 'C:\\' : '/'), stdio: errFd ? ['ignore', 'ignore', errFd] : 'ignore' });
      } catch (err) {
...
export function apply(ctx) {
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        const pkgBytes = abs.toLowerCase().endsWith('.json') ? null : new Uint8Array(await readFile(abs));
        let bytes = abs.toLowerCase().endsWith('.json')
          ? extractSceneResourceFromDir(dirname(abs), subpath)
          : extractSceneResource(pkgBytes, subpath);
        let mime = null;
        if (!bytes) {
          ...
          const access = glSceneAccess(abs, pkgBytes);
  ...
  function glTexInfo(access, ref) {
    ...
    if (!texPath) return undefined; // 引用存在但解析失败（≠ 槽位为空）
    const file = access.readFile(texPath);
  ...
  const SCENE_GL_EFFECT_WHITELIST = new Set(['waterripple', 'iris']);
  const SCENE_GL_ENGINE = 'dsh-we-scene-gl/1';
  const SCENE_GL_ANIM_KEYS = ['alpha', 'scale', 'origin', 'angles', 'visible', 'color', 'size', 'brightness', 'parallaxDepth', 'zoom'];

  // 白名单 gate（主计划 §3，宁漏勿错）：返回 { ok, reason, objects }
  function sceneGLCheck(scene, access) {
    const fail = (reason) => ({ ok: false, reason });
    const general = scene.general || {};
    if (general.bloom === true) return fail('bloom');
    if (general.hdr === true) return fail('hdr');
    if (general.cameraparallax === true) return fail('parallax');
    const cam = scene.camera || {};
    if (cam.paths) return fail('camera-paths');
    const eye = glVec(glVal(cam, 'eye', '0 0 0'), 3, [0, 0, 0]);
    if (!eye || eye.some((v) => Math.abs(v) > 1e-4)) return fail('eye');
    const zoom = Number(glVal(cam, 'zoom', 1));
    if (!Number.isFinite(zoom) || Math.abs(zoom - 1) > 1e-4) return fail('zoom');
    const objects = Array.isArray(scene.objects) ? scene.objects : [];
    if (objects.length !== 1) return fail('objects:' + objects.length);
    const obj = objects[0];
    if (obj.puppet || obj.particle || obj.particles || !obj.image) return fail('model');
    if (glVal(obj, 'camera', null) === 'default') return fail('camera-object');
    // 纯 image 对象：model json 必须有 material（非 puppet 模型）
    const modelJson = access.readJson(String(obj.image));
    if (!modelJson || modelJson.puppet || !modelJson.material) return fail('model');
    const matJson = access.readJson(String(modelJson.material));
    if (!matJson || !Array.isArray(matJson.passes) || matJson.passes.length !== 1) return fail('passes>1');
    const matPass = matJson.passes[0];
    if (!/^genericimage/i.test(String(matPass.shader || ''))) return fail('shader:' + (matPass.shader || '?'));
    // 对象静态性：alpha/brightness=1、无 colorBlendMode/alignment、无属性动画
    if (Math.abs(Number(glVal(obj, 'alpha', 1)) - 1) > 1e-4) return fail('alpha');
    if (Math.abs(Number(glVal(obj, 'brightness', 1)) - 1) > 1e-4) return fail('brightness');
    if (Number(glVal(obj, 'colorBlendMode', 0)) !== 0) return fail('cbm');
    if (glVal(obj, 'alignment', '') !== '') return fail('alignment');
    for (const key of SCENE_GL_ANIM_KEYS) if (glHasAnim(obj[key])) return fail('anim');
    // 主纹理
    const mainRef = Array.isArray(matPass.textures) ? matPass.textures[0] : null;
    const mainTex = glTexInfo(access, mainRef);
    if (!mainTex) return fail('texture');
    // 效果：不可见效果先剔除（CPU effects.js:37 同款语义），可见效果全过 gate
    const effects = [];
    for (const ef of Array.isArray(obj.effects) ? obj.effects : []) {
      if (glVal(ef, 'visible', true) === false) continue;
      const dir = ef && ef.file ? basename(dirname(String(ef.file))) : '';
      if (!SCENE_GL_EFFECT_WHITELIST.has(dir)) return fail('effect:' + (dir || '?'));
      const passes = Array.isArray(ef.passes) ? ef.passes : [];
      if (passes.length !== 1) return fail('passes>1');
      const pass = passes[0];
      const passCombos = pass.combos || {};
      if (dir === 'waterripple' && String(passCombos.PERSPECTIVE ?? '0') === '1') return fail('perspective');
      const texRefs = Array.isArray(pass.textures) ? pass.textures : [];
      const texSlots = [];
      for (const ref of texRefs) {
        const t = glTexInfo(access, ref);
        if (t === undefined) return fail('texture');
    ...
    return {
      ...
      objects: [{
        ...
        blending: String(matPass.blending || 'normal'),
        mainTexture: mainTex,
        effects,
      }],
    };
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const check = sceneGLCheck(scene, access);
  ...
  // 3g. GET /scene-shader/<token>/<effectDir> — 官方 ES 1.00 shader 展开下发
  ...
  disposers.push(webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const effectDir = rest.slice(token.length).replace(/^\/+/, '');
      if (!SCENE_GL_EFFECT_WHITELIST.has(effectDir)) {
        res.statusCode = 404;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: 'effect-not-allowed', effect: effectDir }));
        return;
      }
      try {
        const access = glSceneAccess(abs, null);
        ...
        const fragFile = access.readFile(`shaders/effects/${effectDir}.frag`);
        const vertFile = access.readFile(`shaders/effects/${effectDir}.vert`);
        if (!fragFile || !vertFile) {
          res.statusCode = 404;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
          res.end(JSON.stringify({ error: 'shader-missing', effect: effectDir }));
          return;
        }
        ...
        const frag = expandIncludes(fragRaw, resolveStub);
        const vert = expandIncludes(vertRaw, resolveStub);
        ...
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store');
        res.end(JSON.stringify({ effect: effectDir, engine: SCENE_GL_ENGINE, vert, frag, combos, uniforms, textures }));
      } catch (err) {
        res.statusCode = 500;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: String(err && err.message ? err.message : err) }));
