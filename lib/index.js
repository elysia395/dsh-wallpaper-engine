  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf26 = 根 puppet 确定性偏移 (壁纸文件数据):
        // shiftY = 网格底部(-minY+1) + shiftX = 网格宽(maxX-minX) — 头整体
        // 上移+右移一个头宽, 子对象跟随. Amiya 697: y+291 x+449.
        const key = 'sf26_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
