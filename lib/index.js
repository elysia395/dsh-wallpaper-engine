  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf30 = image viewShift y 符号修复: vs[1] 直接加
        // (与 renderPuppet 一致) — 旧实现把 vs[1] 放进 oy(减号前) 导致 image 与
        // puppet 的 y 平移方向相反 (差 2×eye.y×ps) — image 组件相对错位
        // = "身体和头发位置相反" (用户实证).
        const key = 'sf32_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
