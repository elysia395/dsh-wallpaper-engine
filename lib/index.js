...
// Live WebGL scene player (ported from dsh-web-ui's skin-center we-* modules).
// WE_SCENE_PLAYER_HTML is served by /wallpaper-engine/scene-runtime; the
// manifest/resource extractors feed it. They are separate modules so the
// existing static-frame path (pkg-extract.js) is never disturbed.
...
export function apply(ctx) {
  ...
  async function buildInventory() {
    ...
    const wallpapers = await Promise.all(all.map(async (w) => {
      ...
      return {
        ...
        // contexts and freeze the page). Falls back to sceneUrl/frameUrl.
  ...
  //     parent's backdrop-filter (liquid glass) can still sample it.
  disposers.push(webServer.register({
    kind: 'prefix',
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const manifest = abs.toLowerCase().endsWith('.json')
          ...
          : buildSceneManifest(new Uint8Array(readFileSync(abs)), tokenB64);
        if (!manifest) {
          res.statusCode = 404;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
    ...
    handler: (req, res) => {
      ...
      try {
        const bytes = abs.toLowerCase().endsWith('.json')
          ...
          : extractSceneResource(new Uint8Array(readFileSync(abs)), subpath);
  ...
  //     static frame / live WebGL player. This is the smooth, non-freezing path
  //     for scene wallpapers (avoids a live WebGL context per scene).
  disposers.push(webServer.register({
    kind: 'prefix',
    ...
    handler: async (req, res) => {
      ...
      const abs = mediaMap.get(token);
      if (!abs) { res.statusCode = 404; res.end(JSON.stringify({ error: 'unknown-token' })); return; }
      try {
        // Read the scene package asynchronously so a large .pkg does not block the
        // event loop (which would freeze the GUI when several wallpapers are clicked
        // in quick succession). The extractor only scans for the embedded MP4, so it
        // is cheap.
        const data = abs.toLowerCase().endsWith('.json') ? null : await readFile(abs);
        const bytes = abs.toLowerCase().endsWith('.json')
          ? extractSceneVideoFromDir(dirname(abs))
          : extractSceneVideo(new Uint8Array(data));
        ...
        let mtime = 0;
        try { mtime = statSync(abs).mtimeMs; } catch { /* keep 0 */ }
        const key = 'sv1_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime) + '.mp4';
        const mp4Path = join(ensureFrameCacheDir(), key);
        if (!existsSync(mp4Path)) await writeFile(mp4Path, bytes);
        res.setHeader('Content-Type', 'video/mp4');
        res.setHeader('Cache-Control', 'no-store');
        trackStream(createReadStream(mp4Path), res);
      } catch (err) {
        res.statusCode = 500;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: String(err && err.message ? err.message : err) }));
