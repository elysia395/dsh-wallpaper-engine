  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf25 = 根 puppet 网格 x 修正 (用户实测):
        // 根 puppet (697) 网格渲染 x 左移 65px (一个眼睛宽度), 仅渲染层偏移
        // 不改 origin → 头相对发/耳/眼右移修复, 发/耳/眼挂载点不变.
        const key = 'sf25_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
