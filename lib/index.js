  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // sf36 = CPU 渲染路径移除: SceneRenderer worker 帧退役, 提取器
        // (extractSceneMainImage 多图层拼合) 转正为唯一静态帧实现 (无特效/
        // 粒子/文字; GL 失败/未就绪时的回退底图)。
        // (sf35 = shake 2π 平滑数学 + UV→像素单位 + mask header/mip0 比。)
        // P-12: mtime 失效键走 sceneCacheStamp — 全精度 mtimeMs + 松散场景
        // 纳入目录内引用文件的 mtime。
        const key = 'sf36_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + sceneCacheStamp(abs);
        ...
        // P2-11: 失败负缓存命中 → 直接回缓存错误码 (键含 sceneCacheStamp,
        // mtime/内容变化自动换键失效; TTL 内失败结论视为仍成立)。
        negKey = key;
        ...
        if (!servePath) {
          ...
          if (!inflight) {
            inflight = (async () => {
              ...
              // 提取器实现 (scene-manifest 管线: 主纹理评分 + 多图层按
              // scene.json 拼合; JPEG 透传)。CPU 渲染路径已移除, 这是唯一的
              // 静态帧实现。异步读盘: scene.pkg 可达几十 MB, 阻塞事件循环不可取。
