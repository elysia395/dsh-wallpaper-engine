...
export function apply(ctx) {
  ...
  function sceneGLCheck(scene, access) {
    ...
    // W1: 官方语义 — 空纹理槽用 shader 元注释 default 填充 ({"default":"util/noise"}
    // 等; WE 编辑器对未覆盖槽位即按 default 取全局纹理)。按效果目录惰性解析
    // (pkg 内 shaders/effects/<dir>.frag/.vert, 与 /scene-shader 同路径同 parseMetaGL);
    // 解析失败按无 default (回退旧行为: 槽位 null → 客户端白/中灰兜底)。
    const samplerDefaultCache = new Map();
    const samplerDefaultFor = (dir, slot) => {
      if (!samplerDefaultCache.has(dir)) {
        const map = new Map();
        try {
          for (const ext of ['frag', 'vert']) {
            const f = access.readFile(`shaders/effects/${dir}.${ext}`);
            if (!f) continue;
        ...
        samplerDefaultCache.set(dir, map);
      }
      return samplerDefaultCache.get(dir).get(slot) || null;
    };
    const out = [];
    let idx = -1;
    for (const obj of objects) {
      ...
      for (const ef of Array.isArray(obj.effects) ? obj.effects : []) {
        ...
        const texSlots = [];
        let texBad = false;
        // W1: 空槽 (null/'null'/'') 先按 shader default 填 (官方语义); 槽数不足
        // 时按 default 表扩展 (如 [null,mask] 缺 slot2 → default util/noise 补上)。
        samplerDefaultFor(dir, -1); // 触发该目录惰性解析 (返回值丢弃, 只为预热缓存)
        const defaults = samplerDefaultCache.get(dir);
        const maxDefaultSlot = defaults && defaults.size ? Math.max(...defaults.keys()) : -1;
        const slotCount = Math.max(texRefs.length, maxDefaultSlot + 1);
        for (let si = 0; si < slotCount; si++) {
          let ref = si < texRefs.length ? texRefs[si] : null;
          if (ref == null || ref === '' || ref === 'null') ref = samplerDefaultFor(dir, si);
