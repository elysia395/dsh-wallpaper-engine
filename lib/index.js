  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        // served stale from disk. sf22 = 官方视图矩阵仅应用于 puppet:
        // image/text 对象不经视图 (ortho(0,w,0,h) 直映), puppet 经视图矩阵
        // (-eye 平移, wallpaper64 逆向 0x1401e8b60/0x1401e8ecb 分支).
        // 修复 Amiya 头被身体盖住 (重叠 145px → 10px).
        const key = 'sf22_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
