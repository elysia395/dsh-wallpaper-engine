...
export function apply(ctx) {
  ...
  function sceneGLCheck(scene, access) {
    ...
    for (const obj of objects) {
      ...
      // 静态不可见对象直接剔除（CPU 同款语义；visible.user 条件取默认值）
      if (glVal(obj, 'visible', true) === false) continue;
      // 跳过类对象：粒子发射器 / 无图像（文字、音频脚本、纯 solid 等）
      if (obj.particle || obj.particles) { mark(name, 'particle', '粒子发射器未支持，已跳过该对象'); continue; }
      if (!obj.image) { mark(name, 'no-image', '文字/脚本/纯色对象未支持，已跳过该对象'); continue; }
      if (glVal(obj, 'camera', null) === 'default') { mark(name, 'camera-object', '相机对象未支持，已跳过该对象'); continue; }
      ...
      for (const ef of Array.isArray(obj.effects) ? obj.effects : []) {
        ...
        if (dir === 'waterwaves' && comboOf('DUALWAVES') === '1') { skipFx('双波变体未支持'); continue; }
