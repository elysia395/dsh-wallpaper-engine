window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function RopeDock() {
		  ...
		  // When the mascot FORM or SIZE changes (bigger box), re-clamp the position so
		  // a larger form (e.g. whale) never ends up half off-screen. Runs after render
		  // so ropeSize() reads the freshly updated box.
		  React.useEffect(() => {
		  ...
		  return React.createElement(React.Fragment, null,
		    React.createElement("div", {
		      ...
		      style: { top: pos.y + "px", left: pos.x + "px", width: box.w + "px", height: box.h + "px" },
		      role: "button",
		      tabIndex: 0,
