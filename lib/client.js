window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const _WE_GL_VERSION = 6; // W4: 木偶网格渲染 (绑定姿态) — 与 host SCENE_GL_ENGINE 同步
		...
		// ps = 画布/ortho 比; origin/scale 取 effTr; 相机眼位 x 平移 (木偶非满幅背景,
		// 无 isBg 豁免)。矩阵列主序, m[10]=-1/m[15]=1 与 _weGLQuadMVP 同 z 约定。
		function _weGLPuppetMVP(obj, CW, CH, ortho, camEye, out) {
		  ...
		  m[10] = -1; m[15] = 1;
		  m[12] = 2 * (tr.origin[0] * ps[0] - (camEye[0] || 0) * ps[0]) / CW - 1;
		...
		function createSceneGLRenderer(opts) {
		  ...
		  function _weGLSkinPuppet(pm, t, verts, obj) {
		    ...
		    const FPS = 30; // 官方骨骼动画 30fps (CPU 同款硬编码)
		    ...
		    for (let li = 0; li < S.layers.length; li++) {
		      ...
		      const frame = Math.floor(t * FPS * (layer.rate || 1)) % anim.frameCount;
		      const lw = (fr) => {
		        const base = fr * nb * 3;
		    ...
		    for (let i = 0; i < objs.length; i++) {
		      ...
		      if (o.puppetMesh) {
		        ...
		        if (!pm.mvp || pm.mvpW !== CW || pm.mvpH !== CH) {
		          pm.mvp = _weGLPuppetMVP(o.obj, CW, CH, layoutOrtho, camEye, pm.mvp || new Float32Array(16));
		          pm.mvpW = CW;
		          pm.mvpH = CH;
		        }
