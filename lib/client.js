window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const selection = {
		  ...
		  type: null,
		  previewUrl: null,
		  // Live WebGL scene player URL (transient, set at apply time). When a
		  // scene wallpaper is selected and this is non-null, buildMedia renders
		  // it as a sandboxed same-origin iframe instead of the static frame.
		  sceneUrl: null,
		  // Scene animation exposed as an MP4 (<video>). Preferred over the live
		  // WebGL player: hardware-decoded and smooth, no per-scene WebGL context.
		  sceneVideo: null,
		...
		function applySelection(id) {
		  ...
		  // renders it as a same-origin iframe; frameUrl remains the static-frame
		  // fallback for scenes that fail to parse or render.
		...
		function buildMedia(sel) {
		  ...
		  // dsh-web-ui uses. Scenes without an embedded video fall back to the
		  // extracted static frame.
		...
		function syncLayers() {
		  ...
		  if (selection.url) {
		    ...
		    if (existing && gotKey !== wantKey) {
		      ...
		      // rapid wallpaper switches never accumulate live/decoding media (the
		      // page would otherwise grind to a halt as old <video>s keep decoding).
