...
export function decodeTex(data) {
    return decodeTexParsed(parseTexInternal(data));
}
// P1-2（docs/perf-audit-2026-08-29-review2.md §4）：供已持有 parseTexInternal 结果
// 的调用方复用，消除 decodeTex 内部的二次容器解析（extractSceneResourceVia 此前
// parse 一次 + decodeTex 内再 parse 一次）。decodeTex 对外契约不变。
export function decodeTexParsed(parsed) {
 ...
 */
// P1-2 注：本函数在宿主主线程同步 deflate level 9。异步化暂缓: 需 worker 化
// （/scene-resource 路由必须保持同步返回 Buffer 的语义）；level 亦不动 —— 换
// level 会改 /scene-resource 响应字节，GL 端缓存键/对比依赖现状（review2 §4）。
...
function tryCompositeMultiLayerScene(scene, access, topCandidate) {
    ...
    for (const obj of objects) {
        ...
        // 对象定位: WE 的 origin 是画布中心对齐的世界坐标 (ortho 空间, y 向下,
        // 背景 origin=画布中心即 1920,1080@3840x2160) — 纹理左上角 = origin -
        // texSize/2。origin 缺失/不可解析 (动画/脚本节点) 时退回旧的居中+
        // cropoffset 行为 (单主图场景的既有正确路径)。sf36 前旧实现完全忽略
        // origin — 部件拆分场景 (3735447194: 身体/腿/裙摆独立模型) 全部叠到
        // 画布中心, 静态图乱叠。
        const originRaw = obj.origin;
        let originParts = null;
        if (typeof originRaw === 'string') {
            const nums = originRaw.trim().split(/\s+/).map(Number);
            if (nums.length >= 2 && nums.every((n) => Number.isFinite(n)))
                originParts = nums;
        }
        else if (originRaw && typeof originRaw === 'object' && typeof originRaw.value === 'string') {
            const nums = String(originRaw.value).trim().split(/\s+/).map(Number);
            if (nums.length >= 2 && nums.every((n) => Number.isFinite(n)))
                originParts = nums;
        }
        let ox = 0;
        ...
        let startY;
        if (originParts) {
            startX = Math.round(originParts[0] - decoded.width / 2 + ox);
            startY = Math.round(originParts[1] - decoded.height / 2 - oy);
        }
        else {
            startX = Math.round(canvasWidth / 2 + ox - decoded.width / 2);
            startY = Math.round(canvasHeight / 2 - oy - decoded.height / 2);
        }
...
// P1-2：导出以接入宿主缓存访问层 —— index.js 的 glSceneAccess 返回 pkg-extract
// 的 SceneAccess（其 pkgSceneAccess/dirSceneAccess 的 readFile 语义与本模块同构），
// 可安全传入；候选链与 decode 失败回退原始字节的行为不变。
export function extractSceneResourceVia(access, subpath) {
    ...
    try {
        ...
        // P1-2：复用上方已 parse 的结果，去掉内嵌的二次 parseTexInternal。
        const dec = decodeTexParsed(parsed);
