...
export function readPkgDir(dir) {
  ...
  // P2-22: JSON 解析结果按路径缓存 (访问器实例级 — 每次调用各自新建)。
  // 目录内容在渲染器实例生命周期内不可变; 已核对全部调用方
  // (core/camera/model/particles/image) 对返回对象只读, 缓存原引用安全
  const jsonCache = new Map();
  const readJson = (p) => {
    if (jsonCache.has(p)) return jsonCache.get(p);
    const b = read(p);
    const j = b ? JSON.parse(b.toString('utf8')) : null;
    jsonCache.set(p, j);
    return j;
  };
  return {
...
export function readPkg(pkgPath) {
  ...
  // P2-22: JSON 解析结果按路径缓存 (pkg 内容不可变; 旧实现每帧 readJson 重跑
  // probeLz4Chain + readPkgEntry 整条目 LZ4 解压 + JSON.parse — image/model/
  // camera/particles 每帧逐对象读模型/材质 JSON 全中)。调用方只读, 缓存原引用
  const jsonCache = new Map();
  const readJson = (p) => {
    if (jsonCache.has(p)) return jsonCache.get(p);
    const b = read(p);
...
// P1-11: parseTex 按同一 buffer memoize (WeakMap) — parseTexInternal 需逐 mip
// LZ4 解压整个容器, 同 buffer 重复解析是纯浪费。loadTexImage 内 parseTex 与
// decodeTex 的双解析已合并: parseTex 结果携带完整解析结果 + 源 buffer 引用,
// 经 decodeTex 的 preParsed 参数复用 (内部引用比对做同源校验, 见 pkg-extract.js)
const _texParseMemo = new WeakMap();
function parseTexCached(raw) {
  let v = _texParseMemo.get(raw);
  if (v === undefined) { v = parseTex(raw); _texParseMemo.set(raw, v); }
  return v;
}
export function loadTexImage(raw) {
  const info = parseTexCached(raw);
  // P1-11: memoize 的 parse 结果作 preParsed 传入 — decodeTex 内不再二次
  // parseTexInternal (键同源: raw 既是 memo 的 WeakMap 键, 也是这里的 data)
  const dec = decodeTex(raw, info);
