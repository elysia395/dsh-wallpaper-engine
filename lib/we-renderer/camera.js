...
export function setupCameraMatrices(scene, cam, gen, pose, W, H, fovOverride) {
  ...
  // A1: lwe 正交 mvp = ortho·T(+eye)·lookAt — 与 lookAt 的 T(-eye) 抵消
  // (center=eye+(0,0,-1) 常态)。正交时 vp 补 T(eye), 消除 eye 净位移。
  if (isOrtho) {
    const Teye = mat4Identity();
    Teye[12] = pose.eye[0]; Teye[13] = pose.eye[1]; Teye[14] = pose.eye[2];
    return { view, proj, vp: mat4Mul(mat4Mul(proj, Teye), view), isOrtho };
  }
...
,
    _setupCamera() {
        ...
        // A1: lwe 正交 mvp = ortho·T(+eye)·lookAt — eye 与 view 平移抵消
        // (center = eye+(0,0,-1) → lookAt 纯平移 T(-eye))。模型路径同样
        // 消除 eye 净位移; 透视路径 lwe 无 T(+eye), 保持原样。
        if (this.camIsOrtho) {
          const Teye = mat4Identity();
          Teye[12] = eye[0]; Teye[13] = eye[1]; Teye[14] = eye[2];
          this.camVP = mat4Mul(mat4Mul(this.camProj, Teye), this.camView); // Clip = Proj·T(eye)·View·World·p
        } else {
...
,
    _viewShift(o, size, ps) {
        // A1 (官方/lwe 对照复核): 正交相机 eye 完全抵消, 静态 eye 不产生任何位移。
        //  1. lwe Camera.cpp:42-51: projection = glm::translate(ortho, eye)
        //     = ortho·T(+eye); CImage.cpp:382/1108: mvp = projection·lookAt·model。
        //  2. 测试集 7 张壁纸 (WE 常态) 全部 center = eye+(0,0,-1), up = (0,1,0)
        //     → lookAt 旋转部分 = 单位阵 → view = T(-eye)
        //     → T(+eye)·T(-eye) = I, eye 零影响。
        //  3. 布局实证: 3427824116 背景 (4096,2303)+8192×4608 正交正中、
        //     3735447194 背景 (1920,1080)+3840×2160 正中 — 若 eye 产生净位移,
        //     官方引擎中背景即离心中 ~439px, 不可能。
        // 旧 "静态 eye 仅 x 平移前景" (sf32 校准) 是系统性错误:
        //  3735447194 前景错移 -267.5×ps → 右侧拼缝; 3427824116 前景错位 438.9×ps。
        // 相机对象 camera:"default" 运镜 (sf33: origin 完整位移 x+y, 背景豁免)
        // 保留兼容 — 测试集无相机对象。
        if (!this._camObjDriven) return [0, 0];
