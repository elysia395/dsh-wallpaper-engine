...
// P0-5: blitScaled 的 colorBlendMode 逐像素混合改 applyBlendingInto 直写
// (A/B/blended scratch 提到循环外, 数值语义与原 applyBlending 逐位一致)
import zlib from 'node:zlib';
import { applyBlendingInto } from './math.js';
...
export class Canvas {
  ...
  blit(img, dx, dy, alpha = 1) {
    ...
    for (let ty = y0; ty < y0 + img.height; ty++) {
      ...
      for (let tx = x0; tx < x0 + img.width; tx++) {
        ...
        // P2-16: 不透明快速路径 — a===1 且目标 alpha=255 (dstA=1) 时 source-over
        // 退化: outA=1+1*0=1、(src*1 + dst*1*0)/1 === src (整数)、
        // Math.round(整数)=整数、alpha=round(1*255)=255 — 与通用路径逐位一致
        // (差分测试覆盖全 alpha/边界组合)
        if (a === 1 && this.data[di + 3] === 255) {
          this.data[di] = img.rgba[si];
          this.data[di+1] = img.rgba[si+1];
          this.data[di+2] = img.rgba[si+2];
          this.data[di+3] = 255;
          continue;
        }
  ...
  blitScaled(img, dx, dy, dw, dh, alpha = 1, blendMode = 0) {
    ...
    const rgba = img.rgba;
    // P0-5: colorBlendMode 逐像素混合 scratch (循环外一次分配)
    const blendA = [0, 0, 0], blendB = [0, 0, 0], blendOut = [0, 0, 0];
    for (let ty = y0; ty < y1; ty++) {
      ...
      for (let tx = x0; tx < x1; tx++) {
        ...
        const di = (ty * this.w + tx) * 4;
        // P2-16: 不透明快速路径 — a===1 且目标 alpha=255 时 (blendMode<=0 路):
        // outA=1、(r*1 + dst*1*0)/1 === r、alpha=round(1*255)=255 — 与通用路径
        // 逐位一致 (blend 路需 a 参与 ApplyBlending, 不走此捷径)
        if (a === 1 && this.data[di + 3] === 255 && blendMode <= 0) {
          this.data[di] = Math.round(r);
          this.data[di+1] = Math.round(g);
          this.data[di+2] = Math.round(b);
          this.data[di+3] = 255;
          continue;
        }
        const dstA = this.data[di + 3] / 255;
        ...
        if (blendMode > 0) {
          ...
          // P0-5: 直写式混合 (scratch 提到像素循环外, 零分配)
          blendA[0] = this.data[di] / 255; blendA[1] = this.data[di + 1] / 255; blendA[2] = this.data[di + 2] / 255;
          blendB[0] = r / 255; blendB[1] = g / 255; blendB[2] = b / 255;
          applyBlendingInto(blendMode, blendA, blendB, a, blendOut);
          this.data[di] = Math.round((blendOut[0] * a + this.data[di] * dstA * (1 - a)) / outA);
          this.data[di+1] = Math.round((blendOut[1] * a + this.data[di + 1] * dstA * (1 - a)) / outA);
          this.data[di+2] = Math.round((blendOut[2] * a + this.data[di + 2] * dstA * (1 - a)) / outA);
          this.data[di+3] = Math.round(outA * 255);
          continue;
        }
  ...
  blitRotated(img, cx, cy, dw, dh, angle, alpha = 1) {
    ...
    for (let ty = y0; ty < y1; ty++) {
      ...
      for (let tx = x0; tx < x1; tx++) {
        ...
        const di = (ty * this.w + tx) * 4;
        // P2-16: 不透明快速路径 — a===1 且目标 alpha=255 时: outA=1 恒过下方
        // <=0 守卫、(r*1 + dst*1*0)/1 === r、alpha=round(1*255)=255 — 与通用
        // 路径逐位一致
        if (a === 1 && this.data[di + 3] === 255) {
          this.data[di] = Math.round(r);
          this.data[di + 1] = Math.round(g);
