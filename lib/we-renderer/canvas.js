...
export class Canvas {
  ...
  // clipRect (可选 [x,y,w,h] 屏幕坐标): 只绘制 clip 内像素 (puppet FBO 裁剪语义)
  blitScaled(img, dx, dy, dw, dh, alpha = 1, clipRect = null) {
    if (dw === 0 || dh === 0) return;
    const flipX = dw < 0, flipY = dh < 0;
    let x0 = Math.floor(flipX ? dx + dw : dx), y0 = Math.floor(flipY ? dy + dh : dy);
    let x1 = Math.ceil(flipX ? dx : dx + dw), y1 = Math.ceil(flipY ? dy : dy + dh);
    if (clipRect) {
      const cX0 = Math.max(x0, Math.floor(clipRect[0])), cY0 = Math.max(y0, Math.floor(clipRect[1]));
      const cX1 = Math.min(x1, Math.ceil(clipRect[0] + clipRect[2])), cY1 = Math.min(y1, Math.ceil(clipRect[1] + clipRect[3]));
