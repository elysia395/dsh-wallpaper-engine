...
export function installText(proto) {
  Object.assign(proto, {
    ...
    // P2-20: 位图缓存改字节预算 LRU — 旧 FIFO 仅按条数 (256), 大位图 (长文本 ×
    // 高 px, 单条可达数 MB) 驻留上界不可控; 现按 RGBA 字节 32MB 预算 + 256 条
    // 双闸门逐出, 命中刷新热度 (Map 删除重插 = 移到最新端), 逐出最旧条目
    _fontCache: new Map(),
    _textBitmapCache: new Map(),
    _textBitmapStat: { bytes: 0 },
    _renderTextCached(fontPath, text, px, color, alpha) {
      ...
      if (hit) {
        this._textBitmapCache.delete(key);
        this._textBitmapCache.set(key, hit);
        return hit;
      }
      ...
      this._textBitmapStat.bytes += img.width * img.height * 4;
      // 双闸门逐出: 字节预算 (32MB) + 条数上限 (256), 任一超限从最旧端逐出
      while ((this._textBitmapStat.bytes > 32 * 1024 * 1024 || this._textBitmapCache.size > 256)
        && this._textBitmapCache.size > 1) {
        const first = this._textBitmapCache.keys().next().value;
        if (first === undefined) break;
        const old = this._textBitmapCache.get(first);
        this._textBitmapCache.delete(first);
        this._textBitmapStat.bytes -= old.width * old.height * 4;
      }
      return img;
    },
