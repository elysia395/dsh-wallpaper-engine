...
,
    ...
    effectPulse(tex, c, t, combos, pass) {
        ...
        if (audioMode > 0) {
          ...
          // 官方非音频: smoothstep(bounds, sin(time×speed + (phase−0.25)×2π)×0.5+0.5) × amount
          const sv = Math.sin(t * speed + (phase - 0.25) * 6.28318530718) * 0.5 + 0.5;
...
,
    effectFilmgrain(tex, c, t, combos, pass) {
      ...
      // opacity (引擎 shader: effects/opacity.frag): albedo.a *= mask.r
      //   g_Texture0 = 对象自身纹理, g_Texture1 = mask (默认 util/white)
      //   v_TexCoord.zw = mask UV (官方按 mask 分辨率缩放; CPU 简化用对象 uv)
      ,
    effectOpacity(tex, c, t, pass) {
        ...
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            out[di + 3] = Math.round(src[di + 3] * m);
          }
        }
        return { width: W, height: H, rgba: out };
      }
    ...
    effectFoliageSway(tex, c, t, pass) {
        ...
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            const pr = rotate(u, v);
            let amp = ampBase;
            if (maskTex) amp *= this._texSample(maskTex, u, v)[0];
    ...
    effectWaterripple(tex, c, t, ef, pass) {
        ...
        for (let y = 0; y < h; y++) {
          for (let x = 0; x < w; x++) {
            ...
            const uu = Math.max(0, Math.min(1, u + (nx / nl) * s2));
            const vv = Math.max(0, Math.min(1, v + (ny / nl) * s2));
            const s = this._texSample(tex, uu, vv);
            const di = (y * w + x) * 4;
            out[di] = Math.round(s[0] * 255); out[di + 1] = Math.round(s[1] * 255);
            out[di + 2] = Math.round(s[2] * 255); out[di + 3] = Math.round(s[3] * 255);
    ...
    effectShake(tex, c, t, pass) {
        ...
        for (let y = 0; y < h; y++) {
          for (let x = 0; x < w; x++) {
            ...
            let fmx = 0, fmy = 0;
            if (flowTex) {
              const fm = this._texSample(flowTex, u, v);
              fmx = (fm[0] - 0.498) * 2;
              fmy = (fm[1] - 0.498) * 2;
            }
