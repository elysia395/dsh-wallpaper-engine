...
export function installPuppet(proto) {
  Object.assign(proto, {
    renderPuppet(o, model, tr, t) {
        ...
        // MOD-07: 层解析抽为 _resolveAnimLayers 单份实现 (原 renderPuppet 内联
        // 副本与 core.js _puppetBoneFinal 内副本三份易漂移; core.js 本轮不可改,
        // 其副本待后续统一到此)
        const animLayers = this._resolveAnimLayers(o, mesh);
        ...
        // MOD-11: 退化/空网格 (无有限顶点) → _meshBounds 返回 null, 跳过渲染并
        // log/onDegraded。旧实现 ±1e9 哨兵值进尺寸计算 → W 非有限 → Uint8Array
        // 分配抛错被外层 catch 吞 → 整对象静默丢失。
        if (!rawBounds) {
          this.log('跳过 puppet ' + (o.name || o.id) + ': 退化网格 (无有效顶点)');
          if (typeof this.onDegraded === 'function') {
            try { this.onDegraded({ object: o.name != null ? o.name : String(o.id), feature: 'puppet-mesh', action: 'skip-degenerate-mesh' }); } catch { /* 回调失败不影响渲染 */ }
          }
          return;
        }
        ...
        const alphaMul = getVal(o, 'alpha', 1) * getVal(o, 'brightness', 1);
        if (tr.angle !== 0) {
          // P1-22/MOD-03: 应用对象旋转 tr.angle (含父链累积) — 与 renderImage 同款
          // 像素空间刚体旋转 (§9.4: 正角屏幕逆时针, 绕对象中心); 旧实现忽略 angle,
          // 旋转的 puppet/其父组不转而其余对象转。
          this.canvas.blitRotated(img, dx + dw / 2, dy + dh / 2, dw, dh, tr.angle, alphaMul);
        } else {
          this.canvas.blitScaled(img, dx, dy, dw, dh, alphaMul);
        }
      }

      // 动画层解析 (MOD-07 抽出): visible 层过滤 + 层名/数字后缀/索引回退 →
      // [{animIdx, blend, rate, additive}]; 无动画层时返回 null
,
...
,
    _skinPuppet(mesh, t, cxs, cys, layers = null) {
        ...
        // P1-3 (豁免, 仅注释): fps 硬编码 30 — MDLA 头部疑带帧率浮点字段但布局
        // 未实证 (MOD-04 的 0xF0 0x41 魔数扫描只定位头不读值); fps≠30 的模型存在
        // 前提下播放速度才错, 待官方元数据实测后再动。
...
,
    _meshBounds(positions) {
        // MOD-11: 跳过非有限顶点; 无任何有效顶点 → 返回 null (调用方跳过渲染),
        // 不让 ±1e9 哨兵值进尺寸计算 (空/退化网格 → W=ceil(±2e9) → 分配抛错)
...
,
    _rasterizeMesh(mesh, tex, skinned, bounds, W, H, flipY) {
        ...
        // P1-10: 采样写入式 scratch + 标量预乘 (旧实现每像素 pm 4×4 矩阵 + out +
        // 返回值共 7 个数组分配)。逐通道表达式与旧实现逐位一致; 调用方即取即用,
        // 不跨采样持有
        const sm = [0, 0, 0, 0];
        const sample = (u, v) => {
          ...
          for (let c = 0; c < 4; c++) {
            // c=3 (alpha) 预乘恒等: 原 pm 第 4 列是 a 自身 (rgb 才乘 a)
            const p00 = c === 3 ? tdata[i00 + 3] : tdata[i00 + c] * tdata[i00 + 3];
            const p10 = c === 3 ? tdata[i10 + 3] : tdata[i10 + c] * tdata[i10 + 3];
            const p01 = c === 3 ? tdata[i01 + 3] : tdata[i01 + c] * tdata[i01 + 3];
            const p11 = c === 3 ? tdata[i11 + 3] : tdata[i11 + c] * tdata[i11 + 3];
            const top = p00 * (1 - tx) + p10 * tx;
            const bot = p01 * (1 - tx) + p11 * tx;
            sm[c] = top * (1 - ty) + bot * ty;
          }
          if (sm[3] < ALPHA_CUTOFF) { sm[0] = 0; sm[1] = 0; sm[2] = 0; sm[3] = 0; return sm; }
          const a = sm[3];
          sm[0] = Math.min(255, Math.round(sm[0] / a));
          sm[1] = Math.min(255, Math.round(sm[1] / a));
          sm[2] = Math.min(255, Math.round(sm[2] / a));
          sm[3] = Math.round(a);
          return sm;
        };
        ...
        for (let t = 0; t < indices.length; t += 3) {
          ...
          // P1-10: 三角形顶点标量化 (旧每三角形 3 个 2 元数组)
          const ax = skinned[i0][0] - bounds.minX, ay = flipY(skinned[i0][1]);
          const bx = skinned[i1][0] - bounds.minX, by = flipY(skinned[i1][1]);
          const cx = skinned[i2][0] - bounds.minX, cy = flipY(skinned[i2][1]);
          const bx0 = Math.max(0, Math.floor(Math.min(ax, bx, cx)));
          const bx1 = Math.min(W - 1, Math.ceil(Math.max(ax, bx, cx)));
          const by0 = Math.max(0, Math.floor(Math.min(ay, by, cy)));
          const by1 = Math.min(H - 1, Math.ceil(Math.max(ay, by, cy)));
          ...
          for (let y = by0; y <= by1; y++) {
            for (let x = bx0; x <= bx1; x++) {
              ...
              const la = ((bx - px) * (cy - py) - (by - py) * (cx - px)) / area;
              const lb = ((cx - px) * (ay - py) - (cy - py) * (ax - px)) / area;
              const lc = ((ax - px) * (by - py) - (ay - py) * (bx - px)) / area;
