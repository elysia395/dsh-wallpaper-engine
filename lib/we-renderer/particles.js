...
import { firstPass } from './materials/compile.js';
...
,
    _buildParticleSystemInner(o, def, ctx) {
        ...
        return {
          ...
          // 在场景时间 starttime 后才开始发射 (静态帧 t=静止态时同样生效)。
...
,
    _spawnParticle(sys, em) {
        ...
        const p = {
          ...
          // 粒子世界场景坐标 (y 向上): **对象世界原点 + scale×(emitter 局部偏移 + 随机)**
          // (根因 C2 修复: 旧 scenePos 不含 sys.origin → 非零原点对象 (樱花 1102,1523
          // 等) 的粒子整体错位到 (350,750); lwe CParticle: world = 对象原点 +
          // scale×(emitter 原点 + random), emitter origin 相对对象原点)
          scenePos: [
            sys.origin[0] + (em.origin[0] * em.scale[0] + rx * em.scale[0]),
            sys.origin[1] + (em.origin[1] * em.scale[1] + ry * em.scale[1]),
            0,
          ],
        ...
        for (const p of sys.particles) {
          switch (op.name) {
            case 'movement': {
              ...
              // 速度/重力 y-up 场景约定 (lwe CParticle: position += velocity·dt 直接
              // 应用; velocityrandom 负 y = 下落 (樱花 -100..-15 → 落), gravity 正 y =
              // 上升 (气泡 +100 → 升))。旧实现 scenePos 用 -vel[1] 且 vel[1] +=
              // -gravity[1] — 双重取反使重力方向恰好正确 (气泡上升) 但速度方向错误
              // (樱花上浮); 修正为直接应用 (画布 y-down 仍取反)。
              p.pos[0] += p.vel[0] * dt;
              p.pos[1] += -p.vel[1] * dt;
...
,
    _drawParticles(sys) {
        ...
        // 相机视图平移 (根因 C1): 与 image/puppet 一致 — 旧实现粒子不应用
        // _viewShift, eye≠0 / 相机对象驱动场景中粒子与场景内容整体错位
        const vs = ps ? this._viewShift(sys.o, [0, 0], ps) : [0, 0];
        // 正交 zoom (根因 B): 粒子位置绕场景中心缩放 + 尺寸×Z
        ...
        for (const p of sys.particles) {
          ...
          let x, y;
          if (ps && p.scenePos) {
            // 正交: scenePos (场景坐标 y 向上) → zoom 中心 + 视图平移 (与 image 同款)
            const [cx, cy] = this._orthoZoomCenter([p.scenePos[0], p.scenePos[1]], vs);
            x = cx; y = cy;
          } else if (ps) {
            x = p.pos[0] * ps[0] * Z; y = p.pos[1] * ps[1] * Z;
          } else {
            x = p.pos[0]; y = p.pos[1];
          }
          const halfX = (ps ? sz * ps[0] : sz) / 2 * Z;
          ...
          const halfY = ((ps ? sz * ps[1] : sz) / 2) * ratio * Z;
          if (tex) {
