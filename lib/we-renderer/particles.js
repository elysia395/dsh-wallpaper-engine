...
,
    _simulateParticleSystem(sys, t) {
        ...
        // MOD-30: 步数上限 × dt 0.05 = 最多模拟 100s, 超出部分直接跳钟
        ...
        let guard = 0;
        const guardMax = Math.ceil(maxSeconds / 0.05);
...
,
    _spawnParticle(sys, em) {
        ...
        // P0-6: 发射初速 (WE 语义) — speed 在 [speedmin, speedmax] 内随机, 方向为
        // cone 锥角 (度) 内随机采样 (绕 directions 基准方向) 的单位向量 × speed;
        // sign 每轴符号约束 (1=只正向, -1=只负向, 0=不限)。速度存画布坐标系
        // (y 向下, 与 velocityrandom/movement 一致)。
        let vel = [0, 0, 0];
        if (em.speedMax > 0 || em.speedMin > 0) {
          const speed = em.speedMin + rng() * Math.max(0, em.speedMax - em.speedMin);
          const baseA = Math.atan2(em.directions[1], em.directions[0]);
          const halfCone = (Math.abs(em.cone) * Math.PI / 180) / 2;
          const a = halfCone > 0 ? baseA + (rng() * 2 - 1) * halfCone : baseA;
          let vx = Math.cos(a), vy = Math.sin(a);
          if (em.sign[0] > 0 && vx < 0) vx = -vx;
          else if (em.sign[0] < 0 && vx > 0) vx = -vx;
          if (em.sign[1] > 0 && vy < 0) vy = -vy;
          else if (em.sign[1] < 0 && vy > 0) vy = -vy;
          vel = [vx * speed, -vy * speed, 0]; // 场景 y 向上 → 画布 y 向下翻转
        }
        ...
        for (const p of sys.particles) {
          switch (op.name) {
            ...
            case 'turbulence': {
              ...
              // MOD-29: 用系统确定性 rng (旧实现 Math.random — 依赖全局替换, 且
              // 每步重抽 sp/phase 场力 20Hz 抖动; rng 换 sys.rng 保证帧可复现)
              const trng = sys.rng || Math.random;
              const sp = speedMin + trng() * (speedMax - speedMin);
