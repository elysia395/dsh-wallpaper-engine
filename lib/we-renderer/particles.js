        ...
        for (const p of sys.particles) {
          switch (op.name) {
            case 'movement': {
              ...
              p.pos[0] += p.vel[0] * dt;
              p.pos[1] += p.vel[1] * dt;
              p.vel[0] += gravity[0] * spd * dt;
              p.vel[1] += -gravity[1] * spd * dt; // Y flip
            ...
            case 'controlpointattract': {
              // sf41a(回退 no-op): 官方力语义未定标 — 直接 a=scale 实测把泡泡
              // 钉死不上升。保持与 GL 一致的 no-op, 解析/表下发保留。
              break;
            }
          ...
          if (tex) {
            ...
            for (let py = y0; py <= y1; py++) {
              ...
              for (let px = x0; px <= x1; px++) {
                ...
                const sa = a * texA;
                const di = (py * W + px) * 4;
                const sr = colorR * texR2 * sa * 255, sg = colorG * texG2 * sa * 255, sb = colorB * texB2 * sa * 255;
                if (additive) {
