...
// P0-5: 着色热路径零分配采样 scratch (同步非重入, 模块级复用; 每像素采样值
// 即取即用, 不跨像素持有 — 与原每次 _texSample 新数组语义等价)
const _smRG = [0, 0];
const _smA = [0, 0, 0, 0], _smB = [0, 0, 0, 0], _smC = [0, 0, 0, 0], _smD = [0, 0, 0, 0];
// P1-10: _rasterizeMesh3D 每像素 shade() 调用的 worldPos/normal scratch (旧实现
// 每像素 2 个字面量数组; shade 各实现同步只读、即取即用, 不跨像素持有)
const _pxWp = [0, 0, 0];
const _pxN = [0, 0, 0];
        ...
        for (let tIdx = 0; tIdx + 2 < indices.length; tIdx += 3) {
          ...
          for (let py = by0; py <= by1; py++) {
            for (let px = bx0; px <= bx1; px++) {
              ...
              // P1-10: 写入式 scratch (替代每像素 2 个字面量数组)
              _pxWp[0] = wx; _pxWp[1] = wy; _pxWp[2] = wz;
              _pxN[0] = nx; _pxN[1] = ny; _pxN[2] = nz;
              const col = shade(u, v, _pxWp, _pxN, this.camEye, lmU, lmV, px / W, py / H);
...
,
    _shadeFlowImage(u, v, uniforms, textures, texNames, t) {
        ...
        // P0-5: RG 写入式采样 (流向图只取 R/G 解码方向)
        const f = this._texRG(flowTex, u, v, false, _smRG);
        ...
        const sampleFlow = (tex, speed) => {
          ...
          // P0-5: 双相位采样写入式 (s1/s2 同像素并读 → 两块 scratch)
          const s1 = this._texSampleInto(tex, u + o1x, v + o1y, false, _smB);
          const s2 = this._texSampleInto(tex, u + o2x, v + o2y, false, _smC);
          return [
...
,
    _shadeGeneric(u, v, wp, n, eye, uniforms, textures, t, combos, u2, v2, su, sv) {
        ...
        // P0-5: 写入式采样 (null 纹理契约 [1,1,1,1] 由 _texSampleInto 直写保留)
        const albedo = this._texSampleInto(tex0, u, v, true, _smA);
        if (!tex0) { albedo[0] = 1; albedo[1] = 1; albedo[2] = 1; albedo[3] = 1; }
        ...
        // 法线
        // P1-10: 法线/视线/逐光源向量全部标量化 (旧实现每像素 v3norm×2 + 每光源
        // v3sub/v3add/v3norm/ldir/c 共 ~5 个数组分配); 表达式与 math.js v3sub/
        // v3add/v3dot/v3norm 逐位同序 — sqrt(dot)||1 及左结合加法均保持
        let nX, nY, nZ;
        if (combos && (combos.NORMALMAP || combos.normalmap) && textures[1]) {
          // P0-5: 法线图写入式采样 (只消费 R/G)
          const nm = this._texSampleInto(textures[1], u, v, false, _smB);
        ...
        const vex = eye[0] - wp[0], vey = eye[1] - wp[1], vez = eye[2] - wp[2];
        const vl = Math.sqrt(vex * vex + vey * vey + vez * vez) || 1;
        const viewX = vex / vl, viewY = vey / vl, viewZ = vez / vl;
        const roughness = uniforms.Rough != null ? uniforms.Rough : 0.5;
        const metallic = uniforms.Metal != null ? uniforms.Metal : 0;
        const gLight = uniforms.Light != null ? uniforms.Light : 0;
        const specPower = (1.01 - roughness) * (400 + (250 - 400) * metallic);
        const specStrength = (0.5 + metallic * 0.5) * (1.0 - roughness * 0.9);
        // P1-10: Math.pow 保留 — specPower 由材质 Rough/Metal 任意组合出连续指数,
        // 2 的幂指数用连乘逼近与 V8 的 (近)正确舍入 pow 不可证逐位一致, LUT 弃用
        let light0 = 0, light1 = 0, light2 = 0, spec0 = 0, spec1 = 0, spec2 = 0;
        const lights = this.lights;
        for (let li = 0; li < Math.min(lights.length, 4); li++) {
          const L = lights[li];
          const lvx = L.origin[0] - wp[0], lvy = L.origin[1] - wp[1], lvz = L.origin[2] - wp[2];
          const dist = Math.sqrt(lvx * lvx + lvy * lvy + lvz * lvz) || 1;
          const ldx = lvx / dist, ldy = lvy / dist, ldz = lvz / dist;
          const attn = sat((L.radius - dist) / L.radius);
          const hx = viewX + ldx, hy = viewY + ldy, hz = viewZ + ldz;
          const hlLen = Math.sqrt(hx * hx + hy * hy + hz * hz) || 1;
          const hnx = hx / hlLen, hny = hy / hlLen, hnz = hz / hlLen;
          const specDot = Math.max(0, hnx * nX + hny * nY + hnz * nZ);
          const c0 = L.color[0] * L.intensity, c1 = L.color[1] * L.intensity, c2 = L.color[2] * L.intensity;
          const st = Math.pow(specDot, specPower) * specStrength * attn;
          spec0 += st * c0; spec1 += st * c1; spec2 += st * c2;
          const lightDot = ldx * nX + ldy * nY + ldz * nZ;
          const hl = lightDot * 0.5 + 0.5;
          const ld = lightDot + (hl - lightDot) * gLight;
          const rim = metallic * 2;
          const rimTerm = Math.pow(Math.max(0, 1 - Math.max(0, nX * viewX + nY * viewY + nZ * viewZ)) * Math.pow(hl, 0.25), 6 - rim) * rim;
          const a2 = attn * attn;
          const dl = sat(ld) + rimTerm;
          light0 += c0 * dl * a2; light1 += c1 * dl * a2; light2 += c2 * dl * a2;
        }
        ...
        if (combos && (combos.LIGHTMAP || combos.lightmap)) {
          ...
          if (lmTex) {
            ...
            // P0-5: 光照贴图写入式采样
            const lm = this._texSampleInto(lmTex, lu, lv, false, _smC);
            light0 *= lm[0]; light1 *= lm[1]; light2 *= lm[2];
            spec0 *= lm[0]; spec1 *= lm[1]; spec2 *= lm[2];
          }
        }
        // ambient (v_LightAmbientColor) + skylight (按法线·up 混合) + albedo*light + specular
        const upMix = (nX * 0 + nY * 1 + nZ * 0) * 0.5 + 0.5;
        const amb0 = this.skylightColor[0] + (this.ambientColor[0] - this.skylightColor[0]) * upMix;
        const amb1 = this.skylightColor[1] + (this.ambientColor[1] - this.skylightColor[1]) * upMix;
        const amb2 = this.skylightColor[2] + (this.ambientColor[2] - this.skylightColor[2]) * upMix;
        light0 += amb0; light1 += amb1; light2 += amb2;
        let r = albedo[0] * light0 + spec0;
        let g = albedo[1] * light1 + spec1;
        let b = albedo[2] * light2 + spec2;
        ...
        if (combos && (combos.REFLECTION || combos.reflection) && textures[3]) {
          ...
          // P0-5: 反射采样写入式 (独立 scratch — albedo 本像素仍要读 [3])
          const ref = this._texSampleInto(textures[3], suv + nX * 0.01, svv + nY * 0.01, false, _smD);
          r += ref[0] * 0.35;
          g += ref[1] * 0.35;
          b += ref[2] * 0.35;
        }
...
,
    // P0-5: _texSample 的写入式 (零分配) 变体 — 数值语义与 _texSample 逐位一致
    // (同 clamp/wrap 分支、同双线性表达式、同 /255; null→白、非有限→0 的契约
    // 也相同), 结果直写 out[0..3]。热路径内层循环用, 消除每采样 4 元素数组分配。
...
,
    // P0-5: RG 双通道写入式采样器 (shake.js 流向图 RG 编码等): 只算 R/G 两通道,
    // 数值语义与 _texSample(...)[0]/[1] 逐位一致 (同 wrap/clamp、同插值表达式)。
