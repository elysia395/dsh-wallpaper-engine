...
class BitReader {
  ...
  // P1-26: 旧实现只清位缓冲不前进 pos — read() 检测到 RST 时已把 pos 回退到
  // 标记起点, consumeRestart 再不前进 → 后续 read 反复撞同一标记, huffDecode
  // 恒返回 -1, AC 循环 k += (r=-1) 后 k++ 原地踏步 → 死循环 (卡死)。
  // 修复: 前进 pos 越过 FF Dn 两字节 (restartPending 已记录标记号)。
  consumeRestart() {
    if (this.restartPending != null) {
      this.pos += 2; // 跳过 FF D0-D7 标记本身
      this.restartPending = undefined;
    } else if (this.b[this.pos] === 0xff && this.b[this.pos + 1] >= 0xd0 && this.b[this.pos + 1] <= 0xd7) {
      this.pos += 2; // 容错: 未挂起但当前位置恰是 RST 也跳过
    }
    this.bitCnt = 0;
    this.bitBuf = 0;
  }
...
function decodeScan(reader, planes, comps, sosComp, huffDC, huffAC, quantTables, block, prevDC, mcusX, mcusY) {
  let invalidCodes = 0;
  const invalid = (v) => v === -2;
  // F-1: JPEG 规范 — 每个 RST 标记处 DC 预测器归零 (重启后首块 DC 为绝对值,
  // 不再相对重启前的 prevDC 差分)。旧实现只在整扫描开始初始化一次 prevDC,
  // 即使 RST 标记被正确跳过, 重启后 DC 仍持续漂移。
  const restart = () => {
    reader.consumeRestart();
    for (let i = 0; i < prevDC.length; i++) prevDC[i] = 0;
  };
  for (let mcuY = 0; mcuY < mcusY; mcuY++) {
    for (let mcuX = 0; mcuX < mcusX; mcuX++) {
      if (reader.atRestart()) restart();
      for (let ci = 0; ci < comps.length; ci++) {
        ...
        for (let by = 0; by < comp.v; by++) {
          for (let bx = 0; bx < comp.h; bx++) {
            ...
            if (s === -1) { restart(); s = huffDecode(reader, huffD); }
            ...
            let k = 1;
            while (k < 64) {
              // P1-26 附带: rs 需可重赋值 (重启后重试读码) — 旧实现 const rs
              // 在 rs===-1 重赋值处抛 TypeError (被外层吞 → 整图黑, 重启路径从未活过)
              let rs = huffDecode(reader, huffA);
              if (invalid(rs)) invalidCodes++;
              if (rs === -1) { restart(); rs = huffDecode(reader, huffA); }
...
function decodeJpegInner(bytes) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      // P1-27: 灰度图 (单分量) 无 Cb 平面 — 与 Cr 的守卫对称, 否则
      // planes[1].data 为 undefined → TypeError → 整张灰度纹理被丢弃
      const Cb = planes.length > 1 ? getPlane(1, x, y) + 128 : 128;
