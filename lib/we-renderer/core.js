...
export class SceneRenderer {
  ...
  renderPuppet(o, model, tr, t) {
    ...
    // 骨骼蒙皮 (动画) 或绑定姿态 — 不用 cropoffset: lwe 无 cropoffset 处理,
    // MDL 网格顶点本身是相对对象中心的局部坐标 (raw bbox 对称, 中心=原点),
    // 加 cropoffset 会把网格整体偏移 (Amiya 头下移 95px 被大衣遮住)
    let skinned = mesh.positions;
    if (mesh.bones && mesh.bones.length && mesh.animations && mesh.animations.length) {
      skinned = this._skinPuppet(mesh, t, 0, 0);
    } else {
