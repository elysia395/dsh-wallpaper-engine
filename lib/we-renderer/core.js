...
// 内嵌脚本执行已按决策移除 (scene-scripts.js 不再存在)。这里只保留
// sceneHasScripts 谓词的本地实现 — W0 降级披露 ("含脚本未执行") 仍需要它。
const sceneHasScripts = (scene) => {
  let found = false;
  const walk = (obj) => {
...
export class SceneRenderer {
  constructor(pkgPath, opts = {}) {
    ...
    // NSL 脚本运行时已随 scene-scripts.js 移除 (W0 决策: 完全不执行壁纸
    // 内嵌脚本)。runScripts 恒为 false — 保留字段以维持 worker/宿主契约
    // (opts.runScripts 被忽略); 含脚本壁纸发一次 scene-script degraded。
  ...
  render() {
    ...
    this._setupCamera();
    // scene scripts: 执行链已移除 (W0 决策恒不执行)。备份/恢复保留为无害
    // 双保险 (脚本从未改值, restore 是 no-op), degraded 披露保留:
    if (this._hasScripts && !this._scriptNoticeSent) {
      this._scriptNoticeSent = true;
      this._degraded(null, 'scene-script', '壁纸含内嵌脚本，未执行（静态渲染不支持脚本）');
    }
