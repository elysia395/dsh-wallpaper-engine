...
import { extractTexVideoMp4, readGlobalAsset } from '../pkg-extract.js';
import { parseCffFont, renderText } from '../font-render.js';
import { applySceneScripts, createScriptCache, sceneHasScripts } from '../scene-scripts.js';
import { getBuiltinTexture } from './util-textures.js';
import {
...
// P0-5: 效果/bloom 整帧 scratch 池 — 帧首召回上一帧借出缓冲 (见 effects/_scratch.js)
import { scratchRecallAll } from './effects/_scratch.js';
...
export class SceneRenderer {
  constructor(pkgPath, opts = {}) {
    ...
    // W0: runScripts 总开关默认 false (用户拍板: 完全不执行壁纸内嵌脚本) —
    // false 时不建编译缓存、render() 跳过 applySceneScripts; 含脚本壁纸发
    // 一次 scene-script degraded (配置页"内嵌脚本未运行"提醒的数据源)。
    this.runScripts = opts.runScripts === true;
    this._scriptCache = this.runScripts ? createScriptCache() : null;
    this._hasScripts = sceneHasScripts(this.scene);
  ...
  readJsonAny(rel) {
    ...
    // P2-22: 全局 assets 回退也走实例级缓存 (文件不可变; 旧实现每帧逐对象
    // existsSync + readFileSync + JSON.parse)。调用方只读, 缓存原引用安全
    if (!this._globalJsonCache) this._globalJsonCache = new Map();
    if (this._globalJsonCache.has(rel)) return this._globalJsonCache.get(rel);
    const gp = path.join(this.weAssetsDir, 'assets', rel);
    let out = null;
    try {
      if (fs.existsSync(gp)) out = JSON.parse(fs.readFileSync(gp, 'utf8'));
    } catch { /* ignore */ }
    this._globalJsonCache.set(rel, out);
    return out;
  }
  ...
  loadTexture(pathOrName) {
    ...
    // WE 全局素材兜底: pkg 未随包分发的官方全局纹理 (粒子 halo/drop 等),
    // 由 assets 根围栏读提供 (pkg-extract.readGlobalAsset; 用户自备素材)
    let raw = this.pkg.read(texPath);
    if (!raw) {
      const g = readGlobalAsset(texPath);
      if (g) raw = Buffer.from(g.bytes.buffer, g.bytes.byteOffset, g.bytes.byteLength);
    }
    let img = null;
    if (raw) {
      try {
    ...
    // W1: 内置 util 纹理兜底 (pkg / weAssetsDir / assetDir 三 miss 后) —
    // 官方语义 = shader 元注释 default 引用的 util/* 全局纹理随 WE 应用分发,
    // 本机无 WE 安装时不可用 → 程序化生成 (util-textures.js, 确定性单例)。
    // 修复 foliagesway 等效果因 util/noise 缺失被整体跳过 (4 张本地壁纸)。
    if (!img) {
      img = getBuiltinTexture(texPath);
    }
    this.textureCache.set(texPath, img);
    return img;
  }
  ...
  _puppetBoneFinal(mesh, t, layers = null) {
    ...
    // N-14: bind 链 (resolveBind 递归 + bindRT) 按 mesh 记忆 — 全部由 MDLS 静态
    // 骨骼矩阵推导, 与 t/layers 无关 (旧实现每次调用即每个 attachment 子对象
    // 每帧重算全套)。注: 父链不可解析的 onDegraded 事件相应只发一次 (静态
    // 条件, 像素输出不受影响)
    if (!mesh._pfBindRT) {
      ...
      mesh._pfBindRT = bindWorld.map((m) => ({ angle: Math.atan2(m[1], m[0]), tx: m[12], ty: m[13] }));
    }
    const bindRT = mesh._pfBindRT;
  ...
  _attachmentOffset(child, parent) {
    ...
    if (parentModel && parentModel.puppet) {
      ...
      if (mesh && mesh.bones && anch.boneIdx < mesh.bones.length) {
        // N-14: 层解析复用 puppet.js 单份实现 (MOD-07 收尾, 语义与 renderPuppet
        // 严格一致 — 仅当 多动画 + 有 animationlayers 时做层合成; 单动画时
        // layers=null → _puppetBoneFinal 用默认动画0 = 父网格蒙皮同款, 否则锚点
        // 跟随错误动画 → 子对象挂载错位), 且走帧内缓存 (findIndex+正则不再每
        // 子对象每帧重跑)。骨骼全套位姿按 (mesh, layers, t) 帧内缓存 — 同一父
        // puppet 的多个 attachment 子对象共享一次计算
        const layers = this._resolveAnimLayers(parent, mesh);
        let bf = this._boneFinalCache && this._boneFinalCache.get(mesh);
        if (!bf || bf.t !== this.time || bf.layers !== layers) {
          bf = { t: this.time, layers, final: this._puppetBoneFinal(mesh, this.time, layers) };
          if (!this._boneFinalCache) this._boneFinalCache = new Map();
          this._boneFinalCache.set(mesh, bf);
        }
        const final = bf.final;
        if (final) {
  ...
  // N-13: 对象 origin/scale/angles 的 parseVec3 结果按对象记忆 (帧内) —
  // 同帧内多个子对象共享祖先链时, 公共祖先只解析一次 (旧实现每对象每帧沿
  // 父链逐祖先重解析)。缓存键覆盖 scene scripts 写回场景: 缓存每帧在
  // render() 内动画烘焙 + 脚本执行完成后清空 (见 render 内失效点), 不跨帧
  _tfVecs(o) {
    if (!this._tfParseCache) this._tfParseCache = new Map();
    let e = this._tfParseCache.get(o);
    if (!e) {
      e = {
        origin: parseVec3(getVal(o, 'origin'), [0, 0, 0]),
        scale: parseVec3(getVal(o, 'scale'), [1, 1, 1]),
        angles: parseVec3(getVal(o, 'angles'), [0, 0, 0]),
      };
      this._tfParseCache.set(o, e);
    }
    return e;
  }
  ...
  resolveTransform(o) {
    ...
    // N-13: 祖先链解析走帧内缓存 (解析式与旧实现逐字相同)
    let rp = this._tfVecs(root);
    let origin = rp.origin, scale = rp.scale, ang = rp.angles;
    // 从根向下: 子 origin × 当前累积 scale → 旋转(当前累积 Z 角) → + 当前 origin
    for (let i = chain.length - 2; i >= 0; i--) {
      const cp = this._tfVecs(chain[i]);
      const co = cp.origin, ca = cp.angles;
  ...
  _isVisibleSelf(o) {
    ...
    if (typeof v === 'object' && v !== null && 'user' in v) {
      const user = v.user;
      // A6: {user:{condition,name},value} 条件绑定 — visible = (属性值 === condition)
      // (3735447194 rover combo: male→泡-中 / female→泡-中-女 互斥)。
      if (typeof user === 'object' && user !== null && user.name != null) {
        if (this.userProps && user.name in this.userProps) {
          return String(this.userProps[user.name]) === String(user.condition);
        }
  ...
  render() {
    // P0-5: 帧首召回效果 scratch 池 — 上一帧效果链借出的整帧缓冲此时已无引用
    // (image.js 对 applyEffects 结果同步 blit 后即丢弃), 可安全复用
    scratchRecallAll();
    const t = this.time;
    ...
    // W0: 脚本总开关 — 默认不执行 (runScripts===true 才进此分支); 含脚本
    // 壁纸在关闭状态下记一次 degraded (对象级配置页提醒 "内嵌脚本未运行")
    if (!this.runScripts && this._hasScripts && !this._scriptNoticeSent) {
      this._scriptNoticeSent = true;
      this._degraded(null, 'scene-script', '壁纸含内嵌脚本，未执行（如需运行请在设置中开启）');
    }
    if (this.runScripts) try {
    ...
    // N-13: 帧内变换解析缓存失效点 — 动画烘焙 (_resolveAnimations) 与 scene
    // scripts 写回 (origin/scale 等可被脚本逐帧改值) 均已完成, 此后到帧尾对象
    // 变换不再变化; 在此清一次, 帧内多对象/多次 resolveTransform 共享祖先解析
    if (this._tfParseCache) this._tfParseCache.clear();
    // N-14: attachment 帧内缓存同款失效点 — animLayers/blend 等可被 scene
    // scripts 写回, 每帧清空; 帧内同父多子对象共享 (见 _attachmentOffset)
    if (this._animLayerCache) this._animLayerCache.clear();
    if (this._boneFinalCache) this._boneFinalCache.clear();
  ...
  // P2-21: 关键帧数组 (filter+sort) 结果按动画对象惰性缓存 (WeakMap) —
  // animation 原始对象经 _animBackup 每帧恢复为同一引用 (内容不可变),
  // 排序结果可跨帧复用; 旧实现 evalChannel 每次调用 (每对象每通道每帧) 重跑
  _kfFrames(a, ch) {
    if (!this._kfCache) this._kfCache = new WeakMap();
    let m = this._kfCache.get(a);
    if (!m) { m = {}; this._kfCache.set(a, m); }
    if (!(ch in m)) {
  ...
  _resolveAnimations(t) {
    ...
    for (const [o, key, v] of this._animBackup) {
      ...
      const evalChannel = (ch) => {
        // P2-21: filter+sort 走动画对象级缓存 (见 _kfFrames), 求值逻辑不变
        const frames = this._kfFrames(a, ch);
  ...
  //
  // P1-12: 结果缓存 — 纹理对象挂 _dsCache = Map<量化 maxSize, img>。maxSize 量化
  // 到最近的 2 的幂 (scale/size 动画时 dw/dh 逐帧连续变化, 不量化则每帧 miss;
  // 键为 2 的幂 → 条目数 ≤ log2(纹理最大边), 有界)。取最近值而非 floor: 降采样
  // 目标的设计语义是「显示尺寸」(见 image.js 效果链注释), 最近幂偏差对称
  // (0.71×~1.41× 线性); floor 会在默认 960 档把内容压到 0.53× 显示尺寸, 超出
  // anim 近似的「分辨率略降」范围)。
  // 缓存安全性依据 (读代码确认): 调用点均在 image.js (普通效果链 + _passthroughFx),
  // 全部处于 anim 帧 (staticFrame=false) 门内, 输入只有两类 — ① loadTexture→
  // textureCache 的静态源纹理: decode 后在渲染器实例整个生命周期内不变, 效果链
  // 只读不写 src (效果均写新缓冲), 可安全跨帧缓存; ② spritesheet 逐帧裁剪 /
  // passthrough 画布快照等每帧新建的临时对象: 缓存随对象即弃, 永不跨帧命中,
  // 无陈旧风险。动态内容 (_rt_ 画布快照) 不会以持久纹理对象进入本函数。
  // 静态帧不调用本函数 → 缓存与量化均不影响静态帧输出。
  _downsample(tex, maxSize) {
    const q = Math.max(1, 1 << Math.round(Math.log2(maxSize)));
    if (!tex._dsCache) tex._dsCache = new Map();
    else if (tex._dsCache.has(q)) return tex._dsCache.get(q);
    const w = tex.width, h = tex.height;
    const scale = Math.min(1, q / Math.max(w, h));
    ...
    const img = { width: tw, height: th, rgba: out };
    tex._dsCache.set(q, img);
    return img;
  }
