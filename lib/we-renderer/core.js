...
export class SceneRenderer {
  ...
  // 官方视图平移 T(-eye): 前景对象经 view 矩阵平移 (wallpaper64 0x1401EC720 压栈
  // 0x1160/0x11a0 = view/proj 缓存; 0x304 的 0x1100 标志区分背景跳过视图)。
  // Amiya: eye=(-360,-269.56) → 前景平移 x=-eye.x=+360 场景单位 × ps。
  // x 取 -eye.x (用户初始"头相对背景偏移 182px@1920 = 360×0.5"证实);
  // y 不平移 (用户实测: 栏杆/花束上移异常 → 官方 y 无视图平移)。
  // 背景判定: size 接近场景正交尺寸 (314 背景 = 3840×2160 全屏)。
  ...
  renderImage(o, t) {
    ...
    const oy = (ps ? tr.origin[1] * ps[1] : tr.origin[1]) + vs[1];
    const sc = tr.scale;
  ...
  renderPuppet(o, model, tr, t) {
    ...
    const dx = (tr.origin[0] + rawBounds.minX) * (ps ? ps[0] : 1) + vs[0];
    const dy = this.H - (tr.origin[1] + rawBounds.maxY) * (ps ? ps[1] : 1) + vs[1];
