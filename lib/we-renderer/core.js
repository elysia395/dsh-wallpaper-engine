...
export class SceneRenderer {
  ...
  renderPuppet(o, model, tr, t) {
    ...
    // cropoffset: 模型网格偏移 (官方 WE puppet 字段)。应用规则: crop 后网格与
    // size 矩形有交集才应用 (网格内容部分留在 size 内), 完全移出则回退 —
    // Amiya 头/大衣 (y负大值) 应用后移出遮罩; 若叶睦眼/嘴 (y正大值) 完全移出
    // size → 回退, 保持面部在头内 (lwe 无此字段, 语义从多壁纸反推)
    const crop = (model.cropoffset || '0 0').trim().split(/\s+/).map(Number);
    let cxs = 0, cys = 0;
    if (crop[0] || crop[1]) {
      const bb = this._meshBounds(mesh.positions);
      const sz = parseVec2(getVal(o, 'size'), [0, 0]);
      if (sz[0] > 0 && sz[1] > 0) {
        const hw = sz[0] / 2, hh = sz[1] / 2;
        const ix = Math.min(bb.maxX + crop[0], hw) - Math.max(bb.minX + crop[0], -hw);
        const iy = Math.min(bb.maxY + crop[1], hh) - Math.max(bb.minY + crop[1], -hh);
        if (ix > 0 && iy > 0) { cxs = crop[0]; cys = crop[1]; }
      } else { cxs = crop[0]; cys = crop[1]; }
    }
    // 骨骼蒙皮 (动画) 或绑定姿态
    let skinned = mesh.positions;
    if (mesh.bones && mesh.bones.length && mesh.animations && mesh.animations.length) {
      skinned = this._skinPuppet(mesh, t, cxs, cys);
    } else {
      skinned = mesh.positions.map((p) => [p[0] + cxs, p[1] + cys, p[2]]);
    }
    ...
    const dx = (tr.origin[0] + rawBounds.minX) * (ps ? ps[0] : 1);
    const dy = this.H - (tr.origin[1] + rawBounds.maxY) * (ps ? ps[1] : 1);
    // FBO 裁剪 (lwe CImage.cpp:528-546): puppet 网格渲染到 FBO(size), 超出 size
    // 矩形的部分裁剪 — 网格 + cropoffset 后可能超出, 只显示 size 矩形内
    let clipRect = null;
    let size = parseVec2(getVal(o, 'size'), [0, 0]);
    if ((size[0] === 0 || size[1] === 0) && tex) size = [tex.width, tex.height];
    if (size[0] > 0 && size[1] > 0) {
      const sw = size[0] * tr.scale[0] * (ps ? ps[0] : 1), sh = size[1] * tr.scale[1] * (ps ? ps[1] : 1);
      const sx = tr.origin[0] * (ps ? ps[0] : 1) - sw / 2;
      const sy = this.H - tr.origin[1] * (ps ? ps[1] : 1) - sh / 2;
      clipRect = [sx, sy, sw, sh];
    }
    this.canvas.blitScaled(img, dx, dy, dw, dh, getVal(o, 'alpha', 1), clipRect);
  }

  // 骨骼蒙皮: 时间 → 动画帧 → 骨骼世界矩阵 → 顶点 × Σ w × (animWorld × bindWorld⁻¹)
  // 引擎 model_vertex_v1.h ApplySkinningPosition: pos' = Σ w·(pos × g_Bones[bi])
  _skinPuppet(mesh, t, cxs, cys) {
    ...
    for (let b = 0; b < nb; b++) {
      ...
      if (isFinite(px) && isFinite(py) && Math.abs(px) < 10000 && Math.abs(py) < 10000 && isFinite(rotZ)) {
        ...
        // [Rz | T]: [c, s, 0, 0, -s, c, 0, 0, 0, 0, 1, 0, px, py, 0, 1]
      ...
      if (mdla >= 0) {
        try {
          ...
          for (let a = 0; a < animCount && p + 12 < buf.length; a++) {
            ...
            animations.push({ frameCount, boneCount, segBytes, segs });
