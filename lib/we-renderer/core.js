...
export class SceneRenderer {
  ...
  resolveTransform(o) {
    ...
    // 官方 puppet 定位 (wallpaper64 逆向): 根 puppet (有其他对象挂载其下) 的
    // origin 对齐网格底部 (网格在挂载点上方展开) 且整体右移网格宽 (一个头)。
    // 子对象 (经父链含根 puppet) 跟随。Amiya 头697: y+291 x+449 → 头-身体重叠
    // 291px→0, 头部链右移一个头宽。偏移表在 render() 前预计算 (_buildRootPuppetShifts)。
    if (this._rootPuppetShifts && this._rootPuppetShifts.size) {
      for (const chainObj of chain) {
        const s = this._rootPuppetShifts.get(chainObj.id);
        if (s) { origin[1] += s.shiftY; origin[0] += s.shiftX; break; }
      }
    }
    return { origin, scale, angle: ang[2], angles: ang };
  }

    // 官方根 puppet 底部对齐偏移表: 遍历对象, 找"有子对象挂载的 puppet"
    // (如 Amiya 头697 挂 五官463/494), 解析其 MDL 网格 (绑定姿态 raw),
    // shiftY = -minY + 1 (网格底部对齐 origin), shiftX = 网格宽 (maxX-minX,
    // 头部整体右移一个头宽 — 用户实测 Amiya 头需右移约一个头, 用网格宽
    // 作为确定性量)。resolveTransform 对父链含该 puppet 的对象应用
    // (头部链整体上移+右移, 身体链不动)。
    _buildRootPuppetShifts() {
      if (this._rootPuppetShiftsBuilt) return;
      this._rootPuppetShiftsBuilt = true;
      this._rootPuppetShifts = new Map();
      if (!this.objects) return;
      for (const o of this.objects) {
        if (!o || typeof o.image !== 'string') continue;
        // 根 puppet: 有子对象挂载, 且所有子对象都是空锚点 (无 image) —
        // 如 Amiya 头697 的子 463(头发)/494(五官) 均无 image; 而 467(鼻子)
        // 的子 697/528 含 image 对象 → 467 是身体锚点, 不底部对齐。
        const children = this.objects.filter((oo) => oo && oo.parent === o.id);
        if (!children.length) continue;
        if (children.some((c) => typeof c.image === 'string')) continue;
        let model = null;
        try { model = this.readJsonAny(o.image); } catch { continue; }
        ...
        try {
          ...
          if (isFinite(minY) && isFinite(maxX)) {
            this._rootPuppetShifts.set(o.id, { shiftY: -minY + 1, shiftX: maxX - minX });
          }
        } catch { /* MDL 解析失败则跳过 */ }
      }
    }
  ...
  render() {
    ...
    // 属性动画 {animation: {c0: 关键帧}} → 按 t 烘焙为静态值
    try {
  ...
  _resolveAnimations(t) {
    const animKeys = ['alpha', 'scale', 'origin', 'angles', 'visible', 'color', 'size', 'brightness', 'parallaxDepth'];
    if (!this._animBackup) {
      this._animBackup = [];
      for (const o of this.objects) {
    ...
    for (const [o, key, v] of this._animBackup) {
      const a = v.animation;
      const frames = (a.c0 || []).filter((f) => f && typeof f.frame === 'number' && f.value != null);
      ...
      if (frame <= frames[0].frame) value = frames[0].value;
        ...
        for (let i = 0; i < frames.length - 1; i++) {
          ...
          if (frame >= a0.frame && frame <= a1.frame) {
            const span = a1.frame - a0.frame || 1;
            const k = (frame - a0.frame) / span;
            value = this._lerpValue(a0.value, a1.value, k);
            break;
          }
        }
  ...
  renderImage(o, t) {
    ...
    // 官方渲染链 (wallpaper64 逆向): 视图平移 (-eye) 统一应用于所有非背景对象
    // (puppet 与 image 一致, 保证组件间相对位置不变 — sf22 曾只对 puppet 应用
    // 导致 403左大衣(puppet) 相对 407身体/手臂/栏杆(image) 错位, 已回退)。
    // 背景对象 (size==场景尺寸) 不经过视图 (全屏直映)。
    let ox0 = tr.origin[0], oy0 = tr.origin[1];
    const ow = ortho && ortho.width ? ortho.width : this.W;
    const oh = ortho && ortho.height ? ortho.height : this.H;
    const isSceneBg = Math.abs(size[0] - ow) < 1 && Math.abs(size[1] - oh) < 1;
    if (!isSceneBg && this.camEye && (this.camEye[0] !== 0 || this.camEye[1] !== 0)) {
      ox0 -= this.camEye[0];
      oy0 -= this.camEye[1];
    }
    const ox = ps ? ox0 * ps[0] : ox0;
    const oy = ps ? oy0 * ps[1] : oy0;
    const sc = tr.scale;
    // CImage 坐标: 像素左上角 = (origin.x - dw/2, H - origin.y - dh/2), y 向下
    const dw = size[0] * sc[0] * (ps ? ps[0] : 1), dh = size[1] * sc[1] * (ps ? ps[1] : 1);
    let dx = ox - dw / 2, dy = this.H - oy - dh / 2;
  ...
  _renderSolidLayer(o, model, tr, t) {
    ...
    const ortho = this.scene.general && this.scene.general.orthogonalprojection;
    const ps = ortho && ortho.width ? [this.W / ortho.width, this.H / (ortho.height || 1080)] : null;
    // 视图平移 (-eye) 统一应用于非背景对象 (同 renderImage)
    let ox0 = tr.origin[0], oy0 = tr.origin[1];
    const ow = ortho && ortho.width ? ortho.width : this.W;
    const oh = ortho && ortho.height ? ortho.height : this.H;
    const isSceneBg = Math.abs(size[0] - ow) < 1 && Math.abs(size[1] - oh) < 1;
    ...
    if (img && tr.angle !== 0) {
      ...
      this.canvas.blitScaled(img, dx, dy, dw, dh, alpha * brightness);
    }
  }
  ...
  // flag (eagleflag): 法线贴图双采样扰动 + cloth + TINT + 光照 (引擎 flag.frag)
  ...
  renderTextObject(o, t) {
    ...
    if (fontPath) {
      try {
        ...
        // 视图平移 (-eye) 统一应用于非背景对象 (同 renderImage)
        let tox = tr.origin[0], toy = tr.origin[1];
        if (this.camEye && (this.camEye[0] !== 0 || this.camEye[1] !== 0)) {
          tox -= this.camEye[0];
          toy -= this.camEye[1];
        }
        const ox = tox * ps[0], oy = this.H - toy * ps[1];
  ...
  renderPuppet(o, model, tr, t) {
    ...
    if (mesh.animations && mesh.animations.length > 1 && o.animationlayers && o.animationlayers.length) {
      ...
      if (layers.length) {
        ...
        if (idx < 0) {
          const layerIdx = o.animationlayers.indexOf(layer);
    ...
    // 渲染 = origin + raw, 且需乘正交比例 ps (renderImage 同语义) — 修复前缺 ps 导致
    // workshop 场景 (3840→480) puppet 组件错 8 倍
    ...
    // 官方视图平移: puppet 同 2D 图像, 非背景对象场景坐标整体 -eye
    let pox = tr.origin[0], poy = tr.origin[1];
    const szp = parseVec2(getVal(o, 'size'), [0, 0]);
    const owp = orthoP && orthoP.width ? orthoP.width : this.W;
    const ohp = orthoP && orthoP.height ? orthoP.height : this.H;
    const isBgP = Math.abs(szp[0] - owp) < 1 && Math.abs(szp[1] - ohp) < 1;
    if (!isBgP && this.camEye && (this.camEye[0] !== 0 || this.camEye[1] !== 0)) {
      pox -= this.camEye[0];
      poy -= this.camEye[1];
    }
    // 官方根 puppet 网格 x 修正 (用户实测: 头相对发/耳/眼右移约一个眼睛宽度,
    // 需左移 65px @3840 场景)。仅渲染层偏移 — 不改 origin (发/耳/眼挂载点
    // 不变, 保持正确位置)。判定与 _buildRootPuppetShifts 一致: 有子对象且
    // 子对象全为空锚点 (Amiya 头697 挂 463/494)。
    const isRootPuppet = (() => {
      if (!this.objects) return false;
      const children = this.objects.filter((oo) => oo && oo.parent === o.id);
      if (!children.length) return false;
      return !children.some((c) => typeof c.image === 'string');
    })();
    if (isRootPuppet) {
      pox -= 65;
    }
    const dw = W * (ps ? ps[0] : 1) * tr.scale[0], dh = H * (ps ? ps[1] : 1) * tr.scale[1];
    const dx = (pox + rawBounds.minX) * (ps ? ps[0] : 1);
    const dy = this.H - (poy + rawBounds.maxY) * (ps ? ps[1] : 1);
    this.canvas.blitScaled(img, dx, dy, dw, dh, getVal(o, 'alpha', 1));
  }

  // 骨骼蒙皮: 时间 → 动画帧 → 骨骼世界矩阵 → 顶点 × Σ w × (animWorld × bindWorld⁻¹)
  // 引擎 model_vertex_v1.h ApplySkinningPosition: pos' = Σ w·(pos × g_Bones[bi])
  _skinPuppet(mesh, t, cxs, cys, animIdx = 0, blend = 1) {
    const bones = mesh.bones;
    const anim = mesh.animations[animIdx] || mesh.animations[0];
    if (!anim) return mesh.positions.map((p) => [p[0] + cxs, p[1] + cys, p[2]]);
    ...
    // 帧: 30fps 循环; 动画有效帧 = frameCount (段含 frameCount+1 块, 末块为归零/填充)
    const fps = 30;
    const frame = Math.floor(t * fps) % Math.max(1, anim.frameCount);
    const segBytes = anim.segBytes;
    ...
    // 动画局部矩阵: T(pos.xy) × Rz(rot.z) (2D 骨骼)
    // MDLA 段布局 (逆向自 Hina 32骨骼/普拉娜 6骨骼, 9 列循环交错):
    ...
    for (let b = 0; b < nb; b++) {
      ...
      // pos 合理性校验 (有限 + 量级 < 10000), 异常则用绑定矩阵 (绑定姿态, 不炸)
      let local;
      if (isFinite(px) && isFinite(py) && Math.abs(px) < 10000 && Math.abs(py) < 10000 && isFinite(rotZ)) {
        // blend 混合: 动画层 blend (0..1) — 骨骼 pos/rot 在绑定姿态与动画间插值
        // (官方动画层系统: 层 blend 加权, Amiya 头层2 blend=0.74)
        let mpx = px, mpy = py, mrot = rotZ;
        if (blend < 1) {
          const bm = bones[b].bind;
          mpx = bm[12] + (px - bm[12]) * blend;
          mpy = bm[13] + (py - bm[13]) * blend;
          const bindRot = Math.atan2(bm[1], bm[0]);
          mrot = bindRot + (rotZ - bindRot) * blend;
        }
        const c = Math.cos(mrot), s = Math.sin(mrot);
        // bind 矩阵为列主序存储 Rz(+θ) (bind[1]=+sin): animLocal 同构
        // [Rz | T]: [c, s, 0, 0, -s, c, 0, 0, 0, 0, 1, 0, mpx, mpy, 0, 1]
        local = [c, s, 0, 0, -s, c, 0, 0, 0, 0, 1, 0, mpx, mpy, 0, 1];
      } else {
        local = bones[b].bind; // fallback 绑定姿态
      }
      animLocal[b] = local;
      const parent = bones[b].parent;
      animWorld[b] = parent >= 0 && parent < nb && animWorld[parent] ? this._matMulRow(animWorld[parent], local) : local;
    }
    // 蒙皮矩阵: g_Bones[b] = animWorld[b] × bindInv[b]
    const gBones = new Array(nb);
    for (let b = 0; b < nb; b++) gBones[b] = this._matMulRow(animWorld[b], bindInv[b]);
    // 顶点蒙皮
