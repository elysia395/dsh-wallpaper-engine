...
export class SceneRenderer {
  ...
  renderImage(o, t) {
    ...
    // 画布 y 向下: top = 上移 dh/2, bottom = 下移 dh/2, left = 左移 dw/2, right = 右移 dw/2
    const align = String(getVal(o, 'alignment', '')).toLowerCase();
    if (align.includes('top')) dy -= dh / 2;
    else if (align.includes('bottom')) dy += dh / 2;
    // CImage.cpp:250-256: left → m_pos.x += size.x/2 (对象右移), right → 左移
  ...
  _skinPuppet(mesh, t, cxs, cys) {
    ...
    // 帧: 30fps 循环 (帧率未定, 用引擎常见值; [u32] 字段非时长)
    const fps = 30;
    ...
    // 动画局部矩阵: T(pos.xy) × Rz(rot.z) (2D 骨骼; rot.z = 段块 col[6+boneIdx])
    ...
    for (let b = 0; b < nb; b++) {
      const segStart = anim.segs[b] + frame * 36;
      const px = dv.getFloat32(segStart + b * 2 * 4, true);
      const py = dv.getFloat32(segStart + b * 2 * 4 + 4, true);
      // 动画段块布局: [2×boneIdx] 只在少骨骼模型验证过; 多骨骼 (≥5) 时越界读到垃圾
      // → pos 合理性校验 (有限 + 量级 < 10000), 异常则用绑定矩阵 (绑定姿态, 不炸)
      let local;
      if (isFinite(px) && isFinite(py) && Math.abs(px) < 10000 && Math.abs(py) < 10000) {
        // rot.z: 暂用绑定值 0 — 段块 col[6+boneIdx] 含占位 (需更多样例)
        const rotZ = 0;
