...
export class SceneRenderer {
  ...
  resolveTransform(o) {
    ...
    // origin 对齐网格底部 (网格在挂载点上方展开)。子对象 (经父链含根 puppet)
    // 跟随上移。Amiya 头697: raw 底部 290 → 头部链整体上移 291 → 头-身体重叠
    // 291px→0, 身体链(528) 不动。偏移表在 render() 前预计算 (_buildRootPuppetShifts)。
  ...
  // (如 Amiya 头697 挂 五官463/494), 解析其 MDL 网格底部 (绑定姿态 raw minY),
  // shift = -minY + 1 (网格底部对齐 origin)。resolveTransform 对父链含该
  // puppet 的对象应用 (头部链整体上移, 身体链不动)。
  _buildRootPuppetShifts() {
    ...
    for (const o of this.objects) {
      ...
      try {
        ...
        let minY = Infinity;
        for (const p of mesh.positions) if (p[1] < minY) minY = p[1];
        if (isFinite(minY)) this._rootPuppetShifts.set(o.id, -minY + 1);
      } catch { /* MDL 解析失败则跳过 */ }
    }
  }
