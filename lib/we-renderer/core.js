...
export class SceneRenderer {
  ...
  _skinPuppet(mesh, t, cxs, cys) {
    ...
    for (let b = 0; b < nb; b++) {
      ...
      bindWorld[b] = parent >= 0 ? this._matMulRow(bindWorld[parent], local) : local;
    }
    ...
    for (let b = 0; b < nb; b++) {
      ...
      // rot.z: 暂用绑定值 0 — 段块 col[6+boneIdx] 含 bone0 的占位 1 (非旋转),
      // bone1 头动画的 col7 平滑 -0.2rad 确认为 rot.z 但布局未完全解明 (需更多样例)
      const rotZ = 0;
      const c = Math.cos(rotZ), s = Math.sin(rotZ);
      // 行主序: [Rz | T]
      animLocal[b] = [c, -s, 0, 0, s, c, 0, 0, 0, 0, 1, 0, px, py, 0, 1];
      const parent = bones[b].parent;
      animWorld[b] = parent >= 0 ? this._matMulRow(animWorld[parent], animLocal[b]) : animLocal[b];
    }
