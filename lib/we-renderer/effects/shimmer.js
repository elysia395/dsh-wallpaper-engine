    ...
    effectShimmer(tex, c, t, pass) {
        ...
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            let mask = 1;
            if (maskTex) mask = this._texR(maskTex, u, v);
            let off = 0;
            if (offsetTex) off = this._texR(offsetTex, u, v) * timeoffsetScale;
            ...
            this._texSampleInto(gradTex, frac(sx), frac(sy), false, shimmerColor);
            eff[0] = shimmerColor[0] * color[0]; eff[1] = shimmerColor[1] * color[1]; eff[2] = shimmerColor[2] * color[2];
            applyBlendingInto(blendMode, albedo, eff, 1, blended);
            // mix 因子 = mask·shimmerColor·amount (vec3 逐通道)
            const f = mask * amount;
            const di = (y * W + x) * 4;
            out[di] = Math.round((albedo[0] + (blended[0] - albedo[0]) * (shimmerColor[0] * f)) * 255);
            out[di + 1] = Math.round((albedo[1] + (blended[1] - albedo[1]) * (shimmerColor[1] * f)) * 255);
            out[di + 2] = Math.round((albedo[2] + (blended[2] - albedo[2]) * (shimmerColor[2] * f)) * 255);
