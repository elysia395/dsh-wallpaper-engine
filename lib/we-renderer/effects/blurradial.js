...
// P2-26: 核参数为常量 — 提升到模块级 (原每次调用重建 3 个对象)
...
export const fx = {
    ...
    effectBlurradial(tex, c, t, pass) {
        ...
        // P2-26: 每 tap 的 cos/sin(o_i·amt) 是整 pass 常量 (amt 只依赖 scale) —
        // 像素循环外一次预计算 (旧实现每 tap 每像素 2 次 Math 调用 + rotate 数组)
        const taps = K.o.length;
        const tapCos = new Array(taps), tapSin = new Array(taps);
        ...
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            for (let i = 0; i < taps; i++) {
              // P2-26: 表驱动旋转 (cosK/sinK 预计算; r1 = rotate(delta,o)−delta
              // 的原表达式逐位保持)
              const cK = tapCos[i], sK = tapSin[i];
              const r1x = (dx * cK - dy * sK) - dx;
              const r1y = (dx * sK + dy * cK) - dy;
              const w = K.w[i];
              if (K.signed) {
