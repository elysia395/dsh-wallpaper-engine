...
export const fx = {
    ...
    effectWaterripple(tex, c, t, ef, pass) {
        ...
        const aspect = w / h;
        // P0-5: 法线双采样需要并读 → 两块写入式 scratch (法线缺失时保持 0.5/0.5/1 默认)
        const n1 = [0, 0, 0, 0];
        const n2 = [0, 0, 0, 0];
        const s = [0, 0, 0, 0];
        for (let y = 0; y < h; y++) {
          for (let x = 0; x < w; x++) {
            ...
            ry1 *= ratio; ry2 *= ratio;
            if (normalTex) this._texSampleInto(normalTex, rx1, ry1, false, n1);
            else { n1[0] = 0.5; n1[1] = 0.5; n1[2] = 1; }
            if (normalTex) this._texSampleInto(normalTex, rx2, ry2, false, n2);
            else { n2[0] = 0.5; n2[1] = 0.5; n2[2] = 1; }
            ...
            let mfactor = 1;
            if (maskTex) mfactor = this._texR(maskTex, u * mSx, v * mSy);
            const uu = Math.max(0, Math.min(1, u + (nx / nl) * s2 * mfactor));
            const vv = Math.max(0, Math.min(1, v + (ny / nl) * s2 * mfactor));
            // P1-6: 主图位移采样传 clamp — 上面 clamp 到 [0,1] 后 uu=1.0 在
            // _texSample 默认 wrap 下回绕左缘 (§9.6: 主图 CLAMP)
            this._texSampleInto(tex, uu, vv, true, s);
            const di = (y * w + x) * 4;
