...
// WAT-08: mix2 提升到模块级 (旧: 像素循环内逐像素闭包分配)
const mix2 = (a, b, f) => [
    ...
    effectWaterflow(tex, c, t, pass) {
        ...
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            // 对侧回绕镜像内容)
            const s1 = this._texSample(tex, u + o1x, v + o1y, true);
            const s2 = this._texSample(tex, u + o2x, v + o2y, true);
            const s3 = this._texSample(tex, u + o3x, v + o3y, true);
            const s4 = this._texSample(tex, u + o4x, v + o4y, true);
