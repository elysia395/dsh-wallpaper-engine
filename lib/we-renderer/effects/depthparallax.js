        ...
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            if (quality === 0) {
              ...
              let curDepth = depthTex ? this._texR(depthTex, cur[0] * dSx, cur[1] * dSy) : 0;
              let currentLayerDepth = 1;
              let i = 0;
              while (currentLayerDepth > curDepth && i < numLayers) {
                cur = [cur[0] - delta[0], cur[1] - delta[1]];
                curDepth = depthTex ? this._texR(depthTex, cur[0] * dSx, cur[1] * dSy) : 0;
                currentLayerDepth -= layerDepth;
                i++;
              }
              const prev = [cur[0] + delta[0], cur[1] + delta[1]];
              const afterDepth = curDepth - currentLayerDepth;
              const beforeDepth = (depthTex ? this._texR(depthTex, prev[0] * dSx, prev[1] * dSy) : 0) - currentLayerDepth - layerDepth;
