// WE 渲染引擎 — 效果 Lightshafts (从 effects.js 拆分)
// P0-5: 整帧 out 改 scratch 池; 采样改写入式; applyBlending 直写 (数值语义逐位不变)
...
export const fx = {
    ...
    effectLightshafts(tex, c, t, pass) {
        ...
        // P1-14: RAYMODE/RENDERING 仅实现 0/缺省 (线性光柱 + 颜色渐变) 变体。
        // 非默认值 → 记一次 degraded (对象保留)。取舍: 按 MODE=0 近似渲染而非
        // 跳过 — 光柱几何 (point0-3 透视) 与 BLENDMODE 混色仍成立, 跳过会整层
        // 失去光照; 近似与官方的差值只在 falloff 曲线/着色模式
        {
          const wide = (v) => v == null || v === 0 || v === '0';
          const bad = [];
          if (!wide(combos.RAYMODE)) bad.push('RAYMODE=' + combos.RAYMODE);
          if (!wide(combos.RENDERING)) bad.push('RENDERING=' + combos.RENDERING);
          if (bad.length) degradedOnce(this, 'effect:lightshafts:combo', 'lightshafts 未支持 combo ' + bad.join('/') + '，按 RAYMODE=0/RENDERING=0 近似渲染（对象保留）');
        }
        const pt = (pass && pass.textures) || [];
        const noiseTex = pt[1] && pt[1] !== 'null' ? this.loadTexture(pt[1]) : this.loadTexture('util/noise');
        // C4/F-6: 噪声纹理缺失 → _texSample(null)=白 → 噪声双采样恒 1 → 整幅
        // 全强度色块脉冲; 无法确定官方默认噪声 → 跳过该效果 + 记一次 degraded
        if (!noiseTex) {
          degradedOnce(this, 'effect:lightshafts:noise', 'lightshafts 噪声纹理缺失（util/noise 不可用），已跳过该效果（对象保留）');
          return tex;
        }
        const src = tex.rgba;
        ...
        // P0-5: 写入式采样/混合 scratch (双噪声采样先暂存 v1)
        const nsS = [0, 0, 0, 0];
        const a4 = [0, 0, 0, 0];
        const col = [0, 0, 0];
        const rgb = [0, 0, 0];
        for (let y = 0; y < H; y++) {
          for (let x = 0; x < W; x++) {
            ...
            this._texSampleInto(noiseTex, n1x, n1y, false, nsS);
            const nv1 = nsS[0];
            this._texSampleInto(noiseTex, n2x, n2y, false, nsS);
            let fxv = nv1 * nsS[0];
            ...
            // 需显式 clamp 到 [0,1]。
            col[0] = Math.max(0, Math.min(1, cStart[0] + (cEnd[0] - cStart[0]) * fxRefY)) * intensity;
            col[1] = Math.max(0, Math.min(1, cStart[1] + (cEnd[1] - cStart[1]) * fxRefY)) * intensity;
            col[2] = Math.max(0, Math.min(1, cStart[2] + (cEnd[2] - cStart[2]) * fxRefY)) * intensity;
            fxv *= mask;
            this._texSampleInto(tex, u, v, false, a4);
            applyBlendingInto(blendMode, a4, col, fxv, rgb);
            const di = (y * W + x) * 4;
            out[di] = Math.round(rgb[0] * 255);
            out[di + 1] = Math.round(rgb[1] * 255);
            out[di + 2] = Math.round(rgb[2] * 255);
            out[di + 3] = Math.round(Math.max(a4[3], fxv) * 255);
          }
        }
