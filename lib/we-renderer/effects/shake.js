...
// P0-5: 整帧 out 改 scratch 池; 流向图 RG 采样改写入式 _texRG (同数学)
    ...
    effectShake(tex, c, t, pass) {
        ...
        for (let y = 0; y < h; y++) {
          for (let x = 0; x < w; x++) {
            ...
            let fmx = 0, fmy = 0;
            if (flowTex) {
              // P0-5: RG 双通道写入式采样 (只取 R/G 解码方向)
              this._texRG(flowTex, u * mSx, v * mSy, false, fm);
              fmx = (fm[0] - 0.498) * 2;
              fmy = (fm[1] - 0.498) * 2;
            }
            ...
            this._texSampleInto(tex, u + offset * s2 * fmx, v + offset * s2 * fmy, true, s);
            const di = (y * w + x) * 4;
