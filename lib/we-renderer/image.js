...
,
    renderImage(o, t) {
        ...
        if (pass && pass.combos && (pass.combos.spritesheet || pass.combos.SPRITESHEET) && tex.frames && tex.frames.count > 1) {
          const fr = tex.frames;
          // 脚本 thisLayer.getTextureAnimation().setFrame(n) 可锁定帧 (昼夜切换等):
          // 锁定帧时不随时间推进; 暂停时停在当前帧 (见 scene-scripts.js getTextureAnimation)
          let frameIdx;
          if (o._texAnimFrame != null) frameIdx = o._texAnimFrame;
          else if (o._texAnimPaused) frameIdx = o._texAnimLastFrame != null ? o._texAnimLastFrame : 0;
          else frameIdx = Math.floor(t / fr.duration) % fr.count;
          if (o) o._texAnimLastFrame = frameIdx;
        ...
        const sc = tr.scale;
        // 正交 zoom (根因 B): 尺寸与位置都乘/绕场景中心缩放 — 只进 camProj 则
        // 2D 组件在 zoom 动画 (入场放大) 中不变, 位置尺寸全错。
        const Z = ps && this.camIsOrtho ? this._orthoZoomScale() : 1;
        const dw = size[0] * sc[0] * (ps ? ps[0] : 1) * Z, dh = size[1] * sc[1] * (ps ? ps[1] : 1) * Z;
        let dx, dy;
        if (ps && this.camIsOrtho) {
          const vs = this._viewShift(o, size, ps);
          const [cx, cy] = this._orthoZoomCenter(tr.origin, vs);
          dx = cx - dw / 2; dy = cy - dh / 2;
        } else {
          const vs = this._viewShift(o, size, ps);
          const ox = (ps ? tr.origin[0] * ps[0] : tr.origin[0]) + vs[0];
          dx = ox - dw / 2; dy = this.H - (ps ? tr.origin[1] * ps[1] : tr.origin[1]) - dh / 2 + vs[1];
        }
...
,
    _renderSolidLayer(o, model, tr, t) {
        ...
        const ps = ortho && ortho.width ? [this.W / ortho.width, this.H / (ortho.height || 1080)] : null;
        const sc = tr.scale;
        // 正交 zoom (根因 B): 与 renderImage 同款定位
        const Z = ps && this.camIsOrtho ? this._orthoZoomScale() : 1;
        const dw = size[0] * sc[0] * (ps ? ps[0] : 1) * Z, dh = size[1] * sc[1] * (ps ? ps[1] : 1) * Z;
        let dx, dy;
        if (ps && this.camIsOrtho) {
...
,
    _renderPassthroughLayer(o, model, pass, t) {
        ...
        if (ps && this.camIsOrtho) {
          ...
          const ox = (ps ? tr.origin[0] * ps[0] : tr.origin[0]) + vs[0];
          dx = ox - dw / 2; dy = H - (ps ? tr.origin[1] * ps[1] : tr.origin[1]) - dh / 2 + vs[1];
        }
