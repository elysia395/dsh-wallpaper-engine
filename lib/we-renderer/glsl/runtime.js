...
// fmod — 官方宏 #define fmod(x,y) ((x)-(y)*trunc((x)/(y))) (ShaderUnit.cpp:49)
// trunc 版: 负数行为 ≠ GLSL 原生 mod (floor 版, 符号随除数); y=0 → NaN 与官方一致
export const fmod = (x, y) => _vecOp(x, y, (a, b) => a - b * Math.trunc(a / b));
...
// ── mul — 官方矩阵乘宏 #define mul(x, y) ((y) * (x)) (ShaderUnit.cpp:30) ──
// GLSL 源码 mul(a, b) 宏展开为 (b) * (a); 参数交换是 HLSL 行主序 ↔ GLSL 列主序的
// 转置补偿 — GLSL * 对矩阵恒为线性代数乘 (vec*mat / mat*vec / mat*mat), 绝不逐分量。
// 按运算数长度分派 (mat = 列主序 flat, 长 4/9/16 = mat2/3/4):
//   mul(M, v)  a=mat(n) × b=vec(n) (HLSL 矩阵×列向量): result[i] = Σ_j b[j]·a[i·n+j]
//   mul(v, M)  a=vec(n) × b=mat(n):                    result[i] = Σ_j b[j·n+i]·a[j]
//   mul(M, N)  mat×mat 标准矩阵积 (列主序): result[c·n+r] = Σ_j b[j·n+r]·a[c·n+j]
//   vec×vec: 宏展开后即 GLSL 原生 * → 逐分量 (mul() 本身无逐分量语义, 仅此退化形态;
//   长度 4&4 的双义对按 vec4 处理 — WE 着色器变换只用 mat3/mat4, 无 mat2×mat2 调用)
const _MUL_FORM = {
  '16,4': ['mv', 4], '4,16': ['vm', 4],
  '9,3': ['mv', 3], '3,9': ['vm', 3],
  '4,2': ['mv', 2], '2,4': ['vm', 2],
...
export function mul2(a, b) {
  if (typeof a === 'number' || typeof b === 'number') return _vmul(a, b); // 标量参与 → GLSL 原生 * 逐分量广播
  const form = _MUL_FORM[`${len(a)},${len(b)}`];
  if (!form) return _vmul(a, b); // vec×vec 等非矩阵形态 → 逐分量
  const n = form[1];
  if (form[0] === 'mv') { // a=mat(n), b=vec(n): 宏展开 (b)*(a) = vec*mat → bᵀ·a
    const out = mk(n);
    for (let i = 0; i < n; i++) {
      let s = 0;
      for (let j = 0; j < n; j++) s += b[j] * a[i * n + j];
      out[i] = s;
    }
    return out;
  }
  if (form[0] === 'vm') { // a=vec(n), b=mat(n): 宏展开 (b)*(a) = mat*vec → b·a
    const out = mk(n);
    for (let i = 0; i < n; i++) {
      let s = 0;
      for (let j = 0; j < n; j++) s += b[j * n + i] * a[j];
      out[i] = s;
    }
    return out;
  }
  // mat×mat 标准矩阵积 (列主序 in/out)
  const out = mk(n * n);
  for (let c = 0; c < n; c++) {
    for (let r = 0; r < n; r++) {
      let s = 0;
      for (let j = 0; j < n; j++) s += b[j * n + r] * a[c * n + j];
      out[c * n + r] = s;
    }
  }
  return out;
}
// 兼容别名: 旧入口仅覆盖 vec×mat 一种形态 (且无宏参数交换), 统一切到官方语义分派
export const mul = mul2;
...
export function runtimeObject(texSample) {
  const rt = {
    ...
    mix, step, smoothstep, clamp, min, max, abs, floor, ceil, fract, mod, fmod,
    ...
    M_PI, M_PI_HALF, M_PI_2, SQRT_2, SQRT_3, saturate, mul, mul2, CAST2, CAST3, CAST4, CAST3X3,
