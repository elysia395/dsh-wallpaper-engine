...
export function makeSceneRef(objects) {
  ...
  // P2-27: name/id → 对象索引一次构建 — 对象集合不变 (脚本只写 origin/scale/
  // visible/alpha/alignment 叶值, 不增删/改名对象), 旧实现每次 getLayer/
  // getSceneObject 线性 objList.find O(n); 首个命中优先 = 旧 find 语义
  const byName = new Map();
  const byId = new Map();
  for (const x of objList) {
    if (!x) continue;
    if (!byName.has(x.name)) byName.set(x.name, x);
    if (!byId.has(x.id)) byId.set(x.id, x);
  }
  const findByName = (name) => (byName.has(name) ? byName.get(name) : null);
  const findById = (id) => (byId.has(id) ? byId.get(id) : null);
  ...
  return {
    layerExists(name) { const o = findByName(name); if (!o) warnMissing(name); return o; },
    layerGet(name, prop) { return readProp(findByName(name), prop, name); },
    layerSet(name, prop, v) {
      const o = findByName(name);
      if (!o) { warnMissing(name); return; } // 假对象写入丢弃 (旧实现同)
      writeProp(o, prop, v);
    },
    objExists(id) { return !!findById(id); },
    objGet(id, prop) { return readProp(findById(id), prop, ''); },
    objSet(id, prop, v) { const o = findById(id); if (o) writeProp(o, prop, v); },
  };
}
...
// P2-27: 预分配数组复用 — 宿主 writeProp/ownerRef.set 同步消费 (立即 toFixed
// 格式化) 后不再触碰, 数组不跨调用滞留; 每帧脚本写 origin/scale 不再逐次分配
var _xyz = [0, 0, 0];
function xyz(v) {
  var x = v.x != null ? v.x : v[0];
  var y = v.y != null ? v.y : v[1];
  var z = v.z != null ? v.z : v[2];
  _xyz[0] = Number(x); _xyz[1] = Number(y); _xyz[2] = Number(z);
  return _xyz;
}
...
// P2-27: 对象级 scriptproperties payload 按引用 memoize — WeakMap 键为 sp 对象
// (scene.json 解析一次, 引用跨帧稳定 → 同引用同内容), userProps 引用一并校验,
// 任一变化即重建。__user 为保留键 (用户属性值表), 与旧逐帧重建的产物逐键相同
const _propsPayloadMemo = new WeakMap();
function propsPayload(sp, userProps) {
  const key = userProps || null;
  const m = _propsPayloadMemo.get(sp);
  if (m && m.key === key) return m.payload;
  const payload = { __user: userProps || {} };
  for (const k of Object.keys(sp)) {
    if (k === '__user') continue;
    payload[k] = sp[k];
  }
  _propsPayloadMemo.set(sp, { key, payload });
  return payload;
}
...
function runScriptValueCached(scriptVal, time, opts = {}) {
  ...
  if (!entry) {
    ...
    entry = {
      ...
      lastProps: undefined, // P2-27: 该 context 最近一次已安装的 props payload (引用比对)
    };
  ...
  // P2-27: payload 按引用 memoize (同引用同内容不重拷贝) + 与该 context 最近一次
  // 已安装 payload 相同则跳过 dispatch — 旧实现每帧重建 payload 并在沙箱内
  // copyPlain 全量重拷; 引用变化 (换对象/换 userProps) 或同源码多对象交替安装
  // 时自动重装, __scriptProperties 写回语义不变
  if (scriptVal.scriptproperties) {
    const payload = propsPayload(scriptVal.scriptproperties, opts.userProps);
    if (payload !== entry.lastProps) {
      try {
        invokeDispatch(entry.context, 'props', payload, INIT_TIMEOUT);
        entry.lastProps = payload; // 安装成功才记引用 (失败下一帧照旧重试)
      } catch { /* 忽略 */ }
    }
  }
  ...
  // P2-27: 调度闭包提升到 entry — 旧实现每帧每脚本重建; entry 按 src 缓存,
  // 捕获的 entry/src 恒定, 语义不变
  if (!entry.call) {
    entry.call = (op, arg, tmo) => {
      let r;
      try {
...
// 壁纸是否含内嵌脚本 (与下方 applySceneScripts 的 walk 谓词逐条一致:
// {script: string, value} 节点; 首个命中即返回)。W0: 脚本总开关默认关闭
// 时, 渲染器/GL gate 用它发 "scene-script" degraded 提醒 (配置页可见)。
export function sceneHasScripts(scene) {
  let found = false;
  const walk = (obj) => {
...
export function applySceneScripts(scene, time, opts = {}) {
  ...
  // P2-27: walk 结果缓存 — 可写路径清单 [obj, owner, ...] (含执行顺序) 只在
  // scene/renderObjects 引用变化时重建 (失效点与渲染器帧首缓存同款: 换场景/
  // 换渲染对象表 = 新一轮)。引用跨帧稳定 (SceneRenderer 构造时解析一次), 且
  // 脚本写回只改 value/origin/scale/visible/alpha/alignment 叶值 (动画烘焙
  // 也只替换叶值 o[key]={value}), 不增删/移动 script 节点 → 清单恒有效;
  // {script,value} 写回语义不变 (逐帧仍按同序执行 update)
  let frame = cache && cache.frame && cache.frame.scene === scene
    && cache.frame.renderObjects === opts.renderObjects ? cache.frame : null;
  if (!frame) {
    ...
    const walk = (obj, owner) => {
      ...
      if ('script' in obj && 'value' in obj && typeof obj.script === 'string') {
        pairs.push(obj, owner); // 扁平双槽存储, 免每节点 pair 对象分配
    ...
    walk(scene, null);
    frame = { scene, renderObjects: opts.renderObjects, sceneObjects, thisScene, ownerRef, pairs };
    if (cache) cache.frame = frame;
  }
  // shared: 沙箱内创建、经宿主登记的跨脚本共享对象 (P-01: 宿主对象不再注入
  // 沙箱; 挂在 cache.shared 上跨帧/跨脚本复用 — NSL 动画调度器/状态依赖)。
  // P2-27: sharedRef 提升到 cache 复用 (帧首重挂当前 shared, 同旧每帧新建语义)
  if (cache) {
    if (!cache.sharedRef) cache.sharedRef = { current: null };
    cache.sharedRef.current = cache.shared;
  }
  const sharedRef = cache ? cache.sharedRef : { current: null };
  // P2-27: 运行参数对象全帧一份复用 (旧实现每脚本每帧新建字面量), 仅
  // currentObject 逐脚本切换 — runScriptValueCached 只读这些字段
  const runOpts = {
    canvasSize: opts.canvasSize,
    userProps: opts.userProps,
    sharedRef,
    sceneObjects: frame.sceneObjects,
    thisScene: frame.thisScene,
    cache: cache ? cache.map : null,
    ownerRef: frame.ownerRef,
  ...
  for (let i = 0; i + 1 < pairs.length; i += 2) {
    runOpts.currentObject = pairs[i + 1] || null;
    runScriptValueCached(pairs[i], time, runOpts);
  }
