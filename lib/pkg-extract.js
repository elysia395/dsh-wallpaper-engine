...
function decodeTex(data) {
  ...
  // Pick the HIGHEST-resolution mipmap, NOT blindly mipmaps[0]. WE normally
  // orders mipmaps largest-first, but a few container variants and post-
  // processed textures list the base (post-progressive-refine) mip out of
  // order. Selecting by pixel area instead of index guarantees we always
  // decode the full-resolution frame, which is what removes the resolution
  // loss on those textures. Tie-break on index keeps deterministic output.
  let mip0 = parsed.mipmaps[0];
  let mip0Area = mip0.width * mip0.height;
  for (let i = 1; i < parsed.mipmaps.length; i++) {
    const m = parsed.mipmaps[i];
    const area = m.width * m.height;
    if (area > mip0Area) { mip0 = m; mip0Area = area; }
  }
...
export { extractSceneMainImage, extractSceneMainImageFromDir, parsePkg, readPkgEntry, parseTex };
