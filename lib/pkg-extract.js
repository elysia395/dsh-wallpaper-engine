 ...
 * DXT1 / DXT3 / DXT5. Raw-format results are cropped to the TEXI image rect
 * (WE pads raw mips to power-of-two sizes; the padding is filler, not
 * content). Embedded MP4 textures and unknown formats throw a
 ...
 */
/** IEEE 754 half → float (A5: R16F/RG1616F 纹理解码用, 无 DataView 半精度 API)。 */
...
/**
 * Crop power-of-two mip padding to the TEXI image rect (top-left anchored).
 * WE stores raw-format mips padded (e.g. a 3840×2160 image in a 4096×4096
 * DXT5 mip — 3784528825 的全部 DXT5 图层); the header image rect is the real
 * content, everything beyond it is filler. Decoding the padded mip as the
 * whole image squashes/displaces every layer in the extractor composite
 * (部件坐标拼接异常的根因)。与 scene-manifest.js cropToImageRect 同款语义,
 * 两处保持一致。
 */
function cropToImageRect(img, imageWidth, imageHeight) {
  const cropW = Math.min(imageWidth, img.width);
  const cropH = Math.min(imageHeight, img.height);
  if (cropW > 0 && cropH > 0 && (cropW < img.width || cropH < img.height)) {
    const cropped = new Uint8Array(cropW * cropH * 4);
    for (let y = 0; y < cropH; y++) {
      cropped.set(img.rgba.subarray(y * img.width * 4, (y * img.width + cropW) * 4), y * cropW * 4);
    }
    return { kind: 'rgba', width: cropW, height: cropH, rgba: cropped };
  }
  return { kind: 'rgba', width: img.width, height: img.height, rgba: img.rgba };
}
...
function decodeTex(data, preParsed) {
  ...
  switch (parsed.format) {
    case TexFormat.RGBA8888: {
      ...
      return cropToImageRect({ width, height, rgba: bytes.slice(0, width * height * 4) }, parsed.width, parsed.height);
    }
    case TexFormat.R8: {
      ...
      // sf41b 官方语义 (assets/shaders/common_fragment.h ConvertTexture0Format,
      // TEX0FORMAT==FORMAT_R8): vec4(1,1,1,_sample.r) — R 进 alpha, RGB 恒白。
      // 旧实现 (R,R,R,255) 把遮罩当亮度 → 形状错。
      ...
      return cropToImageRect({ width, height, rgba }, parsed.width, parsed.height);
    }
    case TexFormat.R16F: {
      // A5: 官方头 ConvertTexture0Format — R16F 与 R8 同式 vec4(1,1,1,_sample.r);
      // 数据为 half float (2B/px), 量化到 0-255 (clamp [0,1]×255, 与 GPU 采样等价)
      if (bytes.length < width * height * 2) {
        const derived = deriveDims(bytes.length, width, height, 2);
        if (!derived) throw new Error('tex: mipmap size mismatch for R16F');
        width = derived.width;
        height = derived.height;
      }
      ...
      const rgba = new Uint8Array(width * height * 4);
      for (let i = 0; i < width * height; i++) {
        const v = halfToFloat(dv.getUint16(i * 2, true));
    ...
    case TexFormat.RG88: {
      ...
      // sf41b 官方语义 (common_fragment.h ConvertTexture0Format, TEX0FORMAT==
      // FORMAT_RG88): vec4(_sample.rrrg) — R 复制进 RGB (亮度/颜色), G 进 alpha。
      // 旧实现 (R,G,0,255) 把双通道数据当 RGB → light_shafts_6/beam_0 渲染成
      // 纯红矩形 (3593194513 红色旋转矩形根因)。
      const rgba = new Uint8Array(width * height * 4);
      for (let i = 0; i < width * height; i++) {
        rgba[i * 4] = bytes[i * 2];
        rgba[i * 4 + 1] = bytes[i * 2];
        rgba[i * 4 + 2] = bytes[i * 2];
        rgba[i * 4 + 3] = bytes[i * 2 + 1];
      }
      return cropToImageRect({ width, height, rgba }, parsed.width, parsed.height);
    }
    case TexFormat.RG1616F: {
      // A5: 官方头 — RG1616F 与 RG88 同式 vec4(_sample.rrrg); half float ×2
      if (bytes.length < width * height * 4) {
        const derived = deriveDims(bytes.length, width, height, 4);
        if (!derived) throw new Error('tex: mipmap size mismatch for RG1616F');
        width = derived.width;
        height = derived.height;
      }
      const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
      const rgba = new Uint8Array(width * height * 4);
      for (let i = 0; i < width * height; i++) {
        const r = halfToFloat(dv.getUint16(i * 4, true));
        const g = halfToFloat(dv.getUint16(i * 4 + 2, true));
        const qr = r <= 0 ? 0 : r >= 1 ? 255 : Math.round(r * 255);
        const qg = g <= 0 ? 0 : g >= 1 ? 255 : Math.round(g * 255);
        rgba[i * 4] = qr;
        rgba[i * 4 + 1] = qr;
        rgba[i * 4 + 2] = qr;
        rgba[i * 4 + 3] = qg;
    ...
    case TexFormat.DXT1: {
      ...
      return cropToImageRect({ width, height, rgba: decodeDxt1(bytes, width, height) }, parsed.width, parsed.height);
    }
    case TexFormat.DXT3: {
      ...
      return cropToImageRect({ width, height, rgba: decodeDxt3(bytes, width, height) }, parsed.width, parsed.height);
    }
    case TexFormat.DXT5: {
      const expected = Math.ceil(width / 4) * Math.ceil(height / 4) * 16;
      if (bytes.length < expected) throw new Error('tex: mipmap size mismatch for DXT5');
      return cropToImageRect({ width, height, rgba: decodeDxt5(bytes, width, height) }, parsed.width, parsed.height);
    }
    default:
 ...
 *  Fully-transparent pixels are skipped — sprites/parts textures are mostly
 *  transparent and their RGB in invisible regions is undefined filler (DXT
 *  blocks decode to arbitrary flat color there). Counting them made the gate
 *  depend on padding junk: a real sprite could pass only while its padding
 *  was (wrongly) kept, and got rejected once decoded correctly cropped
 *  (assets/presets/fern preview 回归, sf39). A fully-transparent frame has
 *  no content at all → rejected as flat. */
function frameQuality(width, height, rgba) {
  const total = width * height;
  const n = Math.min(2000, total);
  const seen = new Set();
  const idxs = [];
  let gray = 0, sr = 0, sg = 0, sb = 0, visible = 0;
  for (let i = 0; i < n; i++) {
    let idx;
  ...
  if (!visible) return { grayRatio: 1, meanVar: 0 };
  const mr = sr / visible, mg = sg / visible, mb = sb / visible;
  let v = 0;
  ...
  return { grayRatio: gray / visible, meanVar: v / (visible * 3) };
}
 ...
 */
function decodePngPayload(bytes, maxPixels = PNG_GATE_MAX_PIXELS) {
  ...
  // 隔行 (Adam7) 扫描线布局不同, 逐行反滤会输出错位像素 — 直接拒绝。
  if (w <= 0 || h <= 0 || w > 16384 || h > 16384 || !channels || b[28] !== 0
    || w * h > maxPixels) return null;
  const idats = [];
  let p = 8;
  while (p < b.length) {
  ...
  // RGB (ct=2) 载荷没有 alpha 采样, alpha 字节从未被写 → 全 0, source-over
  // 拼合时整层不可见。补不透明 alpha。
  if (channels === 3) {
...
function resolveSceneTexPath(access, ref) {
  ...
  if (!want) return null;
  const hit = access.listTexPaths().find((p) => {
  ...
  if (hit) return hit;
  // WE 全局素材兜底 (assets 根已登记时): 官方从 <WE 安装目录>/assets 解析
  // 壁纸未随包分发的全局纹理 (materials/particle/halo.tex 等)。返回与 pkg
  // 命中同款的相对路径形态, 下游读取经 readGlobalAsset 围栏兜底。
  if (_globalAssetsRoots.length) {
    const idx = globalAssetsIndex();
    const rel = idx.get('materials/' + want + '.tex') || idx.get(want + '.tex');
    if (rel) return rel;
  }
  return null;
}
...
// ── WE 全局素材根 (assets) ─────────────────────────────────────────────
// 官方 WE 从 <安装目录>/assets 解析全局纹理/材质 (粒子 halo/drop 等不随壁纸
// pkg 分发)。本机无 WE 安装: 宿主把 <壁纸根>/assets 登记进来 (用户自备素材)。
let _globalAssetsRoots = []; // realpath 后的根列表
let _globalAssetsIndex = null; // Map<lowerRel, rel> 懒建 (大小写不敏感探测)

function setGlobalAssetsRoots(dirs) {
  _globalAssetsRoots = (Array.isArray(dirs) ? dirs : [])
    .filter((d) => typeof d === 'string' && d.trim())
    .map((d) => {
      try {
        const real = realpathSync(resolve(d.trim()));
        return statSync(real).isDirectory() ? real : null;
      } catch { return null; }
    })
    .filter(Boolean);
  _globalAssetsIndex = null; // 根变化 → 索引重建
}

// 追加单个全局素材根 (去重; 优先级低于先登记根 — 索引/读取按登记序)。
// Windows: 真实 WE 安装定位后把 <install>/assets 补进来 (resolveLooseDirs
// 默认仅 linux 有值, 不设 DSH_WE_ASSETS_DIR 时 Windows 上根列表为空)。
function addGlobalAssetsRoot(dir) {
  if (typeof dir !== 'string' || !dir.trim()) return false;
  let real = null;
  try {
    const r = realpathSync(resolve(dir.trim()));
    if (statSync(r).isDirectory()) real = r;
  } catch { return false; }
  if (!real || _globalAssetsRoots.includes(real)) return false;
  _globalAssetsRoots.push(real);
  _globalAssetsIndex = null; // 根变化 → 索引重建
  return true;
}

function globalAssetsIndex() {
  if (_globalAssetsIndex) return _globalAssetsIndex;
  const idx = new Map();
  for (const root of _globalAssetsRoots) {
    const walk = (sub, depth) => {
      if (depth > 6) return;
      let names;
      try { names = readdirSync(sub ? join(root, sub) : root); } catch { return; }
      for (const n of names) {
        const rel = sub ? sub + '/' + n : n;
        let st;
        try { st = lstatSync(join(root, rel)); } catch { continue; }
        if (st.isSymbolicLink()) continue; // H-10: symlink 条目不进索引
        if (st.isDirectory()) walk(rel, depth + 1);
        else if (st.isFile() && /\.tex$/i.test(n)) {
          const k = rel.toLowerCase();
          if (!idx.has(k)) idx.set(k, rel); // 先登记的根优先
        }
      }
    };
    walk('', 0);
  }
  _globalAssetsIndex = idx;
  return idx;
}

// 全局素材围栏读 (H-10 同款: 词法 + realpath 双围栏, 拒 symlink; 仅 .tex)
function readGlobalAsset(rel) {
  const p = String(rel || '').replace(/\\/g, '/').replace(/^\/+/, '');
  if (!p || p.includes('..') || !/\.tex$/i.test(p)) return null;
  for (const root of _globalAssetsRoots) {
    ...
    try {
      ...
      return { path: p, bytes: new Uint8Array(readFileSync(real)) };
    } catch { /* 尝试下一根 */ }
  }
  return null;
}
...
/** 真解码上限 (与 JPEG 路径 maxResolutionInMP: 64 对齐)。质量门pngQuality 仍用
 * 更紧的 PNG_GATE_MAX_PIXELS — 但 decodeTexToRgba 要的是真实像素, 用 12MP
 * 质量门上限会把 8K 工程 (3427824116 8192×4608 = 37.7MP) 的全部图层静默
 * 丢成 null, 拼合回落单纹理路径只剩雾状天空层。 */
const TEX_DECODE_MAX_PIXELS = 64 * 1024 * 1024;
...
function decodeTexToRgba(bytes) {
  ...
  if (decoded.kind === 'png-pass') return decodePngPayload(decoded.bytes, TEX_DECODE_MAX_PIXELS);
  return null;
}
...
/** Downscale budget once a scene exceeds COMPOSITE_MAX_PIXELS. Static frames
 * are consumed as cover-fit wallpaper backgrounds (≤4K displays), so a 4K
 * composite keeps geometry/proportions intact at a fraction of the decode,
 * resize and encode cost of 8K-class scenes (3427824116: 8192×4608). */
const COMPOSITE_DOWNSCALE_TARGET = 3840 * 2160;
...
function tryCompositeSceneLayers(scene, access, label) {
  ...
  for (const obj of imageObjects) {
    ...
    if (obj.image.toLowerCase().endsWith('.tex')) {
      ...
      // 木偶对象: image json 是部件 atlas + 网格骨架 (puppet 字段), 其材质纹理
      // 是散落部件的打包图而非可平面拼合的 quad — 整图拼上画布必然部件乱叠
      // (3784528825 Girl/Dragon Head 四肢飘带散一屏)。CPU 拼合不重建木偶
      // 姿态, 跳过该对象 (GL 路径经 /scene-puppet 网格渲染, 不受影响)。
      if (modelJson.puppet) continue;
    ...
    // WE 坐标系: origin.y 自底向上 (历史 CPU 渲染器 image.js 定位链:
    // dy = H - origin.y*ps - dh/2), 画布 y 向下 — 拼合此前直接把 origin[1] 当
    // 画布 y 用, 全部部件垂直错位 (3735447194 部件 atlas 乱叠的根因)。
    // 这里把 origin.y 翻转成画布坐标 (ortho 高度缺失时用画布高兜底)。
    if (projection) cy = projection.height - cy;
  ...
  // 8K 级工程 (3427824116 投影 8192×4608 = 37.7MP) 超过硬上限时不再整帧放弃
  // (放弃会回落单纹理路径, 静态帧只剩天空雾层) — 均匀缩放画布与全部图层
  // 到 4K 预算内再拼合, 层间几何关系按同一比例保持不变。
  if (cw * ch > COMPOSITE_MAX_PIXELS) {
    const shrink = Math.sqrt(COMPOSITE_DOWNSCALE_TARGET / (cw * ch));
    cw = Math.max(1, Math.floor(cw * shrink));
    ch = Math.max(1, Math.floor(ch * shrink));
    offX = Math.round(offX * shrink);
    offY = Math.round(offY * shrink);
    for (const L of layers) {
      L.cx *= shrink;
      L.cy *= shrink;
      L.lw = Math.round(L.lw * shrink);
      L.lh = Math.round(L.lh * shrink);
    }
  }
...
export { extractSceneMainImage, extractSceneMainImageFromDir, parseTex, decodeTex, parsePkg, readPkgEntry, extractTexVideoMp4, probeTexVideoQuick, resolveSceneTexPath, pkgSceneAccess, dirSceneAccess, setGlobalAssetsRoots, addGlobalAssetsRoot, readGlobalAsset };
