...
(function() {
  ...
  // P2-24: uniform/attrib 位置按 program 缓存 — 位置在 link 后不变，而旧热路径
  // 每帧每 mesh/layer/sprite getUniformLocation×20+ / getAttribLocation×2（链接
  // 后查询在部分驱动上是真实 GL 调用）。首帧惰性建表，之后全 Map 命中；页面在
  // contextrestored 后整页重建（见下方 postMessage），不存在跨程序复用旧表。
  const _uLocCache = new Map(); // program → Map<name, WebGLUniformLocation|null>
  function uLoc(prog, name) {
    let m = _uLocCache.get(prog);
    if (!m) { m = new Map(); _uLocCache.set(prog, m); }
    let l = m.get(name);
    if (l === undefined) { l = gl.getUniformLocation(prog, name); m.set(name, l); }
    return l;
  }
  const _aLocCache = new Map(); // program → Map<name, attrib index（缺失=-1 也可缓存）>
  function aLoc(prog, name) {
    let m = _aLocCache.get(prog);
    if (!m) { m = new Map(); _aLocCache.set(prog, m); }
    let l = m.get(name);
    if (l === undefined) { l = gl.getAttribLocation(prog, name); m.set(name, l); }
    return l;
  }
  // P2-24: MAX_VERTEX_ATTRIBS 启动期取一次（旧实现在 renderFrame 每帧、
  // drawCloudsBgLayer 每次调用都 gl.getParameter 查询）
  ...
  function drawNeonGrid(model, mesh, proj, view, elapsed) {
    ...
    gl.uniformMatrix4fv(uLoc(progNeonGrid, 'u_proj'), false, proj);
    gl.uniformMatrix4fv(uLoc(progNeonGrid, 'u_view'), false, view);
    gl.uniformMatrix4fv(uLoc(progNeonGrid, 'u_model'), false, mat4Transform3D(model.origin, model.angles, model.scale));
    gl.uniform1f(uLoc(progNeonGrid, 'u_time'), elapsed);
    gl.uniform1f(uLoc(progNeonGrid, 'u_mountainScale'), un.mountainscale != null ? un.mountainscale : 1);
    const near = uc.gridnear || [1, 0, 0.2];
    const far = uc.gridfar || [0, 0, 1];
    const bgc = uc.gridbackground || [0.1, 0, 0.1];
    gl.uniform3f(uLoc(progNeonGrid, 'u_gridNear'), near[0], near[1], near[2]);
    gl.uniform3f(uLoc(progNeonGrid, 'u_gridFar'), far[0], far[1], far[2]);
    gl.uniform3f(uLoc(progNeonGrid, 'u_gridBg'), bgc[0], bgc[1], bgc[2]);
    const gpu = getGpuMesh(mesh);
    const gPos = aLoc(progNeonGrid, 'a_pos');
    const gUv = aLoc(progNeonGrid, 'a_uv');
  ...
  function drawCloudsBgLayer(layer, elapsed, width, height) {
    ...
    const cPos = aLoc(progCloudsBg, 'a_corner');
    const cUv = aLoc(progCloudsBg, 'a_uv');
    ...
    gl.uniform1f(uLoc(progCloudsBg, 'u_time'), elapsed);
    gl.uniform1f(uLoc(progCloudsBg, 'u_aspect'), width / Math.max(height, 1));
    const uc = layer.userColors || {};
    const c1 = uc.clouds || [0.05, 0.15, 0.4];
    const ch = uc.horizon || [0.05, 0.15, 0.4];
    gl.uniform3f(uLoc(progCloudsBg, 'u_color1'), c1[0], c1[1], c1[2]);
    gl.uniform3f(uLoc(progCloudsBg, 'u_colorHorizon'), ch[0], ch[1], ch[2]);
    if (layer.texUrl) {
      const texRec = loadTexture(layer.texUrl, true);
      if (texRec.loaded) {
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texRec.texture);
        gl.uniform1i(uLoc(progCloudsBg, 'u_tex'), 0);
        gl.uniform1i(uLoc(progCloudsBg, 'u_hasTex'), 1);
      } else {
        gl.uniform1i(uLoc(progCloudsBg, 'u_hasTex'), 0);
      }
    } else {
      gl.uniform1i(uLoc(progCloudsBg, 'u_hasTex'), 0);
    }
  ...
  function drawBillboard(center, axisX, axisY, texUrl, color, proj, view) {
    ...
    gl.uniformMatrix4fv(uLoc(progSprite, 'u_proj'), false, proj);
    gl.uniformMatrix4fv(uLoc(progSprite, 'u_view'), false, view);
    gl.bindBuffer(gl.ARRAY_BUFFER, spriteBuf);
    const cPos = aLoc(progSprite, 'a_corner');
    const cUv = aLoc(progSprite, 'a_uv');
    gl.enableVertexAttribArray(cPos);
    gl.enableVertexAttribArray(cUv);
    gl.vertexAttribPointer(cPos, 2, gl.FLOAT, false, 16, 0);
    gl.vertexAttribPointer(cUv, 2, gl.FLOAT, false, 16, 8);
    gl.uniform3f(uLoc(progSprite, 'u_center'), center[0], center[1], center[2]);
    gl.uniform2f(uLoc(progSprite, 'u_axisX'), axisX[0], axisX[1]);
    gl.uniform2f(uLoc(progSprite, 'u_axisY'), axisY[0], axisY[1]);
    gl.uniform4f(uLoc(progSprite, 'u_color'), color[0], color[1], color[2], color[3]);
    if (texUrl) {
      const texRec = loadTexture(texUrl);
      if (texRec.loaded) {
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texRec.texture);
        gl.uniform1i(uLoc(progSprite, 'u_tex'), 0);
        gl.uniform1i(uLoc(progSprite, 'u_hasTex'), 1);
      } else {
        gl.uniform1i(uLoc(progSprite, 'u_hasTex'), 0);
      }
    } else {
      gl.uniform1i(uLoc(progSprite, 'u_hasTex'), 0);
    }
  ...
  // P2-24: 3D 队列分桶加载期一次 — 旧实现在 renderFrame 每帧重建
  // skybox/dome/shadow/grid/opaque/glass/additive/translucent/neongrid 队列 +
  // jetPos 扫描 + additive 逐 mesh flag 推断，而这些全部只依赖 manifest 的
  // 静态 models 列表。glassQueue 保留逐帧收集（绘制顺序上的延迟透传队列），
  // 复用模块级数组免每帧分配；manifest 到达时重建一次分桶。
  const glassQueue = [];
  let queues3d = null;
  function buildQueues3d() {
    const q = { skybox: [], dome: [], opaque: [], shadow: [], grid: [], additive: [], translucent: [], neonGrid: [], jetPos: [] };
    if (!sceneData || !sceneData.is3D || !Array.isArray(sceneData.models)) return q;
    for (const model of sceneData.models) {
      const mName = (model.name || '').toLowerCase();
      if (mName === 'skybox') { q.skybox.push(model); continue; }
      if (mName === 'dome') { q.dome.push(model); continue; }
      if (mName === 'shadow') { q.shadow.push(model); continue; }
      if (mName === 'grid') { q.grid.push(model); continue; }
      ...
      for (const mesh of model.meshes) {
        ...
        if (shName === 'neongrid') q.neonGrid.push({ model, mesh });
        else if (mesh.additive || mName.includes('jet')) {
          // Additive glow classification (jet/aurora/thunder/selfillum) is
          // static per mesh — precompute the draw flags here instead of
          // re-deriving them every frame at the draw site.
          const jetLike = shName.includes('jet') || (mesh.texUrl || '').toLowerCase().includes('jet');
          const isAurora = !jetLike && shName.includes('aurora');
          const isThunder = !jetLike && shName.includes('thunder');
          q.additive.push({ model, mesh, flags: {
            jet: jetLike, aurora: isAurora, thunder: isThunder,
            selfIllum: !jetLike && !isAurora && !isThunder,
            followEye: isAurora || isThunder,
          } });
        }
        else if (mesh.translucent) q.translucent.push({ model, mesh });
        else opaqueMeshes.push(mesh);
      }
      if (opaqueMeshes.length > 0) q.opaque.push({ ...model, meshes: opaqueMeshes });
      // Engine-glow boost positions (ricepod.vert): origins of jet models.
      const jetLikeModel = mName.includes('jet') || (model.meshes || []).some((mm) => (mm.shader || '').toLowerCase().includes('jet'));
      if (jetLikeModel && q.jetPos.length < 4) q.jetPos.push(model.origin || [0, 0, 0]);
    }
    return q;
  }
  queues3d = buildQueues3d(); // sceneData 未到时为空桶（renderFrame 有 !sceneData 早退守卫）
  ...
  function renderFrame(now) {
    ...
    if (sceneData.is3D && sceneData.models && sceneData.models.length > 0) {
      ...
      function bindProg3D(viewOverride) {
        ...
        gl.uniformMatrix4fv(uLoc(prog3D, 'u_proj'), false, proj3D);
        gl.uniformMatrix4fv(uLoc(prog3D, 'u_view'), false, viewOverride || view3D);
        gl.uniform3f(uLoc(prog3D, 'u_cameraPos'), eye[0], eye[1], eye[2]);
        gl.uniform1f(uLoc(prog3D, 'u_time'), elapsed);
        // WE-standard scene shading for generic scenes; car scenes keep their
        // dedicated paint/grid pipeline.
        gl.uniform1i(uLoc(prog3D, 'u_sceneStd'), isCarScene ? 0 : 1);
        // Engine-glow boost positions: origins of jet models (ricepod.vert).
        // P2-24: 加载期 buildQueues3d 已预扫，不再每帧遍历 models
        const jetPos = queues3d.jetPos;
        gl.uniform1i(uLoc(prog3D, 'u_jetCount'), jetPos.length);
        for (let ji = 0; ji < 4; ji++) {
          const jp = jetPos[ji] || [0, 0, 0];
          gl.uniform3f(uLoc(prog3D, 'u_jetPos[' + ji + ']'), jp[0], jp[1], jp[2]);
        }
        // Ricepod uses lightDir (-0.577, 0.577, 0.577), car uses (0.577, 0.577, 0.577)
        gl.uniform3f(uLoc(prog3D, 'u_lightDir'), isCarScene ? 0.577 : -0.577, 0.577, 0.577);
        ...
        gl.uniform3f(uLoc(prog3D, 'u_ambientColor'), ambColor[0], ambColor[1], ambColor[2]);
        gl.uniform3f(uLoc(prog3D, 'u_paintColor'), bodyCol[0], bodyCol[1], bodyCol[2]);
      }
      bindProg3D();

      const locPos = aLoc(prog3D, 'a_pos');
      const locNorm = aLoc(prog3D, 'a_norm');
      const locUv = aLoc(prog3D, 'a_uv');
      ...
      // P2-24: 分桶直接取加载期结果（每帧零重建）；glassQueue 逐帧清空重收集
      const skyboxModels = queues3d.skybox;
      const domeModels = queues3d.dome;
      const opaqueModels = queues3d.opaque;
      const shadowModels = queues3d.shadow;
      const gridModels = queues3d.grid;
      const additiveQueue = queues3d.additive;
      const translucentQueue = queues3d.translucent;
      const neonGridQueue = queues3d.neonGrid;
      glassQueue.length = 0;
      ...
      function drawMesh(model, mesh, flags) {
        ...
        gl.uniformMatrix4fv(uLoc(prog3D, 'u_model'), false, modelMat);
        const normMat = mat3NormalMatrix(modelMat);
        gl.uniformMatrix3fv(uLoc(prog3D, 'u_normMat'), false, normMat);
        ...
        gl.uniform1i(uLoc(prog3D, 'u_isDome'), flags.dome ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isShadow'), flags.shadow ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isGrid'), flags.grid ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isSkybox'), flags.skybox ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isSelfIllum'), flags.selfIllum ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isCarBody'), flags.body ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isGlass'), flags.glass ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isJet'), flags.jet ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isAurora'), flags.aurora ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isThunder'), flags.thunder ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isBg'), flags.bg ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_isNeonSun'), flags.neonSun ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_gradFade'), mesh.gradFade ? 1 : 0);
        gl.uniform1i(uLoc(prog3D, 'u_hasTint'), mesh.tint || flags.neonSun ? 1 : 0);
        const uc = mesh.userColors || {};
        ...
        gl.uniform3f(uLoc(prog3D, 'u_tint'), tintCol[0], tintCol[1], tintCol[2]);
        gl.uniform3f(uLoc(prog3D, 'u_tint2'), tint2Col[0], tint2Col[1], tint2Col[2]);
        ...
        if (mesh.texUrl2) {
          ...
          if (tex2Rec.loaded) {
            ...
            gl.uniform1i(uLoc(prog3D, 'u_tex2'), 1);
            gl.activeTexture(gl.TEXTURE0);
          }
        }
        ...
        gl.uniform1f(uLoc(prog3D, 'u_specStrength'), sp[0]);
        gl.uniform1f(uLoc(prog3D, 'u_specPower'), sp[1]);

        if (flags.body) {
          const strCol = sceneData.carStripesColor || [0, 0, 0];
          gl.uniform3f(uLoc(prog3D, 'u_paintColor'), bodyCol[0], bodyCol[1], bodyCol[2]);
          gl.uniform3f(uLoc(prog3D, 'u_stripeColor'), strCol[0], strCol[1], strCol[2]);
        }
        ...
        if (mesh.texUrl && !flags.dome && !flags.shadow && !flags.grid) {
          ...
          if (texRec.loaded) {
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, texRec.texture);
            gl.uniform1i(uLoc(prog3D, 'u_tex'), 0);
            gl.uniform1i(uLoc(prog3D, 'u_hasTex'), 1);
          } else {
            gl.uniform1i(uLoc(prog3D, 'u_hasTex'), 0);
            gl.uniform3f(uLoc(prog3D, 'u_color'), 0.7, 0.7, 0.75);
          }
        } else {
          gl.uniform1i(uLoc(prog3D, 'u_hasTex'), 0);
          gl.uniform3f(uLoc(prog3D, 'u_color'), 0.65, 0.68, 0.72);
        }
      ...
      if (hasGrid) {
        ...
        // Dome to FBO
        gl.depthMask(false);
        gl.uniform1i(uLoc(prog3D, 'u_hasReflTex'), 0);
      ...
      if (hasGrid && reflTex) {
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, reflTex);
        gl.uniform1i(uLoc(prog3D, 'u_reflTex'), 1);
        gl.uniform1i(uLoc(prog3D, 'u_hasReflTex'), 1);
        gl.uniform2f(uLoc(prog3D, 'u_resolution'), width, height);
      }
      ...
      // P2-24: flags 已在 buildQueues3d 预计算，直接解构绘制
      for (const { model, mesh, flags } of additiveQueue) {
        drawMesh(model, mesh, flags);
      }
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, quadBuf);
    const aPos = aLoc(progBasic, 'a_pos');
    const aUv = aLoc(progBasic, 'a_uv');
    ...
    gl.uniform1f(uLoc(progBasic, 'u_time'), elapsed);
    gl.uniform4f(uLoc(progBasic, 'u_uvRect'), 0, 0, 1, 1);
    gl.uniform1f(uLoc(progBasic, 'u_bright'), 1);
    gl.uniform1f(uLoc(progBasic, 'u_power'), 1);
    ...
    for (const layer of sceneData.layers) {
      ...
      gl.uniformMatrix4fv(uLoc(progBasic, 'u_model'), false, model);
      gl.uniform1f(uLoc(progBasic, 'u_alpha'), layer.alpha != null ? layer.alpha : 1.0);
      gl.uniform3f(uLoc(progBasic, 'u_tint'), 1, 1, 1);
      gl.uniform1f(uLoc(progBasic, 'u_sway'), layer.sway || 0);
      gl.uniform1f(uLoc(progBasic, 'u_sway_speed'), layer.swaySpeed || 1.0);

      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, texRec.texture);
      gl.uniform1i(uLoc(progBasic, 'u_tex'), 0);
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    }
    ...
    for (const layer of sceneData.layers) {
      if (layer.isReflection) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, quadBuf);
        const rPos = aLoc(progReflection, 'a_pos');
        const rUv = aLoc(progReflection, 'a_uv');
        ...
        const model = mat4Transform2D(layer.x, layer.y, layer.w, layer.h, layer.angle || 0);
        gl.uniformMatrix4fv(uLoc(progReflection, 'u_proj'), false, proj);
        gl.uniformMatrix4fv(uLoc(progReflection, 'u_model'), false, model);
        gl.uniform4f(uLoc(progReflection, 'u_uvRect'), 0, 0, 1, 1);
        gl.uniform1f(uLoc(progReflection, 'u_time'), elapsed);
        gl.uniform1f(uLoc(progReflection, 'u_alpha'), 0.85);
        ...
        gl.uniform4f(uLoc(progReflection, 'u_rect'),
        ...
        gl.uniform1f(uLoc(progReflection, 'u_waterLine'), waterLine);
        gl.uniform2f(uLoc(progReflection, 'u_reflectRange'),
          waterLine - 0.23 * depthScale, 0.38 * depthScale);

        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, fboTex);
        gl.uniform1i(uLoc(progReflection, 'u_fbo'), 0);

        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, maskRec.texture);
        gl.uniform1i(uLoc(progReflection, 'u_mask'), 1);
      ...
      if (layer.shader === 'flowimage' && layer.texUrls && layer.texUrls.length >= 4) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, quadBuf);
        const fPos = aLoc(progFlow, 'a_pos');
        const fUv = aLoc(progFlow, 'a_uv');
        ...
        const model = mat4Transform2D(layer.x, layer.y, layer.w, layer.h, layer.angle || 0);
        gl.uniformMatrix4fv(uLoc(progFlow, 'u_proj'), false, proj);
        gl.uniformMatrix4fv(uLoc(progFlow, 'u_model'), false, model);
        const fcrop = layer.uvCrop || [0, 0, 1, 1];
        gl.uniform4f(uLoc(progFlow, 'u_uvRect'), fcrop[0], fcrop[1], fcrop[2], fcrop[3]);
        gl.uniform1f(uLoc(progFlow, 'u_time'), elapsed);
        const nums = layer.nums || {};
        gl.uniform3f(uLoc(progFlow, 'u_speeds'),
          nums.Speed0 ?? 0.01, nums.Speed1 ?? 0.01, nums.Speed2 ?? 0.01);
        gl.uniform1f(uLoc(progFlow, 'u_amp'), nums.Amount ?? 1);
        gl.uniform1f(uLoc(progFlow, 'u_bright'), nums.Bright ?? 1);
        ...
        for (let ui = 0; ui < 4; ui++) {
          ...
          gl.uniform1i(uLoc(progFlow, units[ui]), ui);
        }
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
        gl.activeTexture(gl.TEXTURE0);
      ...
      if (layer.shader === 'flag' && layer.texUrls && layer.texUrls.length >= 3) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, quadBuf);
        const flPos = aLoc(progFlag, 'a_pos');
        const flUv = aLoc(progFlag, 'a_uv');
        ...
        const model = mat4Transform2D(layer.x, layer.y, layer.w, layer.h, layer.angle || 0);
        gl.uniformMatrix4fv(uLoc(progFlag, 'u_proj'), false, proj);
        gl.uniformMatrix4fv(uLoc(progFlag, 'u_model'), false, model);
        const flcrop = layer.uvCrop || [0, 0, 1, 1];
        gl.uniform4f(uLoc(progFlag, 'u_uvRect'), flcrop[0], flcrop[1], flcrop[2], flcrop[3]);
        gl.uniform1f(uLoc(progFlag, 'u_time'), elapsed);
        const fnums = layer.nums || {};
        gl.uniform1f(uLoc(progFlag, 'u_speed'), fnums.Speed ?? 0.4);
        gl.uniform1f(uLoc(progFlag, 'u_strength'), fnums.Strength ?? 0.5);
        const fcols = layer.userColors || {};
        const fc1 = fcols.color1 || [0, 0, 0];
        const fc2 = fcols.color2 || [0, 0, 0];
        const fc3 = fcols.color3 || [1, 1, 1];
        gl.uniform3f(uLoc(progFlag, 'u_color1'), fc1[0], fc1[1], fc1[2]);
        gl.uniform3f(uLoc(progFlag, 'u_color2'), fc2[0], fc2[1], fc2[2]);
        gl.uniform3f(uLoc(progFlag, 'u_color3'), fc3[0], fc3[1], fc3[2]);
        const funits = ['u_tex', 'u_normal', 'u_cloth'];
        for (let ui = 0; ui < 3; ui++) {
          gl.activeTexture(gl.TEXTURE0 + ui);
          gl.bindTexture(gl.TEXTURE_2D, recs[ui].texture);
          gl.uniform1i(uLoc(progFlag, funits[ui]), ui);
        }
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
        gl.activeTexture(gl.TEXTURE0);
      ...
      gl.uniform4f(uLoc(progBasic, 'u_uvRect'), crop[0], crop[1], crop[2], crop[3]);
      const lnums = layer.nums || {};
      gl.uniform1f(uLoc(progBasic, 'u_bright'), lnums.Bright ?? 1);
      gl.uniform1f(uLoc(progBasic, 'u_power'), lnums.Power ?? 1);
    ...
    if (activeParticles.length > 0) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, quadBuf);
      const pPos = aLoc(progParticle, 'a_pos');
      const pUv = aLoc(progParticle, 'a_uv');
      ...
      gl.uniformMatrix4fv(uLoc(progParticle, 'u_proj'), false, proj);
      gl.uniform4f(uLoc(progParticle, 'u_uvRect'), 0, 0, 1, 1);
      ...
      for (const p of activeParticles) {
        ...
        if (texRec && texRec.loaded) {
          gl.activeTexture(gl.TEXTURE0);
          gl.bindTexture(gl.TEXTURE_2D, texRec.texture);
          gl.uniform1i(uLoc(progParticle, 'u_tex'), 0);
        }
        ...
        if (p.trail && p.trail.length > 1) {
          for (let ti = 0; ti < p.trail.length; ti++) {
            ...
            gl.uniformMatrix4fv(uLoc(progParticle, 'u_model'), false, tModel);
            gl.uniform4f(uLoc(progParticle, 'u_color'), p.color[0], p.color[1], p.color[2], tAlpha);
            gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
          }
        }

        const model = mat4Transform2D(p.x, p.y, p.size * (p.system.type === 'meteor' ? 3 : 1), p.size, Math.atan2(p.vy, p.vx));
        gl.uniformMatrix4fv(uLoc(progParticle, 'u_model'), false, model);
        gl.uniform4f(uLoc(progParticle, 'u_color'), p.color[0], p.color[1], p.color[2], alpha * p.color[3]);
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
      }
      gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    }
  ...
  fetch('/wallpaper-engine/scene-manifest/' + token)
    ...
    .then(data => {
      if (data.ok && data.manifest) {
        ...
        queues3d = buildQueues3d(); // P2-24: 队列分桶随 manifest 到达构建一次
      } else {
