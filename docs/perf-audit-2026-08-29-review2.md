...
## 11. 修复回执(2026-08,HEAD `c00bdb8`,主验收逐条独立复核)

三波全部执行完毕,13 笔提交。所有 CPU 渲染器改动遵守**铁律 MAD=0**
(每波均以 7 壁纸静态帧 vs `.we-fix-accept/p0-5-baseline` 逐字节 sha256 对照,
由主验收独立重跑,非仅代理自报)。

| 提交 | 波 | 内容 | 主验收要点 |
|---|---|---|---|
| `7bd2a84` | 1 | scene-gl 止血(P0-1/P0-2/P1-3①+N-06/07/09) | 读码核对 5 锚点+build/verify |
| `d1a7dcd` | 1 | host 止血(P0-3/P0-4/P1-4/N-01) | probe vs 真值 47 tex 0 mismatch 独立复算 |
| `d860a6a` | 2 | P0-5 分配风暴(scratch 池+写入式) | 7/7 MAD=0 + A/B 56/56 重跑 |
| `cd602ee` | 2 | P1-2 /scene-resource 缓存 | 字节一致性独立重算(哈希与代理一致) |
| `0e70168` | 2 | client P1-5/6/7+P2-1+N-10 | build/verify/标记断言 |
| `796c7a1` | 2 | P0-6 anim 降采样+P1-12 | 主验收发现代理上下文耗尽遗留的 `_upsampleRgba` 未定义缺口,独立补齐为 `_passthroughUpsample`(glsl/integration 最近邻版是静态基线依赖不可改);7/7 MAD=0 |
| `5df0ff2` | 3 | host 缓存治理(P2-10/11/17/19+N-05) | 三套测试重跑 10+16+30000 全过 |
| `f7bb3f7` | 3 | 核心清扫(P2-20/21/22+P1-11半+N-13/15) | 7/7 MAD=0 独立复算 |
| `b31557c` | 3 | GLSL 解释器(P1-8②-⑤+P2-23) | 7/7 MAD=0 + A/B bench 独立复算 −26.5%(rgba 哈希一致) |
| `bdd58be` | 3 | 效果热点(P2-12/13/16/25) | A/B 48+56 重跑;不可逐位项(先加后糊/可分离化/乘倒数)留浮点实证跳过 |
| `5445b92` | 3 | GL 热路径(P2-7/8/24+N-08) | 桩 35/35+23/23 重跑 |
| `1fda36c` | 3 | 粒子/puppet/model(P1-9/10+P2-26+N-14) | 增量模拟不可逐位一致(证明在案)只交构建缓存;多帧序列两遍一致;w3 A/B 全过 |
| `92abcbd`+`dd13024`+`c00bdb8` | 5 | P1-11 收尾+P2-27+P2-14 | escape battery ALL BLOCKED 重跑;mp4 两路径逐字节一致;mp4 端到端 −39.5% |

**实测收益**(各包 bench 口径):GLSL 解释器 −21.7~−26.5%;bloom 2.11×;blit 1.22×;
粒子壁纸 3427824116 −23%;3735447194 −11%;效果层分配 63-85MB/帧→0;
scene-anim mp4 端到端 −39.5%;视频扫描 memoize 54×/头部探测 88×;
/scene-resource 命中 169-2588×。

**延后清单**(架构级/跨模块,需独立议题):
- P2-9 S3TC 纹理直传(host 供原始 DXT 字节 + client compressedTexImage2D 联动)。
- P2-6 emit() selector 架构改造(109 调用点)。
- client 残余:P2-2 拉绳 transform 化、P2-3 :has() 改 data- 属性、P2-4 轮询守卫、
  P2-5 Edge canvas clearRect、N-11/N-12、P3(base64 吉祥物懒加载/classic 虚拟化/
  进度条 transform/死属性清理)。
- P2-15 timings 探测移 worker(部分已由 P0-3 memoize 缓解)。
- 进度端点 ortho 修正的历史键错位(PF-HOST 发现并标注,建议随下一轮)。
- P1-8① 静态帧 GLSL 不降采样:有意决策,不动。
