...
| 归属 | 效果 |
|---|---|
| **由本插件负责**（默认） | 壁纸 + 液态玻璃 + 配色 + 侧栏玻璃 + 字体全部生效。若此时皮肤中心的皮肤正开着，本插件自动进入**让位过渡态**：只保留壁纸画面，全部配色让给皮肤，并在归属卡中引导你两步关闭皮肤；皮肤一关即自动完整接管。 |
| **由皮肤中心负责** | 本插件**完全待机**：卸载壁纸层、清空所有样式覆写，背景与配色全权交给 skin-center。想切回随时在归属卡点「由本插件负责」。 |

要点：

- **互斥是持续执行的**：选定后若再检出对方激活，状态机自动回到过渡/待机态，不需要人工盯防；
- dsh-web-ui-all 的**其余功能完全不受影响**：任务看板、git 图、桌宠、远程移动端、SSH、性能统计、市场等照常使用；
