...
> ✅ **已修复：沉浸式全屏窗口偶尔全屏闪白**（v0.6.4，自动方案）
> 早期版本在**桌面快捷方式打开的沉浸式全屏窗口**（独立应用 / kiosk 窗口）里，点击对话或输入文字时**可能整屏闪白一下**——这是该窗口 + 硬件加速下，Chromium 合成器在毛玻璃对壁纸重采样时偶发把整屏画白。
> **v0.6.4 已自动修复**：插件会自动识别这种窗口，并**仅在其中把毛玻璃降为半透明玻璃**（保留光泽与色调，只是不再模糊）——**无需手动关闭硬件加速**；普通浏览器标签页完全不受影响，保持完整毛玻璃与硬件加速。
