...
## 与 dsh-web-ui-all（皮肤中心）的二选一互斥（v0.7.0）

同时安装本插件与 [`@linxin666/dsh-web-ui-all`](https://github.com/zhu1090093659/dsh-web-ui) 后，双方都能正常运行；但**主题与壁纸相关功能同一时间只有一个生效**——在插件设置页「外观」区顶部的**外观归属卡**里二选一：

| 归属 | 效果 |
|---|---|
| **由本插件负责**（默认） | **无条件全量生效**：壁纸 + 液态玻璃 + 配色 + 侧栏玻璃 + 字体照常——即使皮肤中心的皮肤正开着（v0.7.1 起取消自动让位；不想看到皮肤观感就在皮肤中心关闭当前皮肤）。 |
| **由皮肤中心负责** | 本插件**完全待机**：卸载壁纸层、清空所有样式覆写，背景与配色全权交给 skin-center。想切回随时在归属卡点「由本插件负责」。 |

要点：

- **默认永远全量**：本插件是外观全权方，皮肤的 CSS 观感不参与裁决、不会触发任何自动让位；
- dsh-web-ui-all 的工具型功能不受归属影响：任务看板、git 图、远程移动端、SSH 运维面板、右侧文件/变更面板等照常使用；
- 双方内置壁纸引擎同时运行时会弹出**阻塞式选择卡**强制二选一（避免两层壁纸叠加的性能与行为混乱）；
- 「向皮肤中心上报背景可见性契约」开关（默认开）可一键退回不可见状态；
- 已知边界：本插件不提供皮肤中心的六层装饰图与皮肤画廊替代品；无法编程操作对方的开关（其 hooks 无对外 API），皮肤侧的关闭始终由你在它的设置里完成。
