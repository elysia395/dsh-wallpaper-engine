...
- **卡片样式**：设置页顶部可切换缩略图卡片的两种样式 —— **经典（CD 风）**（CD 架效果：卡片像 CD 盒一样纵向层叠，下排上沿盖住上排下沿、左右不遮挡，鼠标悬停放大置顶；网格更紧凑、每行约 7 个，且**一页到底不翻页**）或 **新版（防重叠）**（重写后的固定高度卡片，图片绝对定位填满，任何浏览器都不会重叠；默认）。选择保存在浏览器 `localStorage`。
