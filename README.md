...
> ⚠️ **故障排除 · 沉浸式窗口偶尔全屏闪白**
> **两种窗口要分开看：**
> - **沉浸式全屏窗口**（桌面快捷方式打开的独立应用 / kiosk 窗口）：点击对话或输入文字时**可能整屏闪白一下**。这是 Chromium 独立/全屏窗口在开启**硬件加速**时，与动态壁纸合成偶发的合成器问题。
> - **普通浏览器标签页**：**不会闪白**，且需要**硬件加速开启**才流畅。
>
> 插件已**自动区分**这两种窗口：在沉浸式窗口里**降低合成**（去掉毛玻璃模糊、保留半透明玻璃）；普通标签页**保持完整毛玻璃**。
>
> 如果沉浸式窗口在插件降级后仍闪白：**仅在该窗口使用的浏览器配置里**关闭「使用硬件加速(可用时)」并重启——沉浸式窗口即使关硬件加速也很顺滑。
>
> ⚠️ **注意**：硬件加速是**每个浏览器实例**的全局设置。如果你同时用普通标签页，**请保持其「使用硬件加速开启」以免变卡**（普通标签页不闪白）。两者若共用同一个实例，可给沉浸式窗口用独立配置（不同的 `--user-data-dir`），或直接**改用普通标签页**（无闪白、硬件加速正常开、性能最佳）。
>
> 插件更新后会弹一次提示，告知此问题与解决方法（每个新版本仅出现一次）。
