...
const FONT_FAMILY_VALUES = ["inherit", "Microsoft YaHei", "KaiTi", "SimSun", "SimHei", "monospace"];
// 字体族按钮的展示标签（顺序即 UI 排列顺序，与 FONT_FAMILY_VALUES 一一对应）。
...
function applyEffects() {
  ...
  if (selection.fontCustom) {
    ...
    const familyStack = selection.fontFamily === "inherit"
      ? "inherit"
      : '"' + selection.fontFamily + '", "Microsoft YaHei", sans-serif';
    s.setProperty("--we-font-family", familyStack);
    applyFontStyles();
  } else {
        ...
        React.createElement("div", { className: "we-picker__row we-picker__accent-row" },
          ...
          FONT_FAMILY_LABELS.map((f) =>
            React.createElement("button", {
              key: f.v,
              type: "button",
              className: "we-picker__swatch" + (sel.fontFamily === f.v ? " we-picker__swatch--active" : ""),
              style: { fontSize: "11px" },
              title: f.v === "inherit" ? "跟随 dsh 原生字体栈" : f.v,
