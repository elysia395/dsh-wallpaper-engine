...
const selection = {
  ...
  type: null,
  previewUrl: null,
  // Live WebGL scene player URL (transient, set at apply time). When a scene
  // wallpaper is selected and this is non-null, buildMedia renders it as a
  // sandboxed same-origin iframe instead of the static frame.
  sceneUrl: null,
  // Scene animation exposed as an MP4 (<video>). Preferred over the live WebGL
  // player: hardware-decoded and smooth, no per-scene WebGL context.
  sceneVideo: null,
...
function applySelection(id) {
  ...
  // Live WebGL scene player (when the host advertised a sceneUrl). buildMedia
  // renders it as a same-origin iframe; frameUrl stays the static-frame fallback
  // for scenes that fail to parse or render.
  selection.sceneUrl = w.type === "scene" ? (w.sceneUrl || null) : null;
...
function buildMedia(sel) {
  // A scene wallpaper renders as a live WebGL iframe when the host advertised a
  // ⚠️ Live WebGL scene rendering is currently DISABLED: it can spin up
  // multiple WebGL contexts and freeze the page (卡死). Scenes fall back to the
  // Scene wallpapers: prefer the scene animation exposed as an MP4 <video>
  // (hardware-decoded, smooth, no WebGL context) — the non-freezing path
  // dsh-web-ui uses. Scenes without an embedded video fall back to the extracted
  // static frame.
...
function syncLayers() {
  ...
  if (selection.url) {
    const wantKey = selection.type + "\u0000" + selection.url + "\u0000"
      ...
      + "\u0000" + (selection.sceneUrl || "") + "\u0000" + (selection.sceneVideo || "");
    const gotKey = existing && existing.dataset.weKey;
    if (existing && gotKey !== wantKey) {
      // Stop + release the previous wallpaper's media BEFORE removing it so
      // rapid wallpaper switches never accumulate live/decoding media (the page
      // would otherwise grind to a halt as old <video>s keep decoding).
      const prevVideo = existing.querySelector("video");
      if (prevVideo) {
        try { prevVideo.pause(); prevVideo.removeAttribute("src"); prevVideo.load(); } catch { /* ignore */ }
      }
      const prevIframe = existing.querySelector("iframe");
      if (prevIframe) { try { prevIframe.src = "about:blank"; } catch { /* ignore */ } }
      existing.remove();
    ...
    // Scene player: mirror the effective playing state on every layer sync so
    // the WebGL render loop pauses when the window hides / loses focus /
    // battery-saves, instead of burning GPU while covered (matches the video
    // occlusion-pause behavior).
    const scenePlayer = document.querySelector("iframe[data-dsh-scene-player]");
    if (scenePlayer) {
      const paused = !isEffectivelyPlaying();
      try {
        scenePlayer.contentWindow?.postMessage({ type: "dsh-set-pause", paused: paused }, location.origin);
        if (!paused) scenePlayer.contentWindow?.postMessage({ type: "dsh-recover-renderer" }, location.origin);
      } catch { /* ignore */ }
    }
  } else if (existing) {
    weStopDraw();
...
// ── Scene player ↔ parent messaging ─────────────────────────────────────────
// The sandboxed scene iframe runs its own document; the parent steers it with
// postMessage (fit / pause / recover) and the player asks the parent to rebuild
// on a render crash (context loss) or to fall back to the static frame when it
// cannot build its manifest.
if (typeof window !== "undefined" && typeof window.addEventListener === "function") {
  window.addEventListener("message", (ev) => {
    const iframe = document.querySelector("iframe[data-dsh-scene-player]");
    const msg = ev.data;
    if (!msg || typeof msg !== "object" || !iframe) return;
    if (ev.source !== iframe.contentWindow || ev.origin !== location.origin) return;
    if (msg.type === "dsh-scene-needs-reload") {
      // Don't reload the iframe (it churns WebGL contexts and can freeze the
      // page); degrade to the static frame instead.
      if (selection.sceneUrl) {
        selection.sceneUrl = null;
        try { syncLayers(); emit(); } catch { /* ignore */ }
      }
    } else if (msg.type === "dsh-scene-unavailable") {
      if (selection.sceneUrl) {
        selection.sceneUrl = null;
        try { syncLayers(); emit(); } catch { /* ignore */ }
      }
