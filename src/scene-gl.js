...
function createSceneGLRenderer(opts) {
  ...
  async function buildResources() {
    ...
    const objResList = await Promise.all(sceneObjects.map(async (obj, oi) => {
      ...
      // 效果纹理（键 = mask 路径：同对象挂两个同 shader 效果（3427824116
      // 图层 1 双 foliagesway — 两侧花丛 mask + 头发 mask）时，旧键
      // ef.shader+':'+slot 完全相同的两个实例并发 set 互相覆盖，后完成者赢
      // （赛车）→ 两个 pass 绑同一张 mask：头发不摆、两侧摆两次，被误读为
      // "沿用了上一张壁纸的特效"。路径键唯一且同路径天然共享，与 loadOne
      // 去重口径一致）
      ...
      await Promise.all((obj.effects || []).map(async (ef) => {
        for (let slot = 1; slot < ef.textures.length; slot++) {
          ...
          effectTex.set(info.path, await loadOne(info, true, false)); // slot1/2 REPEAT
        }
      }));
    ...
    for (let i = 0; i < o.programs.length; i++) {
      ...
      for (let s = 0; s < p.texSlots.length; s++) {
        ...
        const entry = (slotInfo && slotInfo.previous === true) ? o.mainTexEntry
          : ((slotInfo && slotInfo.path && o.effectTex.get(slotInfo.path))
